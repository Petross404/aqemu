...
bool Network_Widget::Get_Network_Cards( QList<VM_Net_Card_Native> &cards )
{
	...
	{
		AQDebug( "bool Network_Widget::Get_Network_Card( "
			 "QList<VM_Net_Card_Native> &cards ) const",
			 "No Items" );
		cards.clear();
		return true;
	}
...
void Network_Widget::Set_Network_Cards( const QList<VM_Net_Card_Native> &cards )
{
		...
		for ( int nx = 0; nx < Network_Cards.count() && nx < 8; ++nx )
		{
			...
			switch ( Network_Cards[nx].Get_Network_Type() )
			{
				...
				case VM::Net_Mode_Native_Dump:
					new QListWidgetItem(
						"Dump " + QString::number( ++dump ),
						ui.Items_List );
					break;

				default:
					new QListWidgetItem(
						tr( "Unknown Type Item" ),
						ui.Items_List );
					break;
			}
		}
	...
	else { Enable_Buttons( true, false ); }
}
...
{
	connect( ui.CB_Network_Type,
		 SIGNAL( currentIndexChanged( int ) ),
		 this,
		 SIGNAL( Changed() ) );

	connect( ui.CB_model, SIGNAL( currentIndexChanged( int ) ), this, SIGNAL( Changed() ) );

	connect( ui.CH_macaddr, SIGNAL( clicked() ), this, SIGNAL( Changed() ) );

	connect( ui.Edit_macaddr,
		 SIGNAL( textChanged( const QString & ) ),
		 this,
		 SIGNAL( Changed() ) );

	connect( ui.CH_vlan, SIGNAL( clicked() ), this, SIGNAL( Changed() ) );

	connect( ui.SB_vlan, SIGNAL( valueChanged( int ) ), this, SIGNAL( Changed() ) );

	connect( ui.CH_name, SIGNAL( clicked() ), this, SIGNAL( Changed() ) );

	connect( ui.Edit_name,
		 SIGNAL( textChanged( const QString & ) ),
		 this,
		 SIGNAL( Changed() ) );
	...
	connect( ui.Edit_hostname,
		 SIGNAL( textChanged( const QString & ) ),
		 this,
		 SIGNAL( Changed() ) );

	connect( ui.Edit_port_dev,
		 SIGNAL( textChanged( const QString & ) ),
		 this,
		 SIGNAL( Changed() ) );
	...
	connect( ui.Edit_ifname,
		 SIGNAL( textChanged( const QString & ) ),
		 this,
		 SIGNAL( Changed() ) );

	connect( ui.Edit_bridge,
		 SIGNAL( textChanged( const QString & ) ),
		 this,
		 SIGNAL( Changed() ) );
	...
	connect( ui.CB_restrict, SIGNAL( currentIndexChanged( int ) ), this, SIGNAL( Changed() ) );

	connect( ui.CH_dhcpstart, SIGNAL( clicked() ), this, SIGNAL( Changed() ) );

	connect( ui.Edit_dhcpstart,
		 SIGNAL( textChanged( const QString & ) ),
		 this,
		 SIGNAL( Changed() ) );
	...
	connect( ui.CB_vnet_hdr, SIGNAL( currentIndexChanged( int ) ), this, SIGNAL( Changed() ) );

	connect( ui.CH_vhost, SIGNAL( clicked() ), this, SIGNAL( Changed() ) );

	connect( ui.CB_vhost, SIGNAL( currentIndexChanged( int ) ), this, SIGNAL( Changed() ) );
...
{
	disconnect( ui.CB_Network_Type,
		    SIGNAL( currentIndexChanged( int ) ),
		    this,
		    SIGNAL( Changed() ) );

	disconnect( ui.CB_model, SIGNAL( currentIndexChanged( int ) ), this, SIGNAL( Changed() ) );

	disconnect( ui.CH_macaddr, SIGNAL( clicked() ), this, SIGNAL( Changed() ) );

	disconnect( ui.Edit_macaddr,
		    SIGNAL( textChanged( const QString & ) ),
		    this,
		    SIGNAL( Changed() ) );

	disconnect( ui.CH_vlan, SIGNAL( clicked() ), this, SIGNAL( Changed() ) );
	...
	disconnect( ui.Edit_hostname,
		    SIGNAL( textChanged( const QString & ) ),
		    this,
		    SIGNAL( Changed() ) );

	disconnect( ui.Edit_port_dev,
		    SIGNAL( textChanged( const QString & ) ),
		    this,
		    SIGNAL( Changed() ) );
	...
	disconnect( ui.CB_len, SIGNAL( currentIndexChanged( int ) ), this, SIGNAL( Changed() ) );

	disconnect( ui.CH_addr, SIGNAL( clicked() ), this, SIGNAL( Changed() ) );

	disconnect( ui.Edit_addr,
		    SIGNAL( textChanged( const QString & ) ),
		    this,
		    SIGNAL( Changed() ) );
	...
	disconnect( ui.CB_vhost, SIGNAL( currentIndexChanged( int ) ), this, SIGNAL( Changed() ) );
...
void Network_Widget::on_CB_Network_Type_currentIndexChanged( int index )
{
	...
	else
	{
		if ( ui.CB_Network_Type->count() > 0 )
			AQError( "void "
				 "Network_Widget::on_CB_Network_Type_"
...
VM_Net_Card_Native Network_Widget::Get_Net_Card_From_Ui() const
{
	...
	{
		case 0: card.Set_Network_Type( VM::Net_Mode_Native_NIC ); break;

		case 1: card.Set_Network_Type( VM::Net_Mode_Native_User ); break;

		case 2: card.Set_Network_Type( VM::Net_Mode_Native_Chanel ); break;

		case 3: card.Set_Network_Type( VM::Net_Mode_Native_Bridge ); break;

		case 4: card.Set_Network_Type( VM::Net_Mode_Native_TAP ); break;

		case 5: card.Set_Network_Type( VM::Net_Mode_Native_Socket ); break;

		case 6:
			card.Set_Network_Type( VM::Net_Mode_Native_MulticastSocket );
			break;

		case 7: card.Set_Network_Type( VM::Net_Mode_Native_VDE ); break;

		case 8: card.Set_Network_Type( VM::Net_Mode_Native_Dump ); break;

		default:
			AQError( "VM_Net_Card_Native "
				 "Network_Widget::Get_Net_Card_From_Ui() const",
				 "Cannot Read Network Type! Use Default: User "
				 "Mode" );
	...
	if ( ui.CB_model->currentIndex() != -1 )
	{
		if ( Card_Models_QEMU_Name.count() - 1 >= ui.CB_model->currentIndex() )
		{
			// Model
			card.Set_Card_Model(
				Card_Models_QEMU_Name[ui.CB_model->currentIndex()] );
		}
	}
	...
	{
		case 0: card.Set_Len( 64 ); break;

		case 1: card.Set_Len( 128 ); break;

		case 2: card.Set_Len( 512 ); break;

		case 3: card.Set_Len( 768 ); break;

		case 4: card.Set_Len( 1024 ); break;

		case 5: card.Set_Len( 5120 ); break;

		case 6: card.Set_Len( 10240 ); break;

		default: card.Set_Len( 64 ); break;
	}
...
void Network_Widget::Set_Net_Card_To_Ui( const VM_Net_Card_Native &card )
{
	switch ( card.Get_Network_Type() )
	{
		...
		default:
			AQError( "void Network_Widget::Set_Net_Card_To_Ui( const "
				 "VM_Net_Card_Native &card )",
				 "Cannot Read Network Type! Use Default: User "
				 "Mode" );
			ui.CB_Network_Type->setCurrentIndex( 1 );
	...
	switch ( card.Get_Len() )
	{
		case 64: ui.CB_len->setCurrentIndex( 0 ); break;

		case 128: ui.CB_len->setCurrentIndex( 1 ); break;

		case 512: ui.CB_len->setCurrentIndex( 2 ); break;

		case 768: ui.CB_len->setCurrentIndex( 3 ); break;

		case 1024: ui.CB_len->setCurrentIndex( 4 ); break;

		case 5120: ui.CB_len->setCurrentIndex( 5 ); break;

		case 10240: ui.CB_len->setCurrentIndex( 6 ); break;

		default: ui.CB_len->setCurrentIndex( 0 ); break;
	}
...
bool Network_Widget::Net_Card_is_Valid()
{
	bool u_macaddr, u_name, u_hostname, u_port_dev, u_ifname, u_script,
		...
		u_sock, u_group, u_mode, u_file, u_len;

	u_macaddr = u_name = u_hostname = u_port_dev = u_ifname = u_script =
		u_downscript = u_bridge = u_helper = u_listen = u_connect =
			u_mcast = u_sock = u_group = u_mode = u_file = u_len = false;

	switch ( ui.CB_Network_Type->currentIndex() )
	{
		// -net nic[,vlan=n][,macaddr=addr][,model=type][,name=name]
		case 0: u_macaddr = u_name = true; break;

		// -net user[,vlan=n][,hostname=name][,name=name]
		case 1: u_hostname = u_name = true; break;

		// -net channel,port:dev
		case 2: u_port_dev = true; break;

		// -net bridge[,vlan=n][,name=name][,br=bridge][,helper=helper]
		case 3: u_name = u_bridge = u_helper = true; break;
		...
		// -net socket[,vlan=n][,name=name][,fd=h][,listen=[host]:port][,connect=host:port]
		case 5: u_name = u_listen = u_connect = true; break;

		// -net socket[,vlan=n][,name=name][,fd=h][,mcast=maddr:port]
		case 6: u_name = u_mcast = true; break;

		// -net vde[,vlan=n][,name=name][,sock=socketpath][,port=n][,group=groupname][,mode=octalmode]
		case 7: u_name = u_sock = u_group = u_mode = true; break;

		// -net dump[,vlan=n][,file=file][,len=len]
		case 8: u_file = u_len = true; break;
...
void Network_Widget::Update_Card_Item()
{
	...
	if ( ui.Edit_name->text().isEmpty() == false && ui.CH_name->isChecked() == true )
	{
	...
	switch ( ui.CB_Network_Type->currentIndex() )
	{
		case 0:
			ui.Items_List->currentItem()->setText(
				"NIC "
				+ ( item_name.isEmpty() ? Get_Items_Count( VM::Net_Mode_Native_NIC )
							: n ) );
			break;
		...
		case 1:
			ui.Items_List->currentItem()->setText(
				"User "
				+ ( item_name.isEmpty() ? Get_Items_Count( VM::Net_Mode_Native_User )
							: n ) );
			break;
		...
		case 3:
			ui.Items_List->currentItem()->setText(
				"Bridge "
				+ ( item_name.isEmpty() ? Get_Items_Count( VM::Net_Mode_Native_Bridge )
							: n ) );
			break;
		...
		case 4:
			ui.Items_List->currentItem()->setText(
				"TAP "
				+ ( item_name.isEmpty() ? Get_Items_Count( VM::Net_Mode_Native_TAP )
							: n ) );
			break;
		...
		case 5:
			ui.Items_List->currentItem()->setText(
				"Socket "
				+ ( item_name.isEmpty() ? Get_Items_Count( VM::Net_Mode_Native_Socket )
							: n ) );
			break;
		...
		case 6:
			ui.Items_List->currentItem()->setText(
				"Multicast Socket "
				+ ( item_name.isEmpty() ? Get_Items_Count( VM::Net_Mode_Native_MulticastSocket )
							: n ) );
			break;
		...
		case 7:
			ui.Items_List->currentItem()->setText(
				"VDE "
				+ ( item_name.isEmpty() ? Get_Items_Count( VM::Net_Mode_Native_VDE )
							: n ) );
			break;
