	...
	if ( ui.Devices_List->currentItem() != NULL )
	{
		     ...
		     || ui.Devices_List->currentItem()->data( 512 ).toString() == "fd2" )
		{
			...
			if ( ui.Devices_List->currentItem()->data( 512 ).toString() == "fd1" )
			{
				...
				{
					QFileInfo fd_img =
						QFileInfo( Floppy1.Get_File_Name() );
					...
					if ( fd_img.exists() )
					{
						...
						else
						{
							ui.Label_Connected_To->setText(
								tr( "Type: Image" )
								+ "\n" + tr( "On Disk Size: " )
								+ QString::number( (int)size_in_bytes
										   / 1024.0 )
								+ tr( "KB" ) );
						}
					}
			...
			else
			{
				...
				{
					QFileInfo fd_img =
						QFileInfo( Floppy2.Get_File_Name() );
		...
		else
		{
			...
			for ( int fx = 0; fx < 32; ++fx )
			{
				     ...
				     == "device" + QString::number( fx ) )
				{
					...
					Context_Menu->exec(
						ui.Devices_List->mapToGlobal( pos ) );
				}
			}
...
void Device_Manager_Widget::on_Devices_List_currentItemChanged( QListWidgetItem *current,
...
void Device_Manager_Widget::Add_Floppy( VM_Storage_Device &dev, int num )
{
	...
	if ( !QFileInfo( dev_name ).exists() ) return;
...
void Device_Manager_Widget::on_actionAdd_Floppy_triggered()
{
	if ( !Floppy1.Get_Enabled() )
	{
		...
		if ( pw->exec() == QDialog::Accepted ) { Add_Floppy( Floppy1, 1 ); }
	}
	else if ( !Floppy2.Get_Enabled() )
	{
		...
		if ( pw->exec() == QDialog::Accepted ) { Add_Floppy( Floppy2, 2 ); }
	}
	else
	{
...
void Device_Manager_Widget::on_actionAdd_HDD_triggered()
{
	if ( !HDA.Get_Enabled() )
	{
		...
		if ( pw->exec() == QDialog::Accepted ) { Add_HDD( HDA, "A" ); }
	}
	else if ( !HDB.Get_Enabled() )
	{
		...
		if ( pw->exec() == QDialog::Accepted ) { Add_HDD( HDB, "B" ); }
	}
	else if ( HDC.Get_Enabled() == false && CD_ROM.Get_Enabled() == false )
	{
		...
		if ( pw->exec() == QDialog::Accepted ) { Add_HDD( HDC, "C" ); }
	}
	else if ( !HDD.Get_Enabled() )
	{
		...
		if ( pw->exec() == QDialog::Accepted ) { Add_HDD( HDD, "D" ); }
	}
	else
	{
		AQGraphic_Warning(
...
void Device_Manager_Widget::on_actionAdd_Device_triggered()
{
	...
	if ( Device_Window.exec() == QDialog::Accepted )
	{
		...
		QListWidgetItem *devit = new QListWidgetItem(
			...
			Device_Window.Get_Device().Get_QEMU_Device_Name(),
			ui.Devices_List );
		devit->setData( 512,
...
void Device_Manager_Widget::on_actionProperties_triggered()
{
	if ( ui.Devices_List->currentItem()->data( 512 ).toString() == "fd1" )
	{
		...
		if ( !Current_Machine_Devices )
			AQError( "void "
				 "Device_Manager_Widget::on_actionProperties_"
				 "triggered()",
		...
		for ( int fx = 0; fx < 32; ++fx )
		{
			     ...
			     == "device" + QString::number( fx ) )
			{
				...
				if ( !Current_Machine_Devices )
					AQError(
						"void "
						"Device_Manager_Widget::on_"
						"actionProperties_triggered()",
						"Current_Machine_Devices == NULL" );
				Device_Window.Set_Emulator_Devices( *Current_Machine_Devices );

				if ( Device_Window.exec() == QDialog::Accepted )
				{
					if ( Storage_Devices[fx]
					     != Device_Window.Get_Device() )
					{
						Storage_Devices[fx] =
							Device_Window.Get_Device();
...
{
	int mes_ret = QMessageBox::question( this,
					     tr( "Delete?" ),
					     tr( "Delete Device?" ),
	...
	else
	{
		...
		{
			AQError( "void "
				 "Device_Manager_Widget::on_actionDelete_triggered("
				 ")",
				 "Incorrect Device!" );
			return;
		}
		else
		{
			...
			{
				if ( ui.Devices_List->item( ix )
					     ->data( 512 )
					     .toString()
					     .contains( "device" ) )
				{
					ui.Devices_List->item( ix )->setData(
						512,
...
void Device_Manager_Widget::on_actionFormat_HDD_triggered()
{
	...
	else
	{
		AQGraphic_Error(
			"void "
			"Device_Manager_Widget::on_actionFormat_HDD_triggered()",
			tr( "Error!" ),
			tr( "Incorrect Device!" ),
			false );
		return;
...
void Device_Manager_Widget::on_actionQuick_Format_triggered()
{
	...
	else
	{
		AQGraphic_Error(
			"void "
			"Device_Manager_Widget::on_actionQuick_Format_triggered()",
			tr( "Error!" ),
			tr( "Incorrect Device!" ),
			false );
		return;
...
void Device_Manager_Widget::Update_Icons()
{
	...
	if ( CD_ROM.Get_Enabled() )
	{
		QListWidgetItem *cdit = new QListWidgetItem(
			QIcon( ":/cdrom.png" ),
			tr( "CD-ROM" ) + " (" + CD_ROM.Get_File_Name() + ")",
			ui.Devices_List );
		cdit->setData( 512, "cd" );
	...
	if ( Storage_Devices.count() )
	{
		for ( int ix = 0; ix < Storage_Devices.count(); ++ix )
		{
			QListWidgetItem *hdit = new QListWidgetItem(
				QIcon( ":/blockdevice.png" ),
				Storage_Devices[ix].Get_QEMU_Device_Name(),
				ui.Devices_List );
