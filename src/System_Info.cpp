...
bool System_Info::Update_VM_Computers_List()
{
	...
	QEMU_Video_Cards_v0_10_0
		<< Device_Map( QObject::tr( "StdVGA (VESA 2.0)" ), "std" );
	QEMU_Video_Cards_v0_10_0
		<< Device_Map( QObject::tr( "Cirrus CLGD 5446" ), "cirrus" );
	QEMU_Video_Cards_v0_10_0
		<< Device_Map( QObject::tr( "VMWare Video Card" ), "vmware" );
	...
	Machine_ARM_0_10_0 << Device_Map(
		QObject::tr( "Integrator/CP (ARM926EJ-S)" ),
		"integratorcp" );
	Machine_ARM_0_10_0 << Device_Map(
		QObject::tr( "Versatile/PB (ARM926EJ-S)" ),
		"versatilepb" );
	Machine_ARM_0_10_0 << Device_Map(
		QObject::tr( "Versatile/AB (ARM926EJ-S)" ),
		"versatileab" );
	...
	Machine_ARM_0_10_0 << Device_Map(
		QObject::tr( "Marvell 88w8618 / MusicPal" ),
		"musicpal" );
	...
	ad.Machine_List << Device_Map(
		QObject::tr( "SPARCstation Voyager (Sun4m)" ),
		"Voyager" );
...
VM::Emulator_Version System_Info::Get_Emulator_Version( const QString &path )
{
	...
	{
		AQWarning(
			"VM::Emulator_Version System_Info::Get_Emulator_Version( "
			"const QString &path )",
			"path is Empty" );
		return VM::Obsolete;
	}
	...
	for ( int ix = 0; ix < 5; ++ix )
	{
		...
		if ( tmpLine.isEmpty() )
		{
			AQError( "VM::Emulator_Version "
				 "System_Info::Get_Emulator_Version( const QString "
				 "&path )",
	...
	if ( line.isEmpty() )
	{
		AQError( "VM::Emulator_Version System_Info::Get_Emulator_Version( "
			 "const QString &path )",
...
QMap<QString, QString> System_Info::Find_QEMU_Binary_Files( const QString &path )
{
	...
	"qemu" ] = dirPath + "qemu-system-i386.exe"; #else if( QFile::exists(dirPath
	+ "qemu-system-i386") ) emulFiles[ "qemu" ] = dirPath + "qemu-system-i386";
		#endif
	}*/
...
QString System_Info::Find_IMG( const QStringList &paths )
{
	if ( paths.isEmpty() )
	{
		AQError( "static QString System_Info::Find_IMG( const QStringList "
			 "&paths )",
			 "paths.isEmpty()" );
		return "";
	}
	...
	AQWarning(
		"static QString System_Info::Find_IMG( const QStringList &paths )",
						  ...
						  const QString &internalName )
{
	if ( !QFile::exists( path ) )
	{
		AQError( "Available_Devices System_Info::Get_Emulator_Info( const "
			 ...
			 "VM::Emulator_Version version, const QString "
			 "&internalName )",
	...
	if ( rx.exactMatch( all_help ) )
	{
		...
		QString emulDevList =
			Get_Emulator_Output( path, QStringList() << "-device" << "?" );
	...
	switch ( version )
	{
		...
		default:
			AQError(
				"Available_Devices System_Info::Get_Emulator_Info( "
	...
	do {
			...
			|| qemu_dev_name.indexOf( QRegExp( "/^\\S+$/" ), 0 ) != -1 ) )
		{
			...
			if ( cpu_found == false )
				tmp_dev.CPU_List
					<< Device_Map( qemu_dev_name, qemu_dev_name );
		}
		else
			continue;
	} while ( !tmp.isNull() );
	...
	if ( tmp_dev.CPU_List.count() < 2 )
	{
		...
		AQWarning( "Available_Devices System_Info::Get_Emulator_Info( "
			   ...
			   "VM::Emulator_Version version, const QString "
			   "&internalName )",
			   QString( "Cannot get CPU's info from emulator \"%1\". "
				    "Use default list" )
				   .arg( path ) );
	}
	...
	do {
		...
		if ( tmp_rx.exactMatch( tmp ) )
		{
			...
			{
				AQError( "Available_Devices "
					 "System_Info::Get_Emulator_Info( const "
					 "QString &path, bool *ok,"
					 "VM::Emulator_Version version, const "
			...
			|| dev_map.QEMU_Name.indexOf( QRegExp( "/^\\S+$/" ), 0 ) != -1 ) )
		{
			...
			{
				if ( dev_map.QEMU_Name
				     == default_device.Machine_List[ix].QEMU_Name )
				{
					tmp_dev.Machine_List
						<< default_device.Machine_List[ix];
					machine_found = true;
				}
			}
	...
	if ( tmp_dev.Machine_List.count() < 2 )
	{
		...
		AQWarning( "Available_Devices System_Info::Get_Emulator_Info( "
			   ...
			   "VM::Emulator_Version version, const QString "
			   "&internalName )",
			   QString( "Cannot get machines info from emulator "
				    "\"%1\". Use default list" )
				   .arg( path ) );
	}
		...
		if ( rx.capturedTexts().count() > 1 )
		{
			...
			else
			{
				for ( int gx = 0; gx < vga_devices_list.count(); gx++ )
				{
					...
					for ( int ix = 0;
					      ix < default_device.Video_Card_List.count();
					      ix++ )
					{
								...
								.QEMU_Name )
						{
							tmp_dev.Video_Card_List
								<< default_device.Video_Card_List[ix];
							vga_found = true;
						}
					}
					...
					// No this device name in default list
					if ( vga_found == false )
						tmp_dev.Video_Card_List << Device_Map(
							vga_devices_list[gx],
							vga_devices_list[gx] );
				}
			}
	...
	do {
		...
		{
			if ( !qemu_dev_name.isEmpty() )
				AQWarning( "Available_Devices "
					   "System_Info::Get_Emulator_Info( const "
					   "QString &path, bool *ok,"
					   "VM::Emulator_Version version, const "
					   "QString &internalName )",
					   "Unregistred Sound Card Name: \""
						   + qemu_dev_name + "\"" );
			continue;
		}
	} while ( !tmp.isNull() );
	...
	do {
		...
		QString all_models = tmp.mid(
			tmp.indexOf( "models:" ) + QString( "models:" ).count(),
			tmp.count() );
		...
		for ( int ax = 0; ax < net_cards_models.count(); ax++ )
		{
			...
			for ( int bx = 0; bx < default_device.Network_Card_List.count(); bx++ )
			{
				...
				if ( net_dev_str
				     == default_device.Network_Card_List[bx].QEMU_Name )
				{
					tmp_dev.Network_Card_List
						<< default_device.Network_Card_List[bx];
					net_found = true;
					break;
				}
			}
			...
			if ( net_found == false )
				tmp_dev.Network_Card_List
					<< Device_Map( net_dev_str, net_dev_str );
		}
	...
	if ( tmp_dev.Network_Card_List.count() < 2 )
	{
		...
		AQWarning( "Available_Devices System_Info::Get_Emulator_Info( "
			   ...
			   "VM::Emulator_Version version, const QString "
			   "&internalName )",
			   QString( "Cannot get net cards info from emulator "
				    "\"%1\". Use default list" )
				   .arg( path ) );
	}
...
QString System_Info::Get_Emulator_Help_Output( const QString &path )
{
	...
	if ( result.isEmpty() )
	{
		...
		if ( result.isEmpty() )
		{
			AQError( "QString System_Info::Get_Emulator_Help_Output( "
				 "const QString &path )",
...
QString System_Info::Get_Emulator_Output( const QString &path, const QStringList &args )
{
	...
	{
		AQError( "QStringList System_Info::Get_Emulator_Output( const "
			 "QString &path, const QStringList &args )",
...
bool System_Info::Scan_USB_Sys( QList<VM_USB> &list )
{
	...
	for ( int dx = 0; dx < usb_dirs.count(); dx++ )
	{
		...
		{
			AQError(
				"bool System_Info::Scan_USB_Sys( QList<VM_USB> "
				"&list )",
		...
		{
			AQError( "bool System_Info::Scan_USB_Sys( QList<VM_USB> "
				 "&list )",
				 "Cannot read idProduct from "
				 "/sys/bus/usb/devices/" );
			continue;
		}
		...
		{
			AQWarning( "bool System_Info::Scan_USB_Sys( QList<VM_USB> "
				   "&list )",
				   "Cannot read manufacturer from "
				   "/sys/bus/usb/devices/" );
...
bool System_Info::Read_SysFS_File( const QString &path, QString &data )
{
	...
	{
		AQWarning( "bool System_Info::Read_SysFS_File( const QString "
			   "&path, QString &data )",
...
bool System_Info::Scan_USB_Proc( QList<VM_USB> &list )
{
	...
	{
		AQError( "bool System_Info::Scan_USB_Proc( QList<VM_USB> &list )", "Cannot Open File \"/proc/bus/usb/devices\"! USB information not found!" );
		return false;
	}
	...
	for ( int ex = 0; ex < linux_usb_dev.count(); ++ex )
	{
		...
		{
			AQWarning( "bool System_Info::Scan_USB_Proc( QList<VM_USB> "
				   "&list )",
	...
	for ( int ix = 0; ix < linux_usb_dev.count(); ix++ )
	{
		...
		for ( int bx = 0; bx < linux_usb_dev[ix].count(); bx++ )
		{
			if ( busAddr_list.count() <= 0 )
			{
				...
				{
					AQError( "bool System_Info::Scan_USB_Proc( "
						 "QList<VM_USB> &list )",
						 "Cannot Match busAddr! String: "
							 + linux_usb_dev[ix][bx] );
				}
			}
...
void System_Info::Get_Free_Memory_Size( int &allRAM, int &freeRAM )
{
	...
	if ( QFile::exists( "/proc/meminfo" ) )
	{
		...
		if ( !proc_file.open( QIODevice::ReadOnly | QIODevice::Text ) )
		{
			AQDebug( "int System_Info::Get_Free_Memory_Size()", "void System_Info::Get_Free_Memory_Size( int &allRAM, int &freeRAM )" );
		}
		else
		{
			QTextStream in( &proc_file );
			...
			while ( !line.isNull() )
			{
				if ( ( all_mem != -1 ) && ( free_mem != -1 )
				     && ( cached_mem != -1 ) && ( buf_mem != -1 ) )
				{
					allRAM = (int)( all_mem / 1024.0 );
					freeRAM = (int)( ( free_mem + cached_mem + buf_mem )
							 / 1024.0 );
					return;	   // exit this
				}
	...
	allRAM = (int)(unsigned long)( sysInfo.totalram * sysInfo.mem_unit )
		 / 1024.0 / 1024.0;
...
QStringList System_Info::Get_Host_CDROM_List()
{
	...
	{
		tmp_list << settings.value( "Additional_CDROM_Devices/Device"
						    + QString::number( ix ),
					    "" )
				    .toString();
	}
...
void System_Info::Get_Free_Memory_Size( int &allRAM, int &freeRAM )
{
	...
	if ( ( sysctlbyname( "vm.stats.vm.v_page_count", &all_ram, &len, NULL, 0 ) == -1 )
	     || !len )
	{
		all_ram = 0;
		AQError( "void  System_Info::Get_Free_Memory_Size( int &allRAM, "
			 "int &freeRAM )",
			 "Cannot Get Information on Memory! "
			 "(vm.stats.vm.v_page_count)" );
	}

	if ( ( sysctlbyname( "vm.stats.vm.v_free_count", &free_ram, &len, NULL, 0 ) == -1 )
	     || !len )
	{
		free_ram = 0;
		AQError( "void  System_Info::Get_Free_Memory_Size( int &allRAM, "
	...
	if ( ( sysctlbyname( "vm.stats.vm.v_cache_count", &cached_mem, &len, NULL, 0 ) == -1 )
	     || !len )
	{
		cached_mem = 0;
		AQError( "void  System_Info::Get_Free_Memory_Size( int &allRAM, "
...
QStringList System_Info::Get_Host_FDD_List()
{
	...
	{
		if ( buf[ix] != NULL ) { tmp += (char)buf[ix]; }
		else
		{
			...
			if ( uDriveType == DRIVE_REMOVABLE ) { ret_list << tmp; }
...
QStringList System_Info::Get_Host_CDROM_List()
{
	...
	for ( int ix = 0; ix < buf_size - 1; ix++ )
	{
		...
		else
		{
			...
			if ( uDriveType == DRIVE_CDROM ) { ret_list << tmp; }
