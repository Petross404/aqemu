...
int AQEMU_Main::main( int argc, char *argv[] )
{
	...
	if ( args.at( "<AQEMU_FILE>" ).isString() )
		AQEMU_FILE =
			QString::fromStdString( args.at( "<AQEMU_FILE>" ).asString() );
...
int AQEMU_Main::main_window()
{
	...
	{
		std::cout << qPrintable(
			QObject::tr( "AQEMU is already running. Only one main "
				     "window can be used at one time." ) )
			  << std::endl;
		return 0;
	}
...
void AQEMU_Main::upgrade_settings()
{
	...
	if ( QFile::exists( settings->fileName() ) )
	{
		...
		if ( conf_ver == "0.5" )
		{
			AQDebug( "int main( int argc, char *argv[] )", "AQEMU Config Version: 0.5\nRun Firt Start Wizard" );
		...
		{
			AQDebug( "int main( int argc, char *argv[] )", "AQEMU Config Version: 0.7.X\nStart Emulators Search..." );

			QMessageBox::information(
				NULL,
				QObject::tr( "AQEMU emulators search" ),
				QObject::tr( "AQEMU will search for emulators "
					     "after uptating. Please wait." ),
				QMessageBox::Ok );

			First_Start_Wizard *first_start_win =
				new First_Start_Wizard( NULL );
			...
			if ( first_start_win->Find_Emulators() )
				...
				AQGraphic_Error(
					...
					QObject::tr( "Error!" ),
					QObject::tr( "Cannot Find any Emulators "
						     "installed in your OS! You "
						     "should choose them In "
						     "Advanced Settings!" ),
					false );
		...
		else
		{
			...
			{
				AQWarning( "int main( int argc, char *argv[] )", "AQEMU Configuration File No Version 0.5. File Saved: AQEMU.conf.bak" );

				settings->clear();
				settings->sync();

				settings->setValue( "AQEMU_Config_Version",
						    CURRENT_AQEMU_VERSION );
			}
			else
				AQError( "int main( int argc, char *argv[] )", "Cannot Save Old Version AQEMU Configuration File!" );
		}
	}
	else
	{
	...
	if ( settings->value( "AQEMU_Data_Folder", "" ).toString().isEmpty() )
	{
		...
		else
		{
			AQGraphic_Error( "int main( int argc, char *argv[] )",
					 QObject::tr( "Error!" ),
					 QObject::tr( "Cannot Find AQEMU Data!" ),
					 false );
		}
#else
		...
		if ( settings->value( "AQEMU_Data_Folder", "" ).toString().isEmpty() )
		{
			QMessageBox::information(
				...
				QObject::tr( "Cannot Locate AQEMU Data Folder!\n"
					     "You Should Select This Folder in "
					     "Next Window!" ),
				QMessageBox::Ok );
			...
			{
				QMessageBox::critical( NULL, QObject::tr( "Error!" ), QObject::tr( "AQEMU won't Work If Data Folder isn't Selected!" ) );
				return -1;
			}
			else
			{
	...
	if ( settings->value( "First_Start", "yes" ).toString() == "yes" )
	{
		...
		if ( user_uid == 0 )
		{
			int ret = QMessageBox::question(
				NULL,
				QObject::tr( "Warning!" ),
				QObject::tr( "This is the first start of AQEMU "
					     "with root privileges.\n"
					     "On some Linux distributions you may "
					     "run into problems.\n"
					     "Close AQEMU?" ),
...
void AQEMU_Main::vm_dir_exists_or_create()
{
	...
	if ( !vm_dir.exists( settings->value( "VM_Directory", "" ).toString() ) )
	{
		...
		{
#ifdef Q_OS_WIN32
			vm_dir.mkpath( QDir::toNativeSeparators( QDir::homePath()
								 + "/AQEMU_VM/" ) );
#else
