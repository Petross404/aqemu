...
void CustomLineEdit::keyPressEvent( QKeyEvent *event )
{
	...
	if ( ( freshCurPos == 3 ) && ( key != Qt::Key_Right ) ) emit jumpForward();
}

void CustomLineEdit::mouseReleaseEvent( QMouseEvent *event )
{
	if ( !selectOnMouseRelease ) return;
...
void makeCommonStyle( QWidget *line )
{
	...
	if ( lineedit ) lineedit->setAlignment( Qt::AlignCenter );
	...
	: QFrame( parent )
{
	...
	for ( linesIterator = lines.begin(); linesIterator != lines.end(); ++linesIterator )
	{
		...
		{
			connect( *linesIterator,
				 &CustomLineEdit::jumpForward,
				 *( linesIterator + 1 ),
