	...
	: QDialog( parent )
{
	...
	{
		AQWarning( "Convert_HDD_Image_Window::Convert_HDD_Image_Window( "
			   "QWidget *parent )",
	...
	if ( ok )
	{
		ui.Label_HDD_Info_L->setText(
			tr( "Image Format: %1\nAllocated Disk Space: %2 %3" )
				...
				.arg( Get_TR_Size_Suffix(
					HDD_Info->Get_Disk_Info().Disk_Size ) ) );
...
void Convert_HDD_Image_Window::on_CB_Output_Format_currentIndexChanged( const QString &text )
{
	...
	else { ui.CH_Compressed->setEnabled( false ); }
}
...
void Convert_HDD_Image_Window::on_Button_Convert_clicked()
{
	...
	connect( &Conv_Thread,
		 SIGNAL( Conversion_Complete( bool ) ),
		 this,
	...
	ProgDial =
		new QProgressDialog( tr( "Please Wait. Converting HDD Image..." ),
				     tr( "Cancel" ),
				     0,
				     0,
				     this );
...
bool Convert_HDD_Image_Window::Get_QEMU_IMG_Info()
{
	...
	else	// Parse text
	{
		/*
			...
			if( possibleEncrypte.exactMatch(allText) ) Possible_Encrypte
		   = true; else Possible_Encrypte = false; */
		...
		{
			AQError( "bool "
				 "Convert_HDD_Image_Window::Get_QEMU_IMG_Info()",
				 "Cannot match RegExp!" );
			return false;
		}
