...
void Main_Window::init_dbus()
{
	...
	QDBusConnection::sessionBus().unregisterObject( "/main_window",
							QDBusConnection::UnregisterTree );
	QDBusConnection::sessionBus().registerObject( "/main_window",
						      this,
...
void Main_Window::VM_State_Changed( const QString &vm, int state )
{
	...
	{
		if ( QFileInfo( vm )
		     == QFileInfo( VM_List.at( i )->Get_VM_XML_File_Path() ) )
		{
			VM_List.at( i )->Set_State( static_cast<VM::VM_State>( state ) );    // FIXME
			AQError( "void Main_Window::VM_State_Changed(const QString "
				 "&vm, int state)",
...
void Main_Window::closeEvent( QCloseEvent *event )
{
	if ( !Save_Settings() )
		AQGraphic_Error(
			"void Main_Window::closeEvent( QCloseEvent *event )",
			tr( "AQEMU" ),
			tr( "Could not save main window settings!" ),
			false );
...
Virtual_Machine *Main_Window::Get_VM_By_UID( const QString &uid )
{
	...
	// VM Not Found!
	AQWarning( "Virtual_Machine *Main_Window::Get_VM_By_UID( const QString "
		   "&uid, bool &ok )",
		   "UID Not Found!" );
	return NULL;
}
...
const QMap<QString, Available_Devices> Main_Window::Get_Devices_Info( bool *ok ) const
{
	...
	{
		AQError( "QList<Available_Devices> &Main_Window::Get_Devices_Info( "
			 "bool *ok )",
			 "Emulator empty!" );
		*ok = false;
		return retList;
...
Available_Devices Main_Window::Get_Current_Machine_Devices( bool *ok ) const
{
	...
	if ( !devOk )
	{
		AQError( "Available_Devices "
			 "Main_Window::Get_Current_Machine_Devices( bool *ok ) "
			 "const",
			 "Cannot get devices!" );
		*ok = false;
				     ...
				     bool	      show_user_errors )
{
	...
	{
		if ( show_user_errors )
			AQError( "bool Main_Window::Create_VM_From_Ui( "
				 "Virtual_Machine *tmp_vm, Virtual_Machine *old_vm "
				 ")",
				 "old_vm == NULL" );
	...
	switch ( ui->CB_InitGM_Depth->currentIndex() )
	{
		...
		case 1: tmp_mode.Set_Depth( 16 ); break;

		case 2: tmp_mode.Set_Depth( 24 ); break;

		case 3: tmp_mode.Set_Depth( 32 ); break;

		default:
			if ( show_user_errors )
				AQError( "bool Main_Window::Create_VM_From_Ui( "
					 "Virtual_Machine *tmp_vm, QListWidgetItem "
					 "*item )",
					 "Initial Graphical Mode: Default "
					 "Section!" );
...
bool Main_Window::Load_Settings()
{
	...
	VM_Folder = QDir::toNativeSeparators(
		Settings.value( "VM_Directory", "~" ).toString() );
...
{
	QDir	      vm_dir( QDir::toNativeSeparators(
		 Settings.value( "VM_Directory", "~" ).toString() ) );
	QFileInfoList fil =
	...
	for ( int ix = 0; ix < fil.count(); ix++ )
	{
		...
		if ( !new_vm->Load_VM( fil[ix].filePath() ) ) { --real_index; }
		else
		{
			...
			QObject::connect(
				new_vm,
				SIGNAL( State_Changed( Virtual_Machine *, VM::VM_State ) ),
				this,
			...
			if ( new_vm->Get_State() == VM::VMS_Saved
			     && Settings.value( "Use_Screenshot_for_OS_Logo", "yes" )
						.toString()
					== "yes" )
			{
				...
				if ( QFile::exists( new_vm->Get_Screenshot_Path() ) )
					item->setIcon( QIcon(
						new_vm->Get_Screenshot_Path() ) );
				else
					item->setIcon( QIcon( new_vm->Get_Icon_Path() ) );
			}
			else { item->setIcon( QIcon( new_vm->Get_Icon_Path() ) ); }
...
void Main_Window::Update_VM_Ui( bool update_info_tab )
{
	...
	if ( ui->Machines_List->currentRow() < 0 )
	{
		...
		/*
		AQGraphic_Error( "void Main_Window::Update_VM_Ui()", tr("Critical
		Error!"), tr("VM Index Out of Range! Close AQEMU?"), true );*/
		return;
	}
	...
	SMP_Settings->Set_Values( tmp_vm->Get_SMP(),
				  ...
				  curComp.PSO_SMP_Threads,
				  curComp.PSO_SMP_Sockets,
...
QString Main_Window::Get_Storage_Device_Info_String( const QString &path )
{
	...
	{
		AQWarning( "QString Main_Window::Get_Storage_Device_Info_String( "
			   "const QString &path )",
...
void Main_Window::Change_The_Icon( Virtual_Machine *vm, QString _icon )
{
	...
	if ( vm_item == nullptr )
	{
		AQDebug( "void Main_Window::Change_The_Icon(Virtual_Machine* vm, "
			 "QString _icon)",
...
void Main_Window::Show_State_Current( Virtual_Machine *vm )
{
	if ( vm == NULL )
	{
		AQError( "void Main_Window::Show_State( Virtual_Machine *vm, "
			 "VM::VM_State s )",
			 "vm == NULL" );
		return;
	}
...
void Main_Window::Show_State_VM( Virtual_Machine *vm )
{
	...
	switch ( vm->Get_State() )
	{
		case VM::VMS_Running: Change_The_Icon( vm, ":/play.png" ); break;

		case VM::VMS_Power_Off: Change_The_Icon( vm, ":/stop.png" ); break;

		case VM::VMS_Pause: Change_The_Icon( vm, ":/pause.png" ); break;

		case VM::VMS_Saved: Change_The_Icon( vm, ":/save.png" ); break;

		case VM::VMS_In_Error: Change_The_Icon( vm, ":/error.png" ); break;
...
{
	if ( block_VM_changed_signals ) return;
						       ...
						       QListWidgetItem *previous )
{
					...
					< ui->Machines_List->count() )
			{
				if ( old_vm->Get_State() == VM::VMS_Saved )
				{
					previous->setIcon( QIcon(
						old_vm->Get_Screenshot_Path() ) );
					previous->setData( 128, old_vm->Get_Screenshot_Path() );
				}
				else
				{
					previous->setIcon(
						QIcon( old_vm->Get_Icon_Path() ) );
	     ...
	     && ui->Button_Apply->isEnabled() )
	{
		int mes_res = QMessageBox::question(
			this,
			tr( "Warning!" ),
			tr( "Current VM was changed. Save all changes?" ),
			QMessageBox::Yes | QMessageBox::No,
			QMessageBox::Yes );
	...
	else
	{
		...
		{
			AQError( "void "
				 "Main_Window::on_Machines_List_currentItemChanged("
...
bool Main_Window::Boot_Is_Correct( Virtual_Machine *tmp_vm )
{
	...
	if ( tmp_vm->Get_FD0().Get_Enabled() )
	{
		...
		{
			if ( !No_Device_Found( "Floppy A",
					       tmp_vm->Get_FD0().Get_File_Name(),
					       VM::Boot_From_FDA ) )
			{
				return false;
			}
	...
	if ( tmp_vm->Get_FD1().Get_Enabled() )
	{
		...
		{
			if ( !No_Device_Found( "Floppy B",
					       tmp_vm->Get_FD1().Get_File_Name(),
					       VM::Boot_From_FDA ) )
			{
				return false;
			}
	...
	if ( tmp_vm->Get_CD_ROM().Get_Enabled() )
	{
		...
		{
			if ( !No_Device_Found( "CD-ROM",
					       tmp_vm->Get_CD_ROM().Get_File_Name(),
					       VM::Boot_From_CDROM ) )
			{
				return false;
			}
	...
	if ( tmp_vm->Get_HDA().Get_Enabled() )
	{
		...
		{
			if ( !No_Device_Found( "HDA",
					       tmp_vm->Get_HDA().Get_File_Name(),
					       VM::Boot_From_HDD ) )
			{
				return false;
			}
	...
	if ( tmp_vm->Get_HDB().Get_Enabled() )
	{
		...
		{
			if ( !No_Device_Found( "HDB",
					       tmp_vm->Get_HDB().Get_File_Name(),
					       VM::Boot_From_HDD ) )
			{
				return false;
			}
	...
	if ( tmp_vm->Get_HDC().Get_Enabled() )
	{
		...
		{
			if ( !No_Device_Found( "HDC",
					       tmp_vm->Get_HDC().Get_File_Name(),
					       VM::Boot_From_HDD ) )
			{
				return false;
			}
	...
	if ( tmp_vm->Get_HDD().Get_Enabled() )
	{
		...
		{
			if ( !No_Device_Found( "HDD",
					       tmp_vm->Get_HDD().Get_File_Name(),
					       VM::Boot_From_HDD ) )
			{
				return false;
			}
	...
	if ( tmp_vm->Get_Use_Linux_Boot() )
	{
		...
		{
			if ( !No_Device_Found( tr( "bzImage" ),
					       tmp_vm->Get_bzImage_Path(),
					       VM::Boot_None ) )
			{
				return false;
			}
		...
		{
			if ( !No_Device_Found( tr( "Initrd" ),
					       tmp_vm->Get_Initrd_Path(),
					       VM::Boot_None ) )
			{
				return false;
			}
	...
	if ( tmp_vm->Get_Use_ROM_File() )
	{
		...
		{
			if ( !No_Device_Found( tr( "ROM File" ),
					       tmp_vm->Get_ROM_File(),
					       VM::Boot_None ) )
			{
				return false;
	...
	if ( tmp_vm->Use_MTDBlock_File() )
	{
		...
		{
			if ( !No_Device_Found( tr( "On-Board Flash" ),
					       tmp_vm->Get_MTDBlock_File(),
					       VM::Boot_None ) )
			{
				return false;
			}
	...
	if ( tmp_vm->Use_PFlash_File() )
	{
		...
		{
			if ( !No_Device_Found( tr( "Parallel Flash" ),
					       tmp_vm->Get_PFlash_File(),
					       VM::Boot_None ) )
			{
				return false;
			}
...
bool Main_Window::No_Device_Found( const QString &name, const QString &path, VM::Boot_Device type )
{
	...
	else
	{
		     ...
		     && ui->Machines_List->currentRow() < VM_List.count() )
		{
			...
			if ( cur_vm == NULL )
			{
				AQError( "bool Main_Window::No_Device_Found( const "
...
void Main_Window::on_actionChange_Icon_triggered()
{
	...
	if ( QDialog::Accepted == icon_win.exec() )
	{
		...
		{
			AQError( "void "
				 "Main_Window::on_actionChange_Icon_triggered()",
				 "cur_vm == NULL" );
			return;
		}
...
void Main_Window::on_actionAbout_AQEMU_triggered() { About_Window( this ).exec(); }
...
void Main_Window::on_actionDelete_VM_triggered()
{
	...
	if ( mes_ret == QMessageBox::Yes )
	{
		...
		{
			AQGraphic_Error( "void "
					 "Main_Window::on_actionDelete_Virtual_"
					 "Machine_triggered()",
					 tr( "Error!" ),
					 tr( "Cannot Delete VM XML File!" ),
					 false );
		}
	}
...
void Main_Window::on_actionDelete_VM_And_Files_triggered()
{
	...
	{
		AQError( "void "
			 "Main_Window::on_actionDelete_VM_And_Files_triggered()",
			 "cur_vm == NULL" );
		return;
	}
...
void Main_Window::on_actionShow_Advanced_Settings_Window_triggered()
{
	...
	if ( ad_set.exec() == QDialog::Accepted )
	{
		...
		if ( tmpEmulatorsList != All_Emulators_List )
		{
			...
			{
				QString type =
					( VM_List[ix]->Get_Machine_Accelerator() == VM::TCG
						  ? "TCG"
						  : "KVM" );
				...
				else
				{
					if ( VM_List[ix]->Get_State() == VM::VMS_In_Error )
					{
						     ...
						     || ( type == "KVM" && k == true ) )
						{
							...
							VM_List[ix]->Set_State(
								VM::VMS_Power_Off );
							VM_List[ix]->Save_VM();
							VM_List[ix]->Load_VM(
								VM_List[ix]->Get_VM_XML_File_Path() );
						}
					}
				}
			}
		...
		for ( int ix = 0; ix < VM_List.count(); ++ix )
		{
			...
			{
				AQError( "void "
					 "Main_Window::on_actionShow_Settings_"
					 "Window_triggered()",
					 "tmp_vm == NULL" );
				continue;
			}
...
bool Main_Window::Save_Or_Discard( bool forced )
{
	...
	else
	{
		...
		if ( tmp_vm != *cur_vm )
		{
			...
			{
				disconnect( cur_vm,
					    SIGNAL( State_Changed( Virtual_Machine *,
								   VM::VM_State ) ),
					    this,
					    SLOT( VM_State_Changed( Virtual_Machine *,
								    VM::VM_State ) ) );
...
void Main_Window::on_actionSave_triggered()
{
	...
	if ( Settings.value( "Info/Show_Screenshot_in_Save_Mode", "no" ).toString() == "yes" )
	{
		...
		QString img_path = QDir::toNativeSeparators(
			Settings.value( "VM_Directory", "~" ).toString()
			+ Get_FS_Compatible_VM_Name( cur_vm->Get_Machine_Name() )
			+ "_screenshot" );
			...
			if ( copy_win.Get_Copy_Floppy() )
			{
				if ( new_vm->Get_FD0().Get_Enabled() )
				{
					new_vm->Set_FD0( VM_Storage_Device(
						true,
						Copy_VM_Floppy( new_vm->Get_Machine_Name(),
								"FD0",
								new_vm->Get_FD0() ) ) );
				}
...
void Main_Window::on_actionShow_Emulator_Control_triggered()
{
	...
	{
		AQError( "void "
			 "Main_Window::on_actionShow_Emulator_Control_triggered()",
			 "cur_vm == NULL" );
		return;
	}
...
void Main_Window::on_actionCreate_Shell_Script_triggered()
{
	...
	{
		AQError( "void "
			 "Main_Window::on_actionCreate_Shell_Script_triggered()",
			 "cur_vm == NULL" );
		return;
	}
	...
	if ( !fileName.isEmpty() )
	{
		...
		{
			AQGraphic_Error( "void "
					 "Main_Window::on_actionCreate_Shell_"
					 "Script_triggered()",
					 tr( "Error!" ),
					 tr( "Cannot Open File!" ) );
			return;
		...
		scriptFile.setPermissions( scriptFile.permissions()
...
void Main_Window::on_actionShow_QEMU_Error_Log_Window_triggered()
{
	...
	{
		AQError( "void "
			 "Main_Window::on_actionShow_QEMU_Error_Log_Window_"
			 "triggered()",
			 "cur_vm == NULL" );
		return;
	}
...
void Main_Window::on_CB_RAM_Size_editTextChanged( const QString &text )
{
	...
	else
	{
		AQGraphic_Warning( tr( "Error" ),
				   tr( "Cannot convert \"%1\" to size suffix! "
				       "Valid suffixes are: MB, GB" )
					   .arg( ramStrings[2] ) );
		return;
	}
...
void Main_Window::Update_Computer_Types()
{
	...
	for ( int i = 0; i < model->rowCount(); i++ )
	{
		...
		item->setFlags(
			only_native ? item->flags() & ~( Qt::ItemIsSelectable | Qt::ItemIsEnabled )
...
void Main_Window::Apply_Emulator( int mode )
{
	...
	switch ( mode )
	{
		...
		case 1: Update_Accelerator_Options();
		case 2:
			// Computer Type
			Update_Computer_Types();
		case 3: Computer_Type_Changed(); break;
...
void Main_Window::CB_Boot_Priority_currentIndexChanged( int index )
{
	...
	switch ( ui->CB_Boot_Priority->currentIndex() )
	{
		...
		case 1: bootDev = VM::Boot_From_HDD; break;

		case 2: bootDev = VM::Boot_From_CDROM; break;

		case 3: bootDev = VM::Boot_From_Network1; break;

		case 4: bootDev = VM::Boot_None; break;

		default:
			AQWarning( "void "
				   "Main_Window::on_CB_Boot_Priority_"
...
bool Main_Window::Validate_CPU_Count( const QString &text )
{
	...
	{
		AQGraphic_Warning( tr( "Error!" ),
				   tr( "CPU count value is not valid digit!" ) );
		return false;
	}
	...
	if ( !cpuOk )
	{
		AQError( "bool Main_Window::Validate_CPU_Count( const QString "
			 "&text )",
			 "Cannot get devices!" );
		return false;
	}
...
// Port < 1024
	     ...
	     && Settings.value( "Ignore_Redirection_Port_Varning", "no" ).toString() == "no" )
	{
		int ret = QMessageBox::question(
			this,
			tr( "Warning!" ),
			tr( "To Create Socket With Port Number < 1024, in Unix You "
			    "Need to Run AQEMU in root Mode!\n"
			    "Press \"Ignore\" button for hide this message in "
			    "future.\nAdd This Record?" ),
					 ...
					 const VM_HDD  &hd )
{
	...
	else
	{
		...
		{
			AQError( "QString Main_Window::Copy_VM_Hard_Drive( const "
				 "QString &vm_name, const QString &hd_name, const "
				 "VM_HDD &hd )",
				 "Copy Error!" );
		}
				     ...
				     const VM_Storage_Device &fd )
{
	if ( vm_name.isEmpty() )
	{
		AQError( "QString Main_Window::Copy_VM_Floppy( const QString "
			 "&vm_name, const QString &fd_name, const "
	...
	else
	{
		...
		{
			AQError( "QString Main_Window::Copy_VM_Floppy( const "
				 "QString &vm_name, const QString &fd_name, const "
				 "VM_Storage_Device &fd )",
				 "Copy Error!" );
		}
