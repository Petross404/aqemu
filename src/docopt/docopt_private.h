...
public:
	...
	{
		throw std::runtime_error( "Logic error: name() shouldnt be called "
					  "on a BranchPattern" );
	}
	...
	while ( !groups.empty() )
	{
		...
		{    // Required, Optional, OptionsShortcut
			BranchPattern* branch =
				dynamic_cast<BranchPattern*>( child.get() );
...
inline void BranchPattern::fix_repeating_arguments()
{
	...
	for ( auto const& group : either )
	{
		...
		for ( auto const& e : group_set )
		{
			...
			if ( dynamic_cast<Command*>( leaf ) ) { ensureInt = true; }
			...
			else if ( ensureInt ) { leaf->setValue( value{ 0 } ); }
		}
	}
}

inline bool LeafPattern::match( PatternList& left,
				std::vector<std::shared_ptr<LeafPattern>>& collected ) const
{
	...
	auto same_name = std::find_if( collected.begin(),
				       collected.end(),
	...
	if ( getValue().isLong() )
	{
		...
		else { ( **same_name ).setValue( value{ val } ); }
	}
	...
	else { collected.push_back( match.second ); }
	return true;
}
	...
	PatternList const& left ) const
{
	...
	for ( size_t i = 0, size = left.size(); i < size; ++i )
	{
		...
		if ( arg )
		{
			ret.first = i;
			ret.second =
				std::make_shared<Argument>( name(), arg->getValue() );
			break;
		}
	}
	...
	PatternList const& left ) const
{
	...
	for ( size_t i = 0, size = left.size(); i < size; ++i )
	{
		...
		if ( arg )
		{
			...
			{
				ret.first  = i;
				ret.second = std::make_shared<Command>( name(),
									value{ true } );
			}
			break;
		}
...
inline Option Option::parse( std::string const& option_description )
{
	...
	{
		options_end = option_description.begin()
			      + static_cast<std::ptrdiff_t>( double_space );
	}

	static const std::regex pattern{ "(-{1,2})?(.*?)([,= ]|$)" };
	for ( std::sregex_iterator i{ option_description.begin(),
				      options_end,
				      pattern,
...
inline bool Required::match( PatternList& left,
			     std::vector<std::shared_ptr<LeafPattern>>& collected ) const
{
	auto l = left;
	auto c = collected;
...
inline bool OneOrMore::match( PatternList& left,
			      std::vector<std::shared_ptr<LeafPattern>>& collected ) const
{
			   ...
			   std::vector<std::shared_ptr<LeafPattern>>& collected ) const
{
	...
	auto min = std::min_element( outcomes.begin(),
				     outcomes.end(),
