...
static PatternList parse_long( Tokens& tokens, std::vector<Option>& options )
{
	...
	else
	{
		...
		if ( o->argCount() == 0 )
		{
			...
			{
				std::string error =
					o->longOption() + " must not have an argument";
				throw Tokens::OptionError( std::move( error ) );
			}
		}
		else
		{
			if ( !val )
			{
				...
				{
					std::string error =
						o->longOption() + " requires an argument";
					throw Tokens::OptionError( std::move( error ) );
				}
				val = tokens.pop();
			}
		...
		else
		{
			...
			if ( o->argCount() )
			{
				if ( i == token.end() )
				{
					...
					{
						std::string error =
							shortOpt + " requires an argument";
						throw Tokens::OptionError(
							std::move( error ) );
					}
					val = tokens.pop();
				}
...
static PatternList parse_atom( Tokens& tokens, std::vector<Option>& options )
{
	...
	else { ret.emplace_back( std::make_shared<Command>( tokens.pop() ) ); }
...
static PatternList parse_argv( Tokens tokens, std::vector<Option>& options, bool options_first )
{
	...
	while ( tokens )
	{
		...
		if ( token == "--" )
		{
			...
			{
				ret.emplace_back(
					std::make_shared<Argument>( "", tokens.pop() ) );
			}
		}
...
std::vector<Option> parse_defaults( std::string const& doc )
{
	...
	{
		s.erase( s.begin(),
			 s.begin() + static_cast<std::ptrdiff_t>( s.find( ':' ) )
...
static bool isOptionSet( PatternList const& options,
...
DOCOPT_INLINE
std::map<std::string, value> docopt::docopt_parse( std::string const& doc,
...
DOCOPT_INLINE
std::map<std::string, value> docopt::docopt( std::string const&		     doc,
					     std::vector<std::string> const& argv,
					     bool			     help,
