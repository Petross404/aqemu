	...
	: kind_( other.kind_ )
{
	switch ( kind_ )
	{
		...
		case Kind::StringList:
			new ( &variant_.strList )
				std::vector<std::string>( other.variant_.strList );
			break;
	...
	: kind_( other.kind_ )
{
	...
	{
		case Kind::String:
			new ( &variant_.strValue )
				std::string( std::move( other.variant_.strValue ) );
			break;

		case Kind::StringList:
			new ( &variant_.strList ) std::vector<std::string>(
				std::move( other.variant_.strList ) );
			break;
...
inline value::~value()
{
	switch ( kind_ )
	{
		case Kind::String: variant_.strValue.~basic_string(); break;

		case Kind::StringList: variant_.strList.~vector(); break;
...
inline size_t value::hash() const noexcept
{
	switch ( kind_ )
	{
		...
		case Kind::Bool: return std::hash<bool>()( variant_.boolValue );

		case Kind::Long: return std::hash<long>()( variant_.longValue );

		case Kind::Empty:
		default: return std::hash<void*>()( nullptr );
	}
}
