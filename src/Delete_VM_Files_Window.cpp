...
void Delete_VM_Files_Window::init()
{
	...
	connect( ui.delete_VM_and_files_button,
		 SIGNAL( clicked() ),
		 this,
...
bool Delete_VM_Files_Window::Do_Delete()
{
	...
	if ( ui.Files_List->isVisible() )
	{
		...
		for ( int ix = 0; ix < ui.Files_List->rowCount(); ix++ )
		{
			...
			{
				AQError( "void "
					 "Delete_VM_Files_Window::on_Button_Delete_"
					 "clicked()",

					 "item_CheckBox == NULL || item_Text == "
					 "NULL" );
				continue;
			}
			...
			if ( item_CheckBox->checkState() == Qt::Checked )
			{
				...
				{
					AQError( "void "
						 "Delete_VM_Files_Window::on_"
						 "Button_Delete_clicked()",
						 "Cannot Delete File: \""
							 + item_Text->text() + "\"" );
	...
	if ( !no_Delete_Files_List.isEmpty() )
	{
		QMessageBox::information(
			this,
			tr( "An error occurred while deleting files" ),
