	...
	: QDialog( parent )
{
	...
	for ( int ix = 0; ix < cdrom_count; ix++ )
	{
		ui.CDROM_List->addItem(
			Settings.value( "Additional_CDROM_Devices/Device"
						+ QString::number( ix ),
					"" )
				.toString() );
	}
	...
	ui.RB_MAC_QEMU->setChecked( Settings.value( "MAC_Generation_Mode", "Model" ).toString()
				    == "QEMU_Segment" );
...
// QEMU Monitor Type
	...
	{
		case 16: ui.CB_VM_Icons_Size->setCurrentIndex( 0 ); break;

		case 24: ui.CB_VM_Icons_Size->setCurrentIndex( 1 ); break;

		case 32: ui.CB_VM_Icons_Size->setCurrentIndex( 2 ); break;

		case 48: ui.CB_VM_Icons_Size->setCurrentIndex( 3 ); break;

		case 64: ui.CB_VM_Icons_Size->setCurrentIndex( 4 ); break;

		default: ui.CB_VM_Icons_Size->setCurrentIndex( 3 ); break;
	}
	...
	connect( ui.RB_Emul_Show_VNC,
		 SIGNAL( toggled( bool ) ),
		 this,
		 SLOT( VNC_Warning( bool ) ) );

	connect( ui.RB_Emul_Show_VNC_In_Main_Window,
		 SIGNAL( toggled( bool ) ),
		 this,
		 SLOT( VNC_Warning( bool ) ) );
...
Advanced_Settings_Window::~Advanced_Settings_Window() { delete settings_widget; }
...
void Advanced_Settings_Window::VNC_Warning( bool state )
{
	...
	if ( Settings.value( "Show_VNC_Warning", "yes" ).toString() == "yes" )
	{
		if ( QMessageBox::information(
			     this,
			     tr( "Information" ),
			     tr( "Support for this feature is not complete! If "
				 "there is no picture, click \"View->Reinit "
...
void Advanced_Settings_Window::done( int r )
{
	if ( r == QDialog::Accepted )
	{
		...
		{
			Settings.setValue( "Additional_CDROM_Devices/Device"
						   + QString::number( ix ),
...
// QEMU Monitor Type
		...
		if ( Settings.status() != QSettings::NoError )
			AQError( "void Advanced_Settings_Window::done(int)",
				 "QSettings Error!" );
		...
		else
		{
			int mes_res = QMessageBox::question(
				this,
				tr( "Invalid Value!" ),
				tr( "AQEMU VM folder doesn't exist! Do you want to "
				    "create it?" ),
				QMessageBox::Yes | QMessageBox::No,
				QMessageBox::Yes );
			...
			if ( mes_res == QMessageBox::Yes )
			{
						      ...
						      + QDir::toNativeSeparators( "/os_templates/" ) ) ) )
				{
					AQGraphic_Warning(
						tr( "Error!" ),
						tr( "Cannot Create New Folder!" ) );
					return;
				}
				else
					Settings.setValue( "VM_Directory",
							   ui.Edit_VM_Folder->text() );
			}
			else
				return;
		}
		...
		if ( ui.CB_Language->currentIndex() == 0 )
			...
			Settings.setValue( "Language",
					   ui.CB_Language->itemText(
						   ui.CB_Language->currentIndex() ) );

		// VM Icons Size
		switch ( ui.CB_VM_Icons_Size->currentIndex() )
		{
			case 0: Settings.setValue( "VM_Icons_Size", 16 ); break;

			case 1: Settings.setValue( "VM_Icons_Size", 24 ); break;

			case 2: Settings.setValue( "VM_Icons_Size", 32 ); break;

			case 3: Settings.setValue( "VM_Icons_Size", 48 ); break;

			case 4: Settings.setValue( "VM_Icons_Size", 64 ); break;

			default: Settings.setValue( "VM_Icons_Size", 48 ); break;
		}
		/*
		// USB Support
		...
		*/
		...
		/*
		// 64x64 Icons
		if( ui.CH_64_Icons->isChecked() ) Settings.setValue( "64x64_Icons",
		"yes" ); else Settings.setValue( "64x64_Icons", "no" );*/

		Settings.sync();

		if ( Settings.status() != QSettings::NoError )
		{
			AQError( "void Settings_Window::on_Button_Box_clicked( "
...
void Advanced_Settings_Window::on_TB_Find_All_Emulators_clicked()
{
	...
	{
		AQDebug( "void "
			 "Advanced_Settings_Window::on_TB_Find_All_Emulators_"
			 "clicked()",
	...
	else
	{
		AQGraphic_Error( "void "
				 "Advanced_Settings_Window::on_TB_Find_All_"
				 "Emulators_clicked()",
				 tr( "Error!" ),
				 tr( "Cannot find any emulators installed on your "
				     "OS! Please add them manually!" ),
				 false );
	}
...
void Advanced_Settings_Window::on_Button_CDROM_Add_clicked()
{
	...
	QString text = QInputDialog::getText(
		this,
		tr( "Add CD/DVD Device" ),
		tr( "Enter Device Name. Example: /dev/cdrom" ),
		QLineEdit::Normal,
		"",
		&ok );
...
void Advanced_Settings_Window::on_Button_CDROM_Edit_clicked()
{
	if ( ui.CDROM_List->currentRow() >= 0 )
	{
		...
		QString text = QInputDialog::getText(
			this,
			tr( "Add CD/DVD Device" ),
			tr( "Enter Device Name. Sample: /dev/cdrom" ),
			QLineEdit::Normal,
			ui.CDROM_List->currentItem()->text(),
			&ok );
...
bool Advanced_Settings_Window::Save_Emulators_Info()
{
	...
	for ( int ix = 0; ix < Emulators.count(); ++ix )
	{
		...
		{
			AQGraphic_Warning( tr( "Error!" ),
					   tr( "Cannot save emulator \"%1\"!" )
						   .arg( Emulators[ix].Get_Name() ) );
		}
	}
...
void Advanced_Settings_Window::Update_Emulators_Info()
{
	...
	for ( int ix = 0; ix < Emulators.count(); ++ix )
	{
		...
		QTableWidgetItem *newItem =
			new QTableWidgetItem( Emulators[ix].Get_Name() );
		...
		newItem = new QTableWidgetItem(
			Emulators[ix].Get_Default() ? tr( "Yes" ) : tr( "No" ) );
