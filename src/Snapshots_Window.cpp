	...
	if ( prop_win.exec() == QDialog::Accepted )
	{
		...
		if ( next == false )
		{
			for ( int ix = 0; ix < 1024; ++ix )
			{
				...
				for ( int sx = 0;
				      sx < Current_VM->Get_Snapshots().count();
				      ++sx )
				{
					if ( Current_VM->Get_Snapshots()[sx].Get_Tag() == tag )
					{
...
void Snapshots_Window::on_Button_Start_clicked()
{
	     ...
	     || Current_VM->Get_State() == VM::VMS_Saved )
	{
		...
		else
		{
			...
			{
				if ( Settings.value( "Show_Emulator_Control_Window", "yes" )
					     .toString()
				     == "yes" )
				{
			...
			{
				AQError( "void "
					 "Snapshots_Window::on_Button_Start_"
					 "clicked()",
					 "Cannot Start VM!" );
			}
		}
		...
		for ( int ix = 0; ix < Current_VM->Get_Snapshots().count(); ++ix )
		{
			...
			if ( tmp_str == tag )
			{
				...
				if ( prop_win->exec() == QDialog::Accepted )
				{
					...
					tmp.Set_Description(
						prop_win->Get_Snapshot_Description() );
...
bool Snapshots_Window::Update_Info()
{
		...
		for ( int ix = 0; ix < Current_VM->Get_Storage_Devices_List().count(); ++ix )
		{
			if ( Current_VM->Get_Storage_Devices_List()[ix].Use_Media() == false
			     || Current_VM->Get_Storage_Devices_List()[ix].Get_Media()
					== VM::DM_Disk )
			{
				if ( QFile::exists( Current_VM
							    ->Get_Storage_Devices_List()[ix]
							    .Get_File_Path() ) )
					imagePath =
						Current_VM
							->Get_Storage_Devices_List()[ix]
	...
	{
		AQGraphic_Warning( tr( "Error!" ), tr( "You must add HDD image in QCOW2 format if you want to use snapshots!" ) );
		return false;
	}
	...
	QRegExp RegInfo = QRegExp( ".*Snapshot "
				   "list:.*ID[\\s]+TAG[\\s]+VM[\\s]+SIZE[\\s]+DATE["
				   "\\s]+VM[\\s]+CLOCK(.+)" );
	...
	if ( info_lines.count() == 2 )
	{
		...
		QRegExp snap_regexp =
			QRegExp( "([\\d])+[\\s]+([a-zA-Z0-9_]+)[\\s]+([0-9KMG]+|[0-"
				 "9]+.[0-9KMG]+)[\\s]+(.+)[\\s]{2,}(.+)" );
	...
	for ( int fx = 0; fx < found_snapshots.count(); ++fx )
	{
		...
		for ( int cx = 0; next && cx < compare_list.count(); ++cx )
		{
			if ( found_snapshots[fx].Get_Tag() == compare_list[cx].Get_Tag() )
			{
				...
				found_snapshots[fx].Set_Description(
					compare_list[cx].Get_Description() );
