...
Old_Network_Widget::Old_Network_Widget( QWidget *parent )
	: QWidget( parent )
{
	ui.setupUi( this );
	...
	// IP Address
	QRegExp rx( "^([1-9]|[1-9][\\d]|[1][\\d][\\d]|2[0-4][\\d]|25[0-5])(\\.([0-"
		    "9]|[1-9][\\d]|[1][\\d][\\d]|2[0-4][\\d]|25[0-5])){3}$" );
...
bool Old_Network_Widget::Get_Network_Cards( QList<VM_Net_Card> &cards )
{
	...
	{
		AQDebug( "bool Old_Network_Widget::Get_Network_Cards( "
			 "QList<VM_Net_Card> &cards )",
			 "No Items" );
		cards.clear();
		return true;
	}
	...
	for ( int nx = 0; nx < Network_Cards.count(); nx++ )
	{
		if ( !Net_Card_is_Valid( nx ) )
		{
			AQGraphic_Warning( tr( "Error" ),
					   tr( "Network Card #% Not Valid!" ).arg( nx ) );
			return false;
		}
	}
...
void Old_Network_Widget::Set_Network_Cards( const QList<VM_Net_Card> &cards )
{
	...
	else { ui.Widget_All_Options->setEnabled( false ); }
}
...
{
	connect( ui.CB_Connection_Mode,
		 SIGNAL( currentIndexChanged( int ) ),
		 this,
		 SIGNAL( Changed() ) );

	connect( ui.CB_Network_Card_Model,
		 SIGNAL( currentIndexChanged( int ) ),
		 this,
		 SIGNAL( Changed() ) );
	...
	connect( ui.Edit_IP_Address,
		 SIGNAL( textChanged( const QString & ) ),
		 this,
		 SIGNAL( Changed() ) );

	connect( ui.Edit_MAC_Address,
		 SIGNAL( textChanged( const QString & ) ),
		 this,
		 SIGNAL( Changed() ) );

	connect( ui.SB_File_Descriptor, SIGNAL( valueChanged( int ) ), this, SIGNAL( Changed() ) );
	...
	connect( ui.Edit_TUN_TAP_Script,
		 SIGNAL( textChanged( const QString & ) ),
		 this,
		 SIGNAL( Changed() ) );

	connect( ui.Edit_Interface_Name,
		 SIGNAL( textChanged( const QString & ) ),
		 this,
		 SIGNAL( Changed() ) );
}
...
{
	disconnect( ui.CB_Connection_Mode,
		    SIGNAL( currentIndexChanged( int ) ),
		    this,
		    SIGNAL( Changed() ) );

	disconnect( ui.CB_Network_Card_Model,
		    SIGNAL( currentIndexChanged( int ) ),
		    this,
		    SIGNAL( Changed() ) );
	...
	disconnect( ui.Edit_IP_Address,
		    SIGNAL( textChanged( const QString & ) ),
		    this,
		    SIGNAL( Changed() ) );

	disconnect( ui.Edit_MAC_Address,
		    SIGNAL( textChanged( const QString & ) ),
		    this,
		    SIGNAL( Changed() ) );

	disconnect( ui.SB_File_Descriptor,
	...
	disconnect( ui.Edit_TUN_TAP_Script,
		    SIGNAL( textChanged( const QString & ) ),
		    this,
		    SIGNAL( Changed() ) );

	disconnect( ui.Edit_Interface_Name,
		    SIGNAL( textChanged( const QString & ) ),
		    this,
		    SIGNAL( Changed() ) );
}

void Old_Network_Widget::on_Network_Cards_List_currentItemChanged( QListWidgetItem *current,
								   QListWidgetItem *previous )
{
	...
	{
		AQError( "void "
			 "Old_Network_Widget::on_Network_Cards_List_"
...
void Old_Network_Widget::on_Button_Delete_Net_Card_clicked()
{
		  ...
		  && ui.Network_Cards_List->currentRow() >= 0 )
	{
		...
		{
			ui.Network_Cards_List->setCurrentRow(
				ui.Network_Cards_List->count() - 1 );
...
{
	if ( index < 0 || Network_Cards.count() <= 0 || Card_Models_QEMU_Name.count() <= 0
	     || ui.Network_Cards_List->currentRow() < 0
...
void Old_Network_Widget::Set_Net_Card_To_Ui( const VM_Net_Card &card )
{
	...
	switch ( card.Get_Net_Mode() )
	{
		...
		default:
			AQError( "void Main_Window::Set_Net_Card_To_Ui( const "
				 "VM_Net_Card &card )",
				 "Default Section!" );
			ui.CB_Connection_Mode->setCurrentIndex( 8 );
			break;
...
bool Old_Network_Widget::Net_Card_is_Valid( int index )
{
	...
	{
		QRegExp rx_ip( "^([1-9]|[1-9][\\d]|[1][\\d][\\d]|2[0-4][\\d]|25[0-"
			       "5])(\\.([0-9]|[1-9][\\d]|[1][\\d][\\d]|2[0-4][\\d]|"
			       "25[0-5])){3}$" );
		     ...
		     && QFile::exists( Network_Cards[index].Get_TUN_TAP_Script() ) == false )
		{
			int retVal = QMessageBox::critical(
				...
				tr( "In Card " ) + QString::number( index + 1 )
					+ tr( " TUN/TAP Script File \"%1\" Not "
					      "Exists!\nContinue Without It File?" )
