...
void AQEMU_Service::vm_state_changed( Virtual_Machine* vm, VM::VM_State s )
{
	...
	{
		QDBusInterface iface( "org.aqemu.main_window",
				      "/main_window",
				      "",
		...
		if ( iface.isValid() )
		{
			...
			qPrintable(reply.error().message())); return;*/
		}
		else
		{
			fprintf( stderr,
...
bool AQEMU_Service::call( const QString&   command,
			  Virtual_Machine* vm,
...
QString AQEMU_Service::start( const QString& s )
{
	...
	if ( QFileInfo( s ).exists() ) success = vm->Load_VM( s );
	...
	if ( !success )
	{
		...
		if ( QFileInfo( vm_file ).exists() )
			...
			return QString( "VM \"%1\" could not be started. No such "
					"VM found." )
				.arg( s );
	}
...
QString AQEMU_Service::list()
{
	...
	QStringList list;
	for ( int ix = 0; ix < file.count(); ix++ ) list << file[ix].filePath();
