	...
	: QDialog( parent )
{
	...
	connect( ui.CB_FD_Devices,
		 SIGNAL( editTextChanged( QString ) ),
		 this,
		 SLOT( on_Button_Update_Info_clicked() ) );

	connect( ui.CB_CDROM_Devices,
		 SIGNAL( editTextChanged( QString ) ),
		 this,
		 SLOT( on_Button_Update_Info_clicked() ) );

	connect( ui.Edit_HDD_Image_Path,
...
void Properties_Window::Set_Floppy( const VM_Storage_Device &fd, const QString &name )
{
	...
	if ( fd_list.count() < 1 )
	{
		AQDebug( "void Properties_Window::Set_Floppy( const "
			 "VM_Storage_Device &fd, const QString &name )",
...
void Properties_Window::Set_CD_ROM( const VM_Storage_Device &cd, const QString &name )
{
	...
	if ( cd_list.count() < 1 )
	{
		AQDebug( "void Properties_Window::Set_CD_ROM( const VM_CDROM &cd, "
			 "const QString &name )",
	...
	if ( r == QDialog::Accepted )
	{
		if ( ui.GB_Floppy->isEnabled() )
		{
			if ( !QFile::exists( ui.CB_FD_Devices->lineEdit()->text() ) )
			{
				AQGraphic_Warning(
					tr( "Warning" ),
					tr( "Image file doesn't exist!" ) );
				return;
			}
			else
			{
				Add_To_Recent_FDD_Files(
					ui.CB_FD_Devices->lineEdit()->text() );
				QDialog::done( r );
				return;
			}
		}
		else if ( ui.GB_CDROM->isEnabled() )
		{
			...
			{
				Add_To_Recent_CD_Files(
					ui.CB_CDROM_Devices->lineEdit()->text() );
				QDialog::done( r );
				return;
			}
		}
...
void Properties_Window::on_Button_Update_Info_clicked()
{
	...
	{
		VM_Storage_Device *cd = new VM_Storage_Device(
			true,
			ui.CB_CDROM_Devices->lineEdit()->text() );
...
void Properties_Window::on_TB_FD_Image_Browse_clicked()
{
	...
	if ( !fileName.isEmpty() )
	{
		...
		{
			if ( ui.CB_FD_Devices->itemText( ix ) == fileName ) return;
		}
...
void Properties_Window::Update_HDD( bool ok )
{
	...
	ui.Label_Info->setText(
		...
		+ Get_TR_Size_Suffix( PW_HDD.Get_Virtual_Size() )
		+ tr( "\nOn Disk Size: " ) + QString::number( PW_HDD.Get_Disk_Size().Size )
...
void Properties_Window::on_CB_FD_Devices_editTextChanged( const QString &text )
{
	...
	if ( text.isEmpty() ) { tmpDev.Use_File_Path( false ); }
	else
	{
...
void Properties_Window::on_TB_CDROM_Advanced_Settings_clicked()
{
	...
	{
		AQError( "void "
			 "Properties_Window::on_TB_CDROM_Advanced_Settings_clicked("
			 ")",
			 "Current_Machine_Devices == NULL" );
		return;
	}
...
void Properties_Window::on_TB_HDD_Advanced_Settings_clicked()
{
	...
	{
		AQError( "void "
			 "Properties_Window::on_TB_HDD_Advanced_Settings_clicked()",
			 "Current_Machine_Devices == NULL" );
		return;
	}
