...
void First_Start_Wizard::on_Button_Cancel_clicked()
{
	if ( QMessageBox::information(
		     this,
		     tr( "Warning!" ),
		     tr( "Are you sure? You can set these options at any time in "
			 "the configuration dialog. (File -> Configuration)" ),
...
void First_Start_Wizard::on_Button_Back_clicked()
{
	...
	else { ui.All_Pages->setCurrentIndex( ui.All_Pages->currentIndex() - 1 ); }
...
{
	if ( ui.All_Pages->currentWidget() == ui.Welcome_Page ) { retranslateUi(); }
	...
	else
	{
		...
		if ( paths.count() <= 0 )
		{
			AQError(
				...
				"Cannot Find PATH in System Environment "
				"Information!" );
			return;
		}
		else
		{
			...
			if ( qemuIMG_Found )
				AQDebug( "void "
					 "First_Start_Wizard::on_Button_Find_"
					 "Emulators_clicked()",
					 "qemu-img find on: "
						 + Settings.value( "QEMU-IMG_Path", "" )
							   .toString() );
			else
				AQError( "void "
					 "First_Start_Wizard::on_Button_Find_"
			...
			for ( int qx = 0; qx < paths.count(); ++qx )
			{
				...
				for ( QMap<QString, QString>::const_iterator it =
					      qemu_list.constBegin();
				      it != qemu_list.constEnd();
				      ++it )
				{
					if ( !it.value().isEmpty() )
					{
				...
				while ( iter != qemu_list.constEnd() )
				{
					if ( QFile::exists( iter.value() ) )
						qemu_version = System_Info::Get_Emulator_Version(
							iter.value() );
				...
				QProgressDialog progressWin( tr( "Search..." ),
							     tr( "Cancel" ),
							     0,
							     allEmulBinCount,
							     this );
				...
				// Emulator name
				QString emulName =
					Emulator_Version_To_String( qemu_version );
				...
				for ( int ix = 0; ix < qemuEmulatorsList.count(); ++ix )
				{
					if ( emulName == qemuEmulatorsList[ix].Get_Name() )
					{
						...
						emulName =
							QString( "%1 #%2" )
								.arg( Emulator_Version_To_String(
									qemu_version ) )
			...
			if ( qemuEmulatorsList.count() > 0 )
			{
				...
				if ( qemuEmulatorsList.count() > 1 )
				{
					...
					int		     maxVerIndex = 0;
					for ( int ix = 0; ix < qemuEmulatorsList.count();
					      ++ix )
					{
						if ( qemuEmulatorsList[ix].Get_Version()
						     > maxVer )
						{
							maxVer = qemuEmulatorsList[ix]
									 .Get_Version();
							maxVerIndex = ix;
						}
					}
...
void First_Start_Wizard::on_TB_Add_Emulator_Browse_clicked()
{
	QString emulatorDirPath = QFileDialog::getExistingDirectory(
		this,
		tr( "Select QEMU emulator directory" ),
...
void First_Start_Wizard::on_Button_Add_Emulator_Manual_Mode_clicked()
{
	...
	Emul.Set_Version( String_To_Emulator_Version(
		ui.CB_Add_Emulator_Version->currentText() ) );
...
// Virtual Machines Folder
	...
	ui.Edit_VM_Dir->setText( QDir::toNativeSeparators(
		Settings.value( "VM_Directory", QDir::homePath() + "/AQEMU_VM/" ).toString() ) );
#else
	ui.Edit_VM_Dir->setText( QDir::toNativeSeparators(
	...
	if ( Emul.Get_Name().isEmpty() )
	{
		...
		if ( !QFile::exists( Settings.value( "QEMU-IMG_Path", "" ).toString() ) )
		{
			...
			if ( QFile::exists( qemuImgPath ) )
				...
				AQGraphic_Warning( tr( "Warning!" ), tr( "Cannot find qemu-img.exe! You most set it path in: File->Advanced Settings->Advanced->qemu-img path" ) );
		}
	}
...
void First_Start_Wizard::retranslateUi()
{
	...
	ui.Label_Welcome_Text->setText(
		tr( "Welcome to the AQEMU settings wizard!\nThis wizard will help "
		    "you to choose options AQEMU needs to work correctly. Click on "
		    "\"Next\" to go to next page or the \"Back\" button to go to "
		    "the previous page." ) );
