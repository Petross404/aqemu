...
void Emulator_Control_Window::Apply_Full_Size( int w, int h )
{
	...
	else { setMaximumSize( 4096, 2048 ); }
}

void Emulator_Control_Window::QEMU_Quit()
{
#ifdef VNC_DISPLAY
	if ( Machine_View && Use_VNC() ) Machine_View->disconnectVNC();
#endif
}
...
void Emulator_Control_Window::VM_State_Changed( Virtual_Machine *vm, VM::VM_State state )
{
	switch ( state )
	{
		...
		default: this->setWindowTitle( vm->Get_Machine_Name() );
	}
}
...
{
	connect( Cur_VM,
		 SIGNAL( Ready_Removable_Devices_List() ),
		 this,
...
void Emulator_Control_Window::Create_Connect_Menu()
{
	...
	for ( int ix = 0; ix < devices.count() - 1; ++ix )
	{
		...
		if ( curDev[0].contains( "-cd" ) )    // CD-ROM
		{
			...
			{
				AQError( "void "
					 "Emulator_Control_Window::Create_Connect_"
					 "Menu()",
			...
			connect( tmpMenu,
				 SIGNAL( aboutToShow() ),
				 this,
				 SLOT( Create_Device_Menu() ) );
		}
		else if ( curDev[0].contains( "floppy0" ) )    // FDA
		{
			devName = "Floppy A";
			QMenu *tmpMenu =
				ui.menuConnectNew->addMenu( QIcon( ":/fdd.png" ), devName );
			connect( tmpMenu,
				 SIGNAL( aboutToShow() ),
				 this,
				 SLOT( Create_Device_Menu() ) );
		...
		else if ( curDev[0].contains( "sd" ) )	  // SD Card
		{
			...
			QMenu *tmpMenu =
				ui.menuConnectNew->addMenu( QIcon( ":/hdd.png" ), devName );
			connect( tmpMenu,
				 SIGNAL( aboutToShow() ),
				 this,
				 SLOT( Create_Device_Menu() ) );
		}
		else { devName = "Unknown device"; }
...
void Emulator_Control_Window::Create_Device_Menu()
{
	...
	if ( internalDevName.contains( "-cd" ) )
	{
		...
		QAction *browseAct = tmpMenu->addAction( QIcon( ":/open-file.png" ),
							 "Browse...",
							 this,
							 SLOT( Open_Device_File() ) );
		browseAct->setData( internalDevName );

		// Add 'Eject' menu item
		QAction *ejectAct = tmpMenu->addAction( QIcon( ":/eject.png" ),
							"Eject",
							this,
							SLOT( Eject_Device() ) );
	...
	else if ( internalDevName.contains( "floppy" ) )
	{
		...
		{
			QFileInfo tmpInfo( deviceSourcePath );
			QAction	 *conFddAct = tmpMenu->addAction(
				 tr( "Current \"" ) + tmpInfo.fileName() + "\"",
				 this,
				 SLOT( Connect_Device() ) );
	...
	else if ( internalDevName.contains( "sd" ) )
	{
		...
		{
			QFileInfo tmpInfo( deviceSourcePath );
			QAction	 *conSdAct = tmpMenu->addAction(
				 tr( "Current \"" ) + tmpInfo.fileName() + "\"",
				 this,
				 SLOT( Connect_Device() ) );
...
void Emulator_Control_Window::Eject_Device()
{
	...
	if ( act )
	{
		...
		emit Ready_Read_Command(
			QString( "eject %1" ).arg( act->data().toString() ) );
...
void Emulator_Control_Window::Set_Current_VM( Virtual_Machine *vm )
{
	...
	{
		QAction *new_act =
			new QAction( Cur_VM->Get_USB_Ports()[ix].Get_Product_Name(),
				     ui.menuDisconnect );
...
void Emulator_Control_Window::Init()
{
	...
	for ( int ix = 0; ix < max; ++ix )
	{
		...
		connect( Recent_Files_CD_Items[ix],
			 SIGNAL( triggered() ),
			 this,
	...
	for ( int ix = 0; ix < max; ++ix )
	{
		...
		connect( Recent_Files_FD0_Items[ix],
			 SIGNAL( triggered() ),
			 this,
	...
	for ( int ix = 0; ix < max; ++ix )
	{
		...
		connect( Recent_Files_FD1_Items[ix],
			 SIGNAL( triggered() ),
			 this,
...
// Use VNC Embedded Display
	...
	else
	{
		...
		if ( !Machine_View ) delete Machine_View;
		...
		connect( Machine_View,
			 SIGNAL( Full_Size( int, int ) ),
			 this,
...
void Emulator_Control_Window::on_actionSave_Screenshot_triggered()
{
	...
	{
		AQGraphic_Warning( "Warning!", "Shared Screenshots Folder Not Enabled!\nPlease Enter Name for New Screenshot." );
...
void Emulator_Control_Window::Show_Monitor() { on_actionQEMU_Monitor_triggered(); }

void Emulator_Control_Window::on_actionQEMU_Monitor_triggered()
{
	connect( Mon_Win,
		 SIGNAL( Command_Sent( QString ) ),
		 Cur_VM,
		 SLOT( Execute_Emu_Ctl_Command( QString ) ) );

	connect( Cur_VM,
		 SIGNAL( Clean_Console( QString ) ),
		 Mon_Win,
	...
	if ( usb_item )
	{
		...
		for ( int ix = 0; ix < ac_list.count(); ++ix )
		{
			...
			if ( usb_item->data().toString() == ac_list[ix]->data().toString() )
			{
				AQWarning(
					"void "
					"Emulator_Control_Window::Add_USB_To_VM()",
...
void Emulator_Control_Window::on_actionUSB_Update_Device_List_triggered()
{
	...
	{
		QAction *new_act = new QAction( tmp_list[ix].Get_Product_Name(),
						ui.menuUSB_Connect );
...
void Emulator_Control_Window::on_actionOther_Keys_triggered()
{
	...
	if ( ok && !keys.isEmpty() ) emit Ready_Read_Command( "sendkey " + keys );
}
...
QString convert_char( QString s )
{
	...
	if ( s == "(" ) return "shift-9";
	if ( s == ")" ) return "shift-0";
	if ( s == "_" ) return "shift-minus";
	if ( s == "<" ) return "shift-comma";
	if ( s == ">" ) return "shift-dot";
	if ( s == ">" ) return "shift-dot";
	if ( s == "\n" ) return "kp_enter";
	if ( s.at( 0 ).isLetter() && s.at( 0 ).isUpper() )
		return "shift-" + s.toLower();
	if ( s == "[" ) return "ctrl-shift-u-5-b";
	if ( s == "]" ) return "ctrl-shift-u-5-b";
	if ( s == "{" ) return "ctrl-shift-u-7-b";
	if ( s == "}" ) return "ctrl-shift-u-7-d";
	if ( s == "'" ) return "apostrophe";
	if ( s == "\"" ) return "shift-apostrophe";
...
void Emulator_Control_Window::on_actionClipboard_triggered()
{
	...
	if ( !text.isEmpty() )
	{
		...
		{
			emit Ready_Read_Command( QString( "sendkey " )
						 + convert_char( text.at( i ) ) );
		}
	}
}
	...
	if ( ui.actionFullscreen_Mode->isChecked() )
	{
		if ( Settings.value( "Show_Fullscreen_Warning", "yes" ).toString() == "yes" )
		{
			int ret = QMessageBox::question(
				this,
				tr( "Fullscreen mode" ),
				tr( "To return from fullscreen to windowed mode "
...
{
	QMessageBox::information( this, tr( "About Emulator Control..." ), tr( "Emulator Control is a Tool for Managing a Virtual Machine at Runtime." ) );
}
...
void Emulator_Control_Window::Set_Device( const QString &dev_name, const QString &path )
{
	...
	else
	{
		...
		if ( dev_name.contains( "-cd" ) )    // CD-ROM?
		{
			...
			else
			{
				...
				if ( cdromCount == 1 && deviceIndex != -1 )
				{
					...
					Cur_VM->Set_Storage_Device( deviceIndex,
								    devList[deviceIndex] );
				}
				else
				{
		...
		else if ( dev_name.contains( "floppy" ) )    // Floppy?
		{
			if ( dev_name == "floppy0" )
			{
				Cur_VM->Set_FD0( VM_Storage_Device(
					true,
					path,
			...
			{
				Cur_VM->Set_FD1( VM_Storage_Device(
					true,
					path,
