...
Virtual_Machine::Virtual_Machine( const Virtual_Machine &vm )
{
	...
	QObject::connect( Emu_Ctl,
			  SIGNAL( Ready_Read_Command( QString ) ),
			  this,
			  SLOT( Execute_Emu_Ctl_Command( QString ) ) );

	QObject::connect( QEMU_Process,
...
bool Virtual_Machine::operator==( const Virtual_Machine &vm ) const
{
	if ( this->Icon_Path == vm.Get_Icon_Path()
	     ...
	     && this->Show_Cursor == vm.Use_Show_Cursor()
	     && this->Curses == vm.Use_Curses() && this->RTC_TD_Hack == vm.Use_RTC_TD_Hack()
	     && this->Start_Date == vm.Use_Start_Date()
	     && this->Start_DateTime == vm.Get_Start_Date()
	     ...
	     && this->VNC_Display_Number == vm.Get_VNC_Display_Number()
	     && this->VNC_Password == vm.Use_VNC_Password()
	     && this->VNC_TLS == vm.Use_VNC_TLS() && this->VNC_x509 == vm.Use_VNC_x509()
	     && this->VNC_x509_Folder_Path == vm.Get_VNC_x509_Folder_Path()
	     && this->VNC_x509verify == vm.Use_VNC_x509verify()
	     && this->VNC_x509verify_Folder_Path == vm.Get_VNC_x509verify_Folder_Path() )
	{
		...
		if ( Boot_Order_List.count() == vm.Get_Boot_Order_List().count() )
		{
			for ( int bx = 0; bx < Boot_Order_List.count(); bx++ )
			{
				if ( ( Boot_Order_List[bx].Enabled
				     ...
				     || ( Boot_Order_List[bx].Type
					  != vm.Get_Boot_Order_List()[bx].Type ) )
					return false;
			}
		}
		else
		...
		if ( Storage_Devices.count() == vm.Get_Storage_Devices_List().count() )
		{
			...
			{
				if ( Storage_Devices[sx]
				     != vm.Get_Storage_Devices_List()[sx] )
					return false;
			}
		}
		else
		...
		if ( Network_Cards_Nativ.count() == tmp_card_nativ.count() )
		{
			...
			{
				if ( Network_Cards_Nativ[nx] != tmp_card_nativ[nx] )
					return false;
			}
		}
		else
		...
		if ( Get_Network_Redirections_Count() == vm.Get_Network_Redirections_Count() )
		{
			...
			{
				if ( Network_Redirections[rx]
				     != vm.Get_Network_Redirection( rx ) )
					return false;
			}
		}
		else
...
bool Virtual_Machine::Create_VM_File( const QString &file_name, bool template_mode )
{
	...
	{
		AQError( "bool Virtual_Machine::Create_VM_File( const QString "
	...
	QDomProcessingInstruction Pro_Instr = New_Dom_Document.createProcessingInstruction(
		"xml",
	...
	QDomElement VM_Element =
		New_Dom_Document.createElement( "Virtual_Machine" );
	...
	if ( template_mode )
	{
		...
		Dom_Text = New_Dom_Document.createTextNode(
			QString::number( (int)Template_Opts ) );
	...
	else
	{
		switch ( State )
		{
			...
			case VM::VMS_Saved:
				Dom_Text =
					New_Dom_Document.createTextNode( "Saved" );
				break;
	...
	VM_Element.appendChild( Dom_Element );
	Dom_Text = New_Dom_Document.createTextNode(
		VM::Accel_To_String( Machine_Accelerator ) );
	Dom_Element.appendChild( Dom_Text );
	...
	VM_Element.appendChild( Dom_Element );
	Dom_Text = New_Dom_Document.createTextNode(
		QString::number( Boot_Order_List.count() ) );
	Dom_Element.appendChild( Dom_Text );

	for ( int bx = 0; bx < Boot_Order_List.count(); bx++ )
	{
		Dom_Element = New_Dom_Document.createElement(
			"Boot_Device_" + QString::number( bx ) );
		...
		switch ( Boot_Order_List[bx].Type )
		{
			...
			case VM::Boot_From_CDROM:
				Dom_Text =
					New_Dom_Document.createTextNode( "CDROM" );
				break;
			...
			default:
				AQWarning( "bool Virtual_Machine::Create_VM_File( "
	...
	// Check Free RAM
	Dom_Element =
		New_Dom_Document.createElement( "Remove_RAM_Size_Limitation" );
	VM_Element.appendChild( Dom_Element );
	     ...
	     && !( Template_Opts & Create_Template_Window::Template_Save_FDD_CD ) )
	{
		...
		Save_VM_Native_Storage_Device( New_Dom_Document,
					       Sec_Element,
		       ...
		       && Template_Opts & Create_Template_Window::Template_Save_HDD ) ) )
	{
		...
		Dom_Element =
			New_Dom_Document.createElement( "Storage_Device_Count" );
		VM_Element.appendChild( Dom_Element );
		Dom_Text = New_Dom_Document.createTextNode(
	...
	if ( true /*(template_mode == false || (template_mode == true && Template_Opts & Create_Template_Window::Template_Save_Shared_Folders)) //TODO: not using template mode, not sure if it's needed */ )
	{
		// Storage Device Count
		Dom_Element =
			New_Dom_Document.createElement( "Shared_Folder_Count" );
		VM_Element.appendChild( Dom_Element );
		Dom_Text = New_Dom_Document.createTextNode(
		...
		{
			Dom_Element = New_Dom_Document.createElement(
				"Shared_Folder_" + QString::number( sx ) );
			Save_VM_Shared_Folder( New_Dom_Document,
					       Dom_Element,
					       Shared_Folders[sx] );
			VM_Element.appendChild( Dom_Element );
		}
	...
	else
	{
		...
		VM_Element.appendChild( Dom_Element );
		Dom_Text = New_Dom_Document.createTextNode(
			QString::number( Snapshots.count() ) );
		Dom_Element.appendChild( Dom_Text );
		...
		for ( int sx = 0; sx < Snapshots.count(); ++sx )
		{
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Snapshots[sx].Get_Name() );
			Sec_Element.appendChild( Dom_Text );

			// Description
			Sec_Element =
				New_Dom_Document.createElement( "Description" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Snapshots[sx].Get_Description() );
			Sec_Element.appendChild( Dom_Text );
	     ...
	     && !( Template_Opts & Create_Template_Window::Template_Save_Network ) )
	{
		...
		// Network Cards Count
		Dom_Element =
			New_Dom_Document.createElement( "Network_Cards_Count" );
		VM_Element.appendChild( Dom_Element );
		Dom_Text = New_Dom_Document.createTextNode( QString::number( 0 ) );
		Dom_Element.appendChild( Dom_Text );
		...
		// Redirections count
		Dom_Element =
			New_Dom_Document.createElement( "Redirections_Count" );
		VM_Element.appendChild( Dom_Element );
		Dom_Text = New_Dom_Document.createTextNode( QString::number( 0 ) );
		Dom_Element.appendChild( Dom_Text );
	...
	else
	{
		...
		VM_Element.appendChild( Dom_Element );
		Dom_Text = New_Dom_Document.createTextNode(
			QString::number( Network_Cards.count() ) );
		Dom_Element.appendChild( Dom_Text );
		...
		for ( int nx = 0; nx < Network_Cards.count(); nx++ )
		{
			...
			// Card Model
			Sec_Element =
				New_Dom_Document.createElement( "Card_Model" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards[nx].Get_Card_Model() );
			Sec_Element.appendChild( Dom_Text );

			// Network Mode
			Sec_Element =
				New_Dom_Document.createElement( "Network_Mode" );
			Dom_Element.appendChild( Sec_Element );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards[nx].Get_Hostname() );
			Sec_Element.appendChild( Dom_Text );

			// IP Address
			Sec_Element =
				New_Dom_Document.createElement( "IP_Address" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards[nx].Get_IP_Address() );
			Sec_Element.appendChild( Dom_Text );

			// MAC Address
			Sec_Element =
				New_Dom_Document.createElement( "MAC_Address" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards[nx].Get_MAC_Address() );
			Sec_Element.appendChild( Dom_Text );
			...
			if ( Network_Cards[nx].Get_Use_TUN_TAP_Script() )
				Dom_Text =
					New_Dom_Document.createTextNode( "true" );
			else
				Dom_Text =
					New_Dom_Document.createTextNode( "false" );

			Sec_Element.appendChild( Dom_Text );

			// TUN TAP Script
			Sec_Element =
				New_Dom_Document.createElement( "TUN_TAP_Script" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
			...
			// Interface Name
			Sec_Element =
				New_Dom_Document.createElement( "Interface_Name" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
			...
			// File Descriptor
			Sec_Element =
				New_Dom_Document.createElement( "File_Descriptor" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode( QString::number(
				Network_Cards[nx].Get_File_Descriptor() ) );
			Sec_Element.appendChild( Dom_Text );
		...
		VM_Element.appendChild( Dom_Element );
		Dom_Text = New_Dom_Document.createTextNode(
			QString::number( Network_Cards_Nativ.count() ) );
		Dom_Element.appendChild( Dom_Text );
		...
		for ( int nx = 0; nx < Network_Cards_Nativ.count(); nx++ )
		{
			...
			Sec_Element =
				New_Dom_Document.createElement( "Network_Type" );
			Dom_Element.appendChild( Sec_Element );
			...
			switch ( Network_Cards_Nativ[nx].Get_Network_Type() )
			{
				...
				default:
					AQWarning(
						"bool "
						"Virtual_Machine::Create_VM_File( "
						"const QString &file_name, bool "
						"template_mode )",
						"Invalid Network Type. Use "
						"Default: User" );
					Dom_Text = New_Dom_Document.createTextNode( "User" );
					break;
			}
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_Card_Model() );
			Sec_Element.appendChild( Dom_Text );

			// MAC_Address
			Sec_Element =
				New_Dom_Document.createElement( "Use_MAC_Address" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_MAC_Address() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_Name() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "Name" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_Name() );
			Sec_Element.appendChild( Dom_Text );

			// Hostname
			Sec_Element =
				New_Dom_Document.createElement( "Use_Hostname" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_Hostname() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "Hostname" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_Hostname() );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_PortDev() );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_File_Descriptor()
					? "yes"
					: "no" );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode( QString::number(
				Network_Cards_Nativ[nx].Get_File_Descriptor() ) );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_Interface_Name() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_Interface_Name() );
			Sec_Element.appendChild( Dom_Text );

			// Bridge_Name
			Sec_Element =
				New_Dom_Document.createElement( "Use_Bridge_Name" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_Bridge_Name() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element =
				New_Dom_Document.createElement( "Bridge_Name" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_Bridge_Name() );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_TUN_TAP_Script() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_TUN_TAP_Script() );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_TUN_TAP_Down_Script()
					? "yes"
					: "no" );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_TUN_TAP_Down_Script() );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_Bridge_Helper() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element =
				New_Dom_Document.createElement( "Bridge_Helper" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_Bridge_Helper() );
			Sec_Element.appendChild( Dom_Text );

			// Listen
			Sec_Element =
				New_Dom_Document.createElement( "Use_Listen" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_Listen() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "Listen" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_Listen() );
			Sec_Element.appendChild( Dom_Text );

			// Connect
			Sec_Element =
				New_Dom_Document.createElement( "Use_Connect" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_Connect() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "Connect" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_Connect() );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_MCast() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "MCast" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_MCast() );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_Sock() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "Sock" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_Sock() );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_Port() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "Port" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				QString::number( Network_Cards_Nativ[nx].Get_Port() ) );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_Group() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "Group" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_Group() );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_Mode() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "Mode" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_Mode() );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_File() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "File" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_File() );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_Len() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "Len" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				QString::number( Network_Cards_Nativ[nx].Get_Len() ) );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_Addr() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "Addr" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_Addr() );
			Sec_Element.appendChild( Dom_Text );

			// Vectors
			Sec_Element =
				New_Dom_Document.createElement( "Use_Vectors" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_Vectors() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "Vectors" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode( QString::number(
				Network_Cards_Nativ[nx].Get_Vectors() ) );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_Net() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "Net" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_Net() );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_Host() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "Host" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_Host() );
			Sec_Element.appendChild( Dom_Text );

			// Restrict
			Sec_Element =
				New_Dom_Document.createElement( "Use_Restrict" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_Restrict() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "Restrict" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_Restrict() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			// DHCPstart
			Sec_Element =
				New_Dom_Document.createElement( "Use_DHCPstart" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_DHCPstart() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "DHCPstart" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_DHCPstart() );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_DNS() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "DNS" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_DNS() );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_Tftp() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "Tftp" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_Tftp() );
			Sec_Element.appendChild( Dom_Text );

			// Bootfile
			Sec_Element =
				New_Dom_Document.createElement( "Use_Bootfile" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_Bootfile() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "Bootfile" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_Bootfile() );
			Sec_Element.appendChild( Dom_Text );

			// HostFwd
			Sec_Element =
				New_Dom_Document.createElement( "Use_HostFwd" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_HostFwd() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "HostFwd" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_HostFwd() );
			Sec_Element.appendChild( Dom_Text );

			// GuestFwd
			Sec_Element =
				New_Dom_Document.createElement( "Use_GuestFwd" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_GuestFwd() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "GuestFwd" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_GuestFwd() );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_SMB() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "SMB" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_SMB() );
			Sec_Element.appendChild( Dom_Text );

			// SMBserver
			Sec_Element =
				New_Dom_Document.createElement( "Use_SMBserver" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_SMBserver() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "SMBserver" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_SMBserver() );
			Sec_Element.appendChild( Dom_Text );

			// Sndbuf
			Sec_Element =
				New_Dom_Document.createElement( "Use_Sndbuf" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_Sndbuf() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "Sndbuf" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode( QString::number(
				Network_Cards_Nativ[nx].Get_Sndbuf() ) );
			Sec_Element.appendChild( Dom_Text );

			// VNet_hdr
			Sec_Element =
				New_Dom_Document.createElement( "Use_VNet_hdr" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_VNet_hdr() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "VNet_hdr" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_VNet_hdr() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_VHost() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "VHost" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Get_VHost() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			// VHostFd
			Sec_Element =
				New_Dom_Document.createElement( "Use_VHostFd" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				Network_Cards_Nativ[nx].Use_VHostFd() ? "yes" : "no" );
			Sec_Element.appendChild( Dom_Text );

			Sec_Element = New_Dom_Document.createElement( "VHostFd" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode( QString::number(
				Network_Cards_Nativ[nx].Get_VHostFd() ) );
			Sec_Element.appendChild( Dom_Text );
		...
		for ( int rx = 0; rx < Network_Redirections.count(); rx++ )
		{
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode( QString::number(
				Network_Redirections[rx].Get_Host_Port() ) );
			Sec_Element.appendChild( Dom_Text );
			...
			// Guest Port
			Sec_Element =
				New_Dom_Document.createElement( "Guest_Port" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode( QString::number(
				Network_Redirections[rx].Get_Guest_Port() ) );
			Sec_Element.appendChild( Dom_Text );
	...
	if ( template_mode && !( Template_Opts & Create_Template_Window::Template_Save_Ports ) )
	{
		// Serial Port
		Dom_Element =
			New_Dom_Document.createElement( "Serial_Ports_Count" );
		VM_Element.appendChild( Dom_Element );
		Dom_Text = New_Dom_Document.createTextNode( QString::number( 0 ) );
		Dom_Element.appendChild( Dom_Text );
		...
		// Parallel Port
		Dom_Element =
			New_Dom_Document.createElement( "Parallel_Ports_Count" );
		VM_Element.appendChild( Dom_Element );
		Dom_Text = New_Dom_Document.createTextNode( QString::number( 0 ) );
		Dom_Element.appendChild( Dom_Text );
	...
	else
	{
		...
		VM_Element.appendChild( Dom_Element );
		Dom_Text = New_Dom_Document.createTextNode(
			QString::number( Serial_Ports.count() ) );
		Dom_Element.appendChild( Dom_Text );

		for ( int px = 0; px < Serial_Ports.count(); px++ )
		{
			Dom_Element = New_Dom_Document.createElement(
				"Serial_Port_" + QString::number( px ) );
		...
		// Parallel Port
		Dom_Element =
			New_Dom_Document.createElement( "Parallel_Port_Count" );
		VM_Element.appendChild( Dom_Element );
		Dom_Text = New_Dom_Document.createTextNode(
		...
		{
			Dom_Element = New_Dom_Document.createElement(
				"Parallel_Port_" + QString::number( px ) );
		...
		VM_Element.appendChild( Dom_Element );
		Dom_Text = New_Dom_Document.createTextNode(
			QString::number( USB_Ports.count() ) );
		Dom_Element.appendChild( Dom_Text );
		...
		for ( int ux = 0; ux < USB_Ports.count(); ++ux )
		{
			...
			// Use Host Device
			Sec_Element =
				New_Dom_Document.createElement( "Use_Host_Device" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
			...
			// Product Name
			Sec_Element =
				New_Dom_Document.createElement( "Product_Name" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				USB_Ports[ux].Get_Product_Name() );
			Sec_Element.appendChild( Dom_Text );
			...
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
				USB_Ports[ux].Get_Vendor_ID() );
			Sec_Element.appendChild( Dom_Text );

			// Product_ID
			Sec_Element =
				New_Dom_Document.createElement( "Product_ID" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
			...
			// Serial Number
			Sec_Element =
				New_Dom_Document.createElement( "Serial_Number" );
			Dom_Element.appendChild( Sec_Element );
			Dom_Text = New_Dom_Document.createTextNode(
			...
			// Keyboard
			Dom_Element =
				New_Dom_Document.createElement( "USB_Keyboard" );
			VM_Element.appendChild( Dom_Element );
			...
			// Tablet
			Dom_Element =
				New_Dom_Document.createElement( "USB_Tablet" );
			VM_Element.appendChild( Dom_Element );
			...
			// Wacom Tablet
			Dom_Element =
				New_Dom_Document.createElement( "USB_WacomTablet" );
			VM_Element.appendChild( Dom_Element );
			...
			// Braille
			Dom_Element =
				New_Dom_Document.createElement( "USB_Braille" );
			VM_Element.appendChild( Dom_Element );
	...
	Dom_Element =
		New_Dom_Document.createElement( "Use_KVM_No_Pit_Reinjection" );
	VM_Element.appendChild( Dom_Element );
	...
	VM_Element.appendChild( Dom_Element );
	Dom_Text = New_Dom_Document.createTextNode(
		QString::number( KVM_Shadow_Memory_Size ) );
	Dom_Element.appendChild( Dom_Text );
	...
	Dom_Element.appendChild( Sec_Element );
	Dom_Text = New_Dom_Document.createTextNode(
		QString::number( SPICE.Get_RAM_Size() ) );
	Sec_Element.appendChild( Dom_Text );
	...
	switch ( SPICE.Get_Image_Compression() )
	{
		...
		default:
			AQError( "bool Virtual_Machine::Create_VM_File( const "
				 "QString &file_name, bool template_mode )",
	...
	Sec_Element =
		New_Dom_Document.createElement( "Use_Video_Stream_Compression" );
	Dom_Element.appendChild( Sec_Element );
	Dom_Text = New_Dom_Document.createTextNode(
	...
	for ( int ix = 0; ix < SPICE.Get_Renderer_List().count(); ++ix )
	{
		switch ( SPICE.Get_Renderer_List()[ix] )
		{
			case VM::SPICE_Renderer_oglpbuf:
				Sec_Element =
					New_Dom_Document.createElement( "oglpbuf" );
				break;
			...
			case VM::SPICE_Renderer_cairo:
				Sec_Element =
					New_Dom_Document.createElement( "cairo" );
				break;

			default:
				AQError( "bool Virtual_Machine::Create_VM_File( "
					 "const QString &file_name, bool "
					 "template_mode )",
	...
	if ( VNC ) { Dom_Text = New_Dom_Document.createTextNode( "true" ); }
	...
	if ( VNC_TLS ) { Dom_Text = New_Dom_Document.createTextNode( "true" ); }
	else { Dom_Text = New_Dom_Document.createTextNode( "false" ); }
	...
	if ( VNC_x509 ) { Dom_Text = New_Dom_Document.createTextNode( "true" ); }
	else { Dom_Text = New_Dom_Document.createTextNode( "false" ); }
	...
	Dom_Element =
		New_Dom_Document.createElement( "VNC_x509verify_Folder_Path" );
	VM_Element.appendChild( Dom_Element );
...
bool Virtual_Machine::Create_Template( const QString &vm_path,
...
bool Virtual_Machine::Load_VM( const QString &file_name )
{
	...
	else
	{
		...
		{
			AQError( "bool Virtual_Machine::Load_VM( const QString "
				 "&file_name )",
				 QString( "Parse Error!\nLine: %1\nColumn: "
					  "%2\nString: %3" )
					 .arg( Error_Line )
		...
		else
		{
			...
			if ( Root_Element.tagName() != "AQEMU" )
			{
				...
				{
					AQError( "bool Virtual_Machine::Load_VM( "
						 "const QString &file_name )",
						 "This is not a AQEMU VM File!" );
					return false;
				}
				else { template_mode = true; }
			}
			...
			{
				aqemu_vm_file_version =
					Root_Element.attribute( "version" );
			}

			if ( aqemu_vm_file_version == "0.8" )
			{
				AQDebug( "bool Virtual_Machine::Load_VM( const "
					 "QString &file_name )",
			...
			{
				AQWarning( "bool Virtual_Machine::Load_VM( const "
					   "QString &file_name )",
			...
			else if ( aqemu_vm_file_version == "0.5" )
			{
				AQWarning( "bool Virtual_Machine::Load_VM( const "
					   "QString &file_name )",
					   "Uses compatible mode for config "
					   "version 0.5" );
				...
				// KVM ?
				if ( Child_Element
					     .firstChildElement( "Computer_Type" )
					     .text()
				     == "qemu-kvm" )
				{
			...
			{
				AQError( "bool Virtual_Machine::Load_VM( const "
					 "QString &file_name )",
					 "This is AQEMU VM File Version Not "
					 "Support!" );

				int ret_but = QMessageBox::question(
					NULL,
					tr( "Version Invalid!" ),
					tr( "Version of this AQEMU VM File isn't "
			...
			if ( template_mode )
			{
				// Template Options
				Template_Opts =
					(Create_Template_Window::Template_Options)Child_Element
						.firstChildElement(
							"Template_Options" )
						.text()
						.toInt();
			...
			if ( load_emulator )
			{
				...
				else
				{
					if ( !Child_Element
						      ...
						      .isEmpty() )
						accel = Child_Element
								.firstChildElement( "Emulator_Type" )
								.text();
				}
			...
			// Computer Type
			Computer_Type =
				Child_Element.firstChildElement( "Computer_Type" ).text();
			...
			else { Set_State( VM::VMS_Power_Off ); }

			// Check Emulator
			AQDebug( "bool Virtual_Machine::Load_VM( const QString "
				 "&file_name )",
			...
			if ( !Icon_Path.startsWith( ":/" ) )
			{
				...
				for ( int i = 0; i < dataDirs.count(); i++ )
				{
					...
					{
						Icon_Path.replace( dataDirs.at( i ),
								   settings.value( "AQEMU_Data_Folder",
										   "" )
									   .toString() );
						abs_found = true;
						break;
			...
			// Machine Name
			Machine_Name =
				Child_Element.firstChildElement( "Machine_Name" ).text();

			// Machine Type
			Machine_Type =
				Child_Element.firstChildElement( "Machine_Type" ).text();
			...
			// SMP
			SMP.SMP_Count =
				Child_Element.firstChildElement( "SMP_CPU_Count" )
					.text()
					.toInt();
			SMP.SMP_Cores =
				Child_Element.firstChildElement( "SMP_Cores" )
					.text()
					.toInt();
			SMP.SMP_Threads =
				Child_Element.firstChildElement( "SMP_Threads" )
					.text()
					.toInt();
			SMP.SMP_Sockets =
				Child_Element.firstChildElement( "SMP_Sockets" )
					.text()
					.toInt();
			SMP.SMP_MaxCPUs =
				Child_Element.firstChildElement( "SMP_MaxCPUs" )
					.text()
					.toInt();
			...
			if ( load_boot_order_setcton
			     && Child_Element.firstChildElement( "Boot_Device" )
					.text()
					.isEmpty() )
			{
				int bootOrderCount =
				...
				for ( int bx = 0; bx < bootOrderCount; bx++ )
				{
					...
					tmpBootDev.Enabled = Second_Element
								     .firstChildElement( "Enabled" )
								     .text()
							     == "true";
					QString bootType =
						Second_Element
			...
			Show_Boot_Menu =
				( Child_Element
					  .firstChildElement( "Show_Boot_Menu" )
					  .text()
				  == "true" );

			// Video Card
			Video_Card =
				Child_Element.firstChildElement( "Video_Card" ).text();

			// Audio Cards
			Second_Element =
				Child_Element.firstChildElement( "Audio_Cards" );

			VM::Sound_Cards snd_card;

			// Audio_sb16
			snd_card.Audio_sb16 =
				Second_Element.firstChildElement( "Audio_sb16" ).text()
				== "true";

			// Audio_es1370
			snd_card.Audio_es1370 =
				Second_Element.firstChildElement( "Audio_es1370" ).text()
				== "true";

			// Audio_Adlib
			snd_card.Audio_Adlib =
				Second_Element.firstChildElement( "Audio_Adlib" ).text()
				== "true";

			// Audio_PC_Speaker
			snd_card.Audio_PC_Speaker =
				Second_Element
					.firstChildElement( "Audio_PC_Speaker" )
					.text()
				== "true";

			// Audio_GUS
			snd_card.Audio_GUS =
				Second_Element.firstChildElement( "Audio_GUS" ).text()
				== "true";

			// Audio_AC97
			snd_card.Audio_AC97 =
				Second_Element.firstChildElement( "Audio_AC97" ).text()
				== "true";

			// Audio_HDA
			snd_card.Audio_HDA =
				Second_Element.firstChildElement( "Audio_HDA" ).text()
				== "true";

			// Audio_cs4231
			snd_card.Audio_cs4231a =
				Second_Element.firstChildElement( "Audio_cs4231a" ).text()
				== "true";

			Set_Audio_Cards( snd_card );

			// Check Free RAM
			Remove_RAM_Size_Limitation = ( Child_Element
							       .firstChildElement( "Remove_RAM_Size_Limitation" )
							       .text()
						       == "true" );

			// Memory Size ( RAM )
			Memory_Size =
				Child_Element.firstChildElement( "Memory_Size" )
					.text()
					.toInt();

			// Fullscreen
			Fullscreen =
				( Child_Element.firstChildElement( "Fullscreen" ).text()
				  == "true" );

			// Win2K_Hack
			Win2K_Hack =
				( Child_Element.firstChildElement( "Win2K_Hack" ).text()
				  == "true" );

			// Localtime
			Local_Time =
				( Child_Element.firstChildElement( "Local_Time" ).text()
				  == "true" );
			...
			// Snapshot_Mode
			Snapshot_Mode =
				( Child_Element.firstChildElement( "Snapshot_Mode" ).text()
				  == "true" );

			// Start_CPU
			Start_CPU = ( Child_Element.firstChildElement( "Start_CPU" ).text()
				      == "true" );

			// No_Reboot
			No_Reboot = ( Child_Element.firstChildElement( "No_Reboot" ).text()
				      == "true" );

			// No_Shutdown
			No_Shutdown =
				( Child_Element.firstChildElement( "No_Shutdown" ).text()
				  == "true" );
			...
			else
			{
				if ( old_version_storage_devices )
				{
					...
					Second_Element = Child_Element.firstChildElement(
						"FD0" );
					FD0.Set_Enabled( ( Second_Element
								   .firstChildElement( "Enabled" )
								   .text()
							   == "true" ) );
					...
					if ( Second_Element
						...
						FD0.Set_File_Name(
							Second_Element
								.firstChildElement( "Host_File_Name" )
								.text() );
					else
						FD0.Set_File_Name(
							Second_Element
								.firstChildElement( "Image_File_Name" )
								.text() );
					...
					Second_Element = Child_Element.firstChildElement(
						"FD1" );
					FD1.Set_Enabled( ( Second_Element
								   .firstChildElement( "Enabled" )
								   .text()
							   == "true" ) );
					...
					Second_Element = Child_Element.firstChildElement(
						"CD_ROM" );
					CD_ROM.Set_Enabled( ( Second_Element
								      .firstChildElement( "Enabled" )
								      .text()
							      == "true" ) );
				...
				else
				{
					...
					FD0.Set_Enabled( ( Second_Element
								   ...
								   .text()
							   == "true" ) );
					FD0.Set_File_Name( Second_Element
								   .firstChildElement( "File_Name" )
								   .text() );

					Second_Element = Second_Element.firstChildElement(
						"Native_Device" );
					FD0.Set_Native_Device( Load_VM_Native_Storage_Device(
						Second_Element ) );
					...
					FD1.Set_Enabled( ( Second_Element
								   ...
								   .text()
							   == "true" ) );
					FD1.Set_File_Name( Second_Element
								   .firstChildElement( "File_Name" )
								   .text() );

					Second_Element = Second_Element.firstChildElement(
						"Native_Device" );
					FD1.Set_Native_Device( Load_VM_Native_Storage_Device(
						Second_Element ) );
					...
					CD_ROM.Set_Enabled( ( Second_Element
								      ...
								      .text()
							      == "true" ) );
					CD_ROM.Set_File_Name( Second_Element
								      .firstChildElement( "File_Name" )
								      .text() );

					Second_Element = Second_Element.firstChildElement(
						"Native_Device" );
					CD_ROM.Set_Native_Device( Load_VM_Native_Storage_Device(
						Second_Element ) );
				}
			}
			...
			if ( template_mode )
			{
				if ( Template_Opts | Create_Template_Window::Template_Save_HDD )
				{
					...
					Second_Element = Child_Element.firstChildElement(
						"HDA" );
					HDA.Set_Enabled( ( Second_Element
								   .firstChildElement( "Enabled" )
								   .text()
							   == "true" ) );
					HDA.Set_Virtual_Size( HDA.String_to_Device_Size(
						Second_Element
							...
							.text() ) );
					HDA.Set_Image_Format( Second_Element
								      .firstChildElement( "Format" )
								      .text() );

					Second_Element = Second_Element.firstChildElement(
						"Native_Device" );
					HDA.Set_Native_Device( Load_VM_Native_Storage_Device(
						Second_Element ) );

					// HDB
					Second_Element = Child_Element.firstChildElement(
						"HDB" );
					HDB.Set_Enabled( ( Second_Element
								   .firstChildElement( "Enabled" )
								   .text()
							   == "true" ) );
					HDB.Set_Virtual_Size( HDB.String_to_Device_Size(
						Second_Element
							.firstChildElement( "Size" )
							.text() ) );
					HDB.Set_Image_Format( Second_Element
								      .firstChildElement( "Format" )
								      .text() );

					Second_Element = Second_Element.firstChildElement(
						"Native_Device" );
					HDB.Set_Native_Device( Load_VM_Native_Storage_Device(
						Second_Element ) );

					// HDC
					Second_Element = Child_Element.firstChildElement(
						"HDC" );
					HDC.Set_Enabled( ( Second_Element
								   .firstChildElement( "Enabled" )
								   .text()
							   == "true" ) );
					HDC.Set_Virtual_Size( HDC.String_to_Device_Size(
						Second_Element
							.firstChildElement( "Size" )
							.text() ) );
					HDC.Set_Image_Format( Second_Element
								      .firstChildElement( "Format" )
								      .text() );

					Second_Element = Second_Element.firstChildElement(
						"Native_Device" );
					HDC.Set_Native_Device( Load_VM_Native_Storage_Device(
						Second_Element ) );

					// HDD
					Second_Element = Child_Element.firstChildElement(
						"HDD" );
					HDD.Set_Enabled( ( Second_Element
								   .firstChildElement( "Enabled" )
								   .text()
							   == "true" ) );
					HDD.Set_Virtual_Size( HDD.String_to_Device_Size(
						Second_Element
							.firstChildElement( "Size" )
							.text() ) );
					HDD.Set_Image_Format( Second_Element
								      .firstChildElement( "Format" )
								      .text() );

					Second_Element = Second_Element.firstChildElement(
						"Native_Device" );
					HDD.Set_Native_Device( Load_VM_Native_Storage_Device(
						Second_Element ) );
				}
				else
				{
			...
			int Storage_Device_Count =
				Child_Element
					.firstChildElement( "Storage_Device_Count" )
					.text()
					.toInt();
			...
			int Shared_Folder_Count =
				Child_Element
					.firstChildElement( "Shared_Folder_Count" )
					.text()
					.toInt();
			...
			// Snapshots
			int Snapshots_Count =
				Child_Element.firstChildElement( "Snapshots_Count" )
					.text()
					.toInt();
			...
			for ( int sx = 0; sx < Snapshots_Count; ++sx )
			{
				...
				// Tag
				tmp_snapshot.Set_Tag(
					Second_Element.firstChildElement( "Tag" ).text() );

				// Name
				tmp_snapshot.Set_Name(
			...
			// Use Network
			Use_Network =
				( Child_Element.firstChildElement( "Use_Network" ).text()
				  == "true" );

			// Use Redirections
			Use_Redirections =
				( Child_Element
					  .firstChildElement( "Use_Redirections" )
					  .text()
				  == "true" );
			...
			int NetCardsCount =
				Child_Element
					.firstChildElement( "Network_Cards_Count" )
					.text()
					.toInt();
			Network_Cards.clear();
			...
			for ( int nx = 0; nx < NetCardsCount; nx++ )
			{
				...
				// Card Model
				tmp_card.Set_Card_Model( Second_Element
								 .firstChildElement( "Card_Model" )
								 .text() );

				QString tmp_str =
					Second_Element
						.firstChildElement( "Network_Mode" )
						.text();
				...
				else
				{
					...
					AQWarning( "bool Virtual_Machine::Load_VM( "
						   "const QString &file_name )",
						   "Net Card Network Mode Invalid! "
						   "Use Default!" );
				}

				// Hostname
				tmp_card.Set_Hostname( Second_Element
							       .firstChildElement( "Hostname" )
							       .text() );

				// IP Address
				tmp_card.Set_IP_Address( Second_Element
								 .firstChildElement( "IP_Address" )
								 .text() );

				// MAC Address
				tmp_card.Set_MAC_Address( Second_Element
								  .firstChildElement( "MAC_Address" )
								  .text() );

				// Port
				tmp_card.Set_Port(
					Second_Element.firstChildElement( "Port" )
						.text()
						.toInt() );

				// VLAN
				tmp_card.Set_VLAN(
					Second_Element.firstChildElement( "VLAN" )
						.text()
						.toInt() );

				// Use TUN TAP Script
				tmp_card.Set_Use_TUN_TAP_Script( !( Second_Element
									    .firstChildElement( "Use_TUN_TAP_Script" )
									    .text()
								    == "false" ) );

				// TUN TAP Script
				tmp_card.Set_TUN_TAP_Script( Second_Element
								     .firstChildElement( "TUN_TAP_Script" )
								     .text() );

				// Interface Name
				tmp_card.Set_Interface_Name( Second_Element
								     .firstChildElement( "Interface_Name" )
								     .text() );

				// File Descriptor
				tmp_card.Set_File_Descriptor( Second_Element
								      .firstChildElement( "File_Descriptor" )
								      .text()
								      .toInt() );
			...
			Native_Network =
				( Child_Element
					  .firstChildElement( "Native_Network" )
					  .text()
				  == "true" );
			...
			for ( int nx = 0; nx < NetCardsNativCount; nx++ )
			{
				...
				QString netCardNative_Type =
					Second_Element
						.firstChildElement( "Network_Type" )
						.text();
				...
				if ( netCardNative_Type == "NIC" )
					...
					tmp_card.Set_Network_Type(
						VM::Net_Mode_Native_MulticastSocket );
				...
				tmp_card.Set_Card_Model( Second_Element
								 .firstChildElement( "Card_Model" )
								 .text() );
				tmp_card.Use_MAC_Address( Second_Element
								  .firstChildElement( "Use_MAC_Address" )
								  .text()
							  == "yes" );
				...
				tmp_card.Use_Name(
					Second_Element
						.firstChildElement( "Use_Name" )
						.text()
					== "yes" );

				tmp_card.Set_Name(
					Second_Element.firstChildElement( "Name" ).text() );
				tmp_card.Use_Hostname( Second_Element
							       .firstChildElement( "Use_Hostname" )
							       .text()
						       == "yes" );
				...
				tmp_card.Set_Hostname( Second_Element
							       .firstChildElement( "Hostname" )
							       .text() );
				tmp_card.Set_PortDev( Second_Element
							      .firstChildElement( "PortDev" )
							      .text() );

				tmp_card.Use_File_Descriptor( Second_Element
								      .firstChildElement( "Use_File_Descriptor" )
								      .text()
							      == "yes" );
				tmp_card.Set_File_Descriptor( Second_Element
								      .firstChildElement( "File_Descriptor" )
				...
				tmp_card.Use_Interface_Name( Second_Element
								     .firstChildElement( "Use_Interface_Name" )
								     .text()
							     == "yes" );
				tmp_card.Set_Interface_Name( Second_Element
								     .firstChildElement( "Interface_Name" )
				...
				tmp_card.Use_Bridge_Name( Second_Element
								  .firstChildElement( "Use_Bridge_Name" )
								  .text()
							  == "yes" );
				tmp_card.Set_Bridge_Name( Second_Element
								  .firstChildElement( "Bridge_Name" )
								  .text() );

				tmp_card.Use_TUN_TAP_Script( Second_Element
								     .firstChildElement( "Use_TUN_TAP_Script" )
								     .text()
							     == "yes" );
				tmp_card.Set_TUN_TAP_Script( Second_Element
				...
				tmp_card.Use_TUN_TAP_Down_Script(
					Second_Element
						...
						.text()
					== "yes" );
				tmp_card.Set_TUN_TAP_Down_Script( Second_Element
									  .firstChildElement( "TUN_TAP_Down_Script" )
									  .text() );

				tmp_card.Use_Bridge_Helper( Second_Element
								    .firstChildElement( "Use_Bridge_Helper" )
								    .text()
							    == "yes" );
				tmp_card.Set_Bridge_Helper( Second_Element
								    .firstChildElement( "Bridge_Helper" )
								    .text() );

				tmp_card.Use_Listen(
					Second_Element
						.firstChildElement( "Use_Listen" )
						.text()
					== "yes" );
				tmp_card.Set_Listen(
					Second_Element.firstChildElement( "Listen" ).text() );

				tmp_card.Use_Connect( Second_Element
							      .firstChildElement( "Use_Connect" )
							      .text()
						      == "yes" );
				tmp_card.Set_Connect( Second_Element
							      .firstChildElement( "Connect" )
							      .text() );

				tmp_card.Use_MCast(
					Second_Element
						.firstChildElement( "Use_MCast" )
						.text()
					== "yes" );
				tmp_card.Set_MCast(
					Second_Element.firstChildElement( "MCast" ).text() );

				tmp_card.Use_Sock(
					Second_Element
						.firstChildElement( "Use_Sock" )
						.text()
					== "yes" );
				tmp_card.Set_Sock(
					Second_Element.firstChildElement( "Sock" ).text() );

				tmp_card.Use_Port(
					Second_Element
						.firstChildElement( "Use_Port" )
						.text()
					== "yes" );
				tmp_card.Set_Port(
					Second_Element.firstChildElement( "Port" )
				...
				tmp_card.Use_Group(
					Second_Element
						.firstChildElement( "Use_Group" )
						.text()
					== "yes" );
				tmp_card.Set_Group(
					Second_Element.firstChildElement( "Group" ).text() );

				tmp_card.Use_Mode(
					Second_Element
						.firstChildElement( "Use_Mode" )
						.text()
					== "yes" );
				tmp_card.Set_Mode(
					Second_Element.firstChildElement( "Mode" ).text() );

				tmp_card.Use_File(
					Second_Element
						.firstChildElement( "Use_File" )
						.text()
					== "yes" );
				tmp_card.Set_File(
					Second_Element.firstChildElement( "File" ).text() );

				tmp_card.Use_Len(
					Second_Element
						.firstChildElement( "Use_Len" )
						.text()
					== "yes" );
				tmp_card.Set_Len(
					Second_Element.firstChildElement( "Len" )
						.text()
						.toInt() );

				tmp_card.Use_Addr(
					Second_Element
						.firstChildElement( "Use_Addr" )
						.text()
					== "yes" );
				tmp_card.Set_Addr(
					Second_Element.firstChildElement( "Addr" ).text() );

				tmp_card.Use_Vectors( Second_Element
							      .firstChildElement( "Use_Vectors" )
							      .text()
						      == "yes" );
				tmp_card.Set_Vectors( Second_Element
							      .firstChildElement( "Vectors" )
							      .text()
							      .toInt() );

				tmp_card.Use_Net(
					Second_Element
						.firstChildElement( "Use_Net" )
						.text()
					== "yes" );
				tmp_card.Set_Net(
					Second_Element.firstChildElement( "Net" ).text() );

				tmp_card.Use_Host(
					Second_Element
						.firstChildElement( "Use_Host" )
						.text()
					== "yes" );
				tmp_card.Set_Host(
					Second_Element.firstChildElement( "Host" ).text() );

				tmp_card.Use_Restrict( Second_Element
							       .firstChildElement( "Use_Restrict" )
							       .text()
						       == "yes" );
				tmp_card.Set_Restrict( Second_Element
							       .firstChildElement( "Restrict" )
							       .text()
						       == "yes" );

				tmp_card.Use_DHCPstart( Second_Element
								.firstChildElement( "Use_DHCPstart" )
								.text()
							== "yes" );
				tmp_card.Set_DHCPstart( Second_Element
								.firstChildElement( "DHCPstart" )
								.text() );

				tmp_card.Use_DNS(
					Second_Element
						.firstChildElement( "Use_DNS" )
						.text()
					== "yes" );
				tmp_card.Set_DNS(
					Second_Element.firstChildElement( "DNS" ).text() );

				tmp_card.Use_Tftp(
					Second_Element
						.firstChildElement( "Use_Tftp" )
						.text()
					== "yes" );
				tmp_card.Set_Tftp(
					Second_Element.firstChildElement( "Tftp" ).text() );

				tmp_card.Use_Bootfile( Second_Element
							       .firstChildElement( "Use_Bootfile" )
							       .text()
						       == "yes" );
				tmp_card.Set_Bootfile( Second_Element
							       .firstChildElement( "Bootfile" )
							       .text() );

				tmp_card.Use_HostFwd( Second_Element
							      .firstChildElement( "Use_HostFwd" )
							      .text()
						      == "yes" );
				tmp_card.Set_HostFwd( Second_Element
							      .firstChildElement( "HostFwd" )
							      .text() );

				tmp_card.Use_GuestFwd( Second_Element
							       .firstChildElement( "Use_GuestFwd" )
							       .text()
						       == "yes" );
				tmp_card.Set_GuestFwd( Second_Element
							       .firstChildElement( "GuestFwd" )
							       .text() );

				tmp_card.Use_SMB(
					Second_Element
						.firstChildElement( "Use_SMB" )
						.text()
					== "yes" );
				tmp_card.Set_SMB(
					Second_Element.firstChildElement( "SMB" ).text() );

				tmp_card.Use_SMBserver( Second_Element
								.firstChildElement( "Use_SMBserver" )
								.text()
							== "yes" );
				tmp_card.Set_SMBserver( Second_Element
								.firstChildElement( "SMBserver" )
								.text() );

				tmp_card.Use_Sndbuf(
					Second_Element
						.firstChildElement( "Use_Sndbuf" )
						.text()
					== "yes" );
				tmp_card.Set_Sndbuf(
					Second_Element.firstChildElement( "Sndbuf" )
						.text()
						.toInt() );

				tmp_card.Use_VNet_hdr( Second_Element
							       .firstChildElement( "Use_VNet_hdr" )
							       .text()
						       == "yes" );
				tmp_card.Set_VNet_hdr( Second_Element
							       .firstChildElement( "VNet_hdr" )
							       .text()
						       == "yes" );

				tmp_card.Use_VHost(
					Second_Element
						.firstChildElement( "Use_VHost" )
						.text()
					== "yes" );
				tmp_card.Set_VHost(
					Second_Element.firstChildElement( "VHost" ).text()
					== "yes" );

				tmp_card.Use_VHostFd( Second_Element
							      .firstChildElement( "Use_VHostFd" )
							      .text()
						      == "yes" );
				tmp_card.Set_VHostFd( Second_Element
							      .firstChildElement( "VHostFd" )
							      .text()
							      .toInt() );
			...
			int RedirsCount =
				Child_Element
					.firstChildElement( "Redirections_Count" )
					.text()
					.toInt();
			...
			for ( int rx = 0; rx < RedirsCount; rx++ )
			{
				...
				tmp_redir.Set_Protocol( Second_Element
								.firstChildElement( "Protocol" )
								.text() );
				tmp_redir.Set_Host_Port( ( Second_Element
								   .firstChildElement( "Host_Port" )
								   .text()
								   .toInt() ) );
				tmp_redir.Set_Guest_IP( Second_Element
								.firstChildElement( "Guest_IP" )
								.text() );
				tmp_redir.Set_Guest_Port( Second_Element
								  .firstChildElement( "Guest_Port" )
								  .text()
								  .toInt() );
			...
			// TFTP Prefix
			TFTP_Prefix =
				Child_Element.firstChildElement( "TFTP_Prefix" ).text();

			// SMB Directory
			SMB_Directory =
				Child_Element.firstChildElement( "SMB_Directory" ).text();

			// Serial Ports Count
			int serial_count =
				Child_Element
					.firstChildElement( "Serial_Port_Count" )
					.text()
					.toInt();
			...
			// USB Count
			int usb_count =
				Child_Element.firstChildElement( "USB_Ports_Count" )
					.text()
					.toInt();
			...
			for ( int ux = 0; ux < usb_count; ++ux )
			{
				...
				tmp_usb.Set_Use_Host_Device( Second_Element
								     .firstChildElement( "Use_Host_Device" )
								     .text()
							     == "true" );
				tmp_usb.Set_Manufacturer_Name( Second_Element
								       .firstChildElement( "Manufacturer_Name" )
								       .text() );
				tmp_usb.Set_Product_Name( Second_Element
								  .firstChildElement( "Product_Name" )
								  .text() );
				tmp_usb.Set_Vendor_ID( Second_Element
							       .firstChildElement( "Vendor_ID" )
							       .text() );
				tmp_usb.Set_Product_ID( Second_Element
								.firstChildElement( "Product_ID" )
								.text() );
				tmp_usb.Set_Serial_Number( Second_Element
								   .firstChildElement( "Serial_Number" )
								   .text() );
				tmp_usb.Set_Speed(
					Second_Element.firstChildElement( "Speed" ).text() );

				// For AQEMU VM files version 0.8.2 and oldest
				if ( !Second_Element.firstChildElement( "BusAddr" )
					      .text()
					      .isEmpty() )
				{
					...
					if ( busAddrList.count() != 2 )
					{
						AQError(
							...
							"const QString &file_name "
							")",
							"Cannot parse obsolete USB "
							"Bus.Addr string! Data: "
								+ Second_Element
									  .firstChildElement( "BusAddr" )
									  .text() );
					}
					else
					{
				...
				{
					tmp_usb.Set_Bus( Second_Element
								 .firstChildElement( "Bus" )
								 .text() );
					tmp_usb.Set_Addr( Second_Element
								  .firstChildElement( "Addr" )
								  .text() );
					tmp_usb.Set_DevPath( Second_Element
				...
				// Mouse
				usb_m = ( Child_Element
						  .firstChildElement( "USB_Mouse" )
						  .text()
					  == "true" );

				// Tablet
				usb_t = ( Child_Element
						  .firstChildElement( "USB_Tablet" )
						  .text()
					  == "true" );

				// Wacom Tablet
				usb_wt = ( Child_Element
						   .firstChildElement(
							   "USB_WacomTablet" )
						   .text()
					   == "true" );
			...
			// Linux Boot
			Linux_Boot =
				( Child_Element.firstChildElement( "Linux_Boot" ).text()
				  == "true" );

			// bzImage Path
			bzImage_Path =
				Child_Element.firstChildElement( "bzImage_Path" ).text();

			// Initrd Path
			Initrd_Path =
				Child_Element.firstChildElement( "Initrd_Path" ).text();

			// Kernel Command Line Arguments
			Kernel_ComLine =
				Child_Element.firstChildElement( "Kernel_ComLine" ).text();

			// Use ROM File
			Use_ROM_File =
				( Child_Element.firstChildElement( "Use_ROM_File" ).text()
				  == "true" );
			...
			MTDBlock =
				( Child_Element
					  .firstChildElement( "Use_MTDBlock_File" )
					  .text()
				  == "true" );

			// MTDBlock File
			MTDBlock_File =
				Child_Element.firstChildElement( "MTDBlock_File" ).text();

			// Use SecureDigital File
			SecureDigital = ( Child_Element
						  .firstChildElement(
							  "Use_SecureDigital_File" )
						  .text()
					  == "true" );
			...
			SecureDigital_File =
				Child_Element
					.firstChildElement( "SecureDigital_File" )
					.text();

			// Use PFlash File
			PFlash = ( Child_Element
					   .firstChildElement( "Use_PFlash_File" )
					   .text()
				   == "true" );

			// PFlash File
			PFlash_File =
				Child_Element.firstChildElement( "PFlash_File" ).text();

			// Enable KVM
			Enable_KVM =
				!( Child_Element.firstChildElement( "Enable_KVM" ).text()
				   == "false" );

			// Use KVM IRQChip
			KVM_IRQChip =
				( Child_Element
					  .firstChildElement( "Use_KVM_IRQChip" )
					  .text()
				  == "true" );
			...
			No_KVM_Pit =
				( Child_Element
					  .firstChildElement( "Use_No_KVM_Pit" )
					  .text()
				  == "true" );
			...
			KVM_Nesting =
				( Child_Element
					  .firstChildElement( "Use_KVM_Nesting" )
					  .text()
				  == "true" );
			...
			Init_Graphic_Mode.Set_Enabled(
				Second_Element.firstChildElement( "Enabled" ).text()
				== "true" );
			...
			// Use No Frame
			No_Frame =
				( Child_Element.firstChildElement( "Use_No_Frame" ).text()
				  == "true" );

			// Use Alt Grab
			Alt_Grab =
				( Child_Element.firstChildElement( "Use_Alt_Grab" ).text()
				  == "true" );

			// Use No Quit
			No_Quit = ( Child_Element.firstChildElement( "Use_No_Quit" ).text()
				    == "true" );

			// Use Portrait
			Portrait =
				( Child_Element.firstChildElement( "Use_Portrait" ).text()
				  == "true" );

			// Show_Cursor
			Show_Cursor =
				( Child_Element.firstChildElement( "Show_Cursor" ).text()
				  == "true" );

			// Curses
			Curses = ( Child_Element.firstChildElement( "Curses" ).text()
				   == "true" );

			// RTC_TD_Hack
			RTC_TD_Hack =
				( Child_Element.firstChildElement( "RTC_TD_Hack" ).text()
				  == "true" );

			// Start Date
			Start_Date =
				( Child_Element
					  .firstChildElement( "Use_Start_Date" )
					  .text()
				  == "true" );
			...
			if ( !Second_Element.isNull() )
			{
				...
				SPICE.Use_SPICE(
					Second_Element
						.firstChildElement( "Use_SPICE" )
						.text()
					== "true" );

				SPICE.Set_GXL_Devices_Count( Second_Element
								     .firstChildElement( "GXL_Devices_Count" )
								     .text()
								     .toUInt( &ok ) );
				if ( !ok )
					AQError( "bool Virtual_Machine::Load_VM( "
						 "const QString &file_name )",
						 "SPICE GXL_Devices_Count "
						 "invalid!" );

				SPICE.Set_RAM_Size(
					Second_Element
						.firstChildElement( "RAM_Size" )
						.text()
						.toInt( &ok ) );
				if ( !ok )
					AQError( "bool Virtual_Machine::Load_VM( "
				...
				SPICE.Use_SPort(
					Second_Element
						.firstChildElement( "Use_SPort" )
						.text()
					== "true" );

				SPICE.Set_SPort(
					Second_Element.firstChildElement( "SPort" )
						.text()
						.toUInt( &ok ) );
				if ( !ok )
					AQError( "bool Virtual_Machine::Load_VM( "
				...
				SPICE.Use_Image_Compression( Second_Element
								     .firstChildElement( "Use_Image_Compression" )
								     .text()
							     == "true" );
				...
				if ( spiceIC == "on" )
					...
					AQError( "bool Virtual_Machine::Load_VM( "
						 "const QString &file_name )",
						 "SPICE image compression type "
						 "invalid!" );

				SPICE.Use_Video_Stream_Compression( Second_Element
									    .firstChildElement( "Use_Video_Stream_Compression" )
									    .text()
								    == "true" );

				SPICE.Use_Renderer(
					Second_Element
						.firstChildElement( "Use_Renderer" )
						.text()
					== "true" );

				QDomElement rendererListElement =
					Second_Element.firstChildElement(
						"Renderers_List" );
				...
				else
				{
					...
					QList<QDomElement>	  renderNodes =
						rendererListElement.childNodes();
					...
					if ( rendererList.isEmpty() )
						AQError( "bool "
							 ...
							 " const QString "
							 "&file_name )",
							 "SPICE renderers list is "
							 "empty!" );
					else
						SPICE.Set_Renderer_List( rendererList );
				}

				SPICE.Use_Playback_Compression( Second_Element
									.firstChildElement( "Use_Playback_Compression" )
									.text()
								== "true" );

				SPICE.Use_Password(
					Second_Element
						.firstChildElement( "Use_Password" )
						.text()
					== "true" );
			...
			// Use VNC
			VNC = ( Child_Element.firstChildElement( "Use_VNC" ).text()
				== "true" );

			// VNC Socket Mode
			VNC_Socket_Mode =
				( Child_Element
					  .firstChildElement( "VNC_Socket_Mode" )
					  .text()
				  == "true" );
			...
			VNC_Display_Number =
				(uint)Child_Element
					.firstChildElement( "VNC_Display_Number" )
					.text()
					.toInt();
			...
			VNC_Password =
				( Child_Element
					  .firstChildElement( "Use_VNC_Password" )
					  .text()
				  == "true" );

			// Use VNC TLS
			VNC_TLS = ( Child_Element.firstChildElement( "Use_VNC_TLS" ).text()
				    == "true" );

			// Use VNC x509
			VNC_x509 =
				( Child_Element.firstChildElement( "Use_VNC_x509" ).text()
				  == "true" );

			// VNC x509 Folder Path
			VNC_x509_Folder_Path =
				Child_Element
					.firstChildElement( "VNC_x509_Folder_Path" )
					.text();

			// Use VNC x509verify
			VNC_x509verify =
				( Child_Element
					  .firstChildElement( "Use_VNC_x509verify" )
					  .text()
				  == "true" );
			...
			Only_User_Args =
				( Child_Element
					  .firstChildElement( "Only_User_Args" )
					  .text()
				  == "true" );
...
QStringList Virtual_Machine::Build_QEMU_Args()
{
	...
	if ( Settings.value( "Emulator_Monitor_Type", "stdio" ).toString() == "tcp" )
	{
		Args << "-monitor"
		     << QString( "tcp:%1:%2,server,nowait" )
				.arg( Settings.value( "Emulator_Monitor_Hostname", "localhost" )
					      .toString() )
				.arg( Settings.value( "Emulator_MonGitor_Port", 6000 )
					      .toInt()
				      + Embedded_Display_Port );

		Monitor_Hostname =
			Settings.value( "Emulator_Monitor_Hostname", "localhost" ).toString();
		Monitor_Port = (unsigned int)Settings
				       .value( "Emulator_MonGitor_Port", 6000 )
				       .toInt()
			       + Embedded_Display_Port;
	}
	else { Args << "-monitor" << "stdio"; }
#endif
	...
	if ( Current_Emulator_Devices.System.QEMU_Name.isEmpty() )
	{
		...
		if ( Current_Emulator_Devices.System.QEMU_Name.isEmpty() )
		{
			AQError( "QStringList Virtual_Machine::Build_QEMU_Args()", "Current_Emulator_Devices is empty! AQEMU Break :(" );
			return Args;
		}
	}
		     ...
		     || Current_Emulator_Devices.PSO_SMP_MaxCPUs )
		{
			     ...
			     || SMP.SMP_Sockets > 0 || SMP.SMP_MaxCPUs > 0 )
			{
				...
				if ( Current_Emulator_Devices.PSO_SMP_Cores
				     && SMP.SMP_Cores > 0 )
					smp_args += ",cores="
						    + QString::number( SMP.SMP_Cores );

				if ( Current_Emulator_Devices.PSO_SMP_Threads
				     && SMP.SMP_Threads > 0 )
					smp_args += ",threads="
						    + QString::number( SMP.SMP_Threads );

				if ( Current_Emulator_Devices.PSO_SMP_Sockets
				     && SMP.SMP_Sockets > 0 )
					smp_args += ",sockets="
						    + QString::number( SMP.SMP_Sockets );

				if ( Current_Emulator_Devices.PSO_SMP_MaxCPUs
				     && SMP.SMP_MaxCPUs > 0 )
					smp_args += ",maxcpus="
						    + QString::number( SMP.SMP_MaxCPUs );
	...
	if ( Current_Emulator_Devices.PSO_KVM_Shadow_Memory && KVM_Shadow_Memory )
		props << "kvm_shadow_mem="
				 + QString::number( KVM_Shadow_Memory_Size * 1024 );
	...
	if ( FD0.Get_Enabled() )
	{
		if ( FD0.Get_Native_Mode() )
		{
			...
			VM::Device_Interface iftype =
				FD0.Get_Native_Device().Get_Interface();
			if ( iftype == VM::DI_Virtio_SCSI )
			{
				has_virt_scsi = true;
			}
			StorageArgs << Build_Native_Device_Args(
				FD0.Get_Native_Device(),
		...
		else
		{
			...
			{
				AQError( "QStringList "
					 "Virtual_Machine::Build_QEMU_Args()",
					 QString( "Image \"%1\" doesn't exists!" )
	...
	if ( FD1.Get_Enabled() )
	{
		if ( FD1.Get_Native_Mode() )
		{
			...
			VM::Device_Interface iftype =
				FD1.Get_Native_Device().Get_Interface();
			if ( iftype == VM::DI_Virtio_SCSI )
			{
				has_virt_scsi = true;
			}
	...
	if ( CD_ROM.Get_Enabled() )
	{
		if ( CD_ROM.Get_Native_Mode() )
		{
			// Testing for the interface type 'virtio-scsi'
			VM::Device_Interface iftype =
				CD_ROM.Get_Native_Device().Get_Interface();
			if ( iftype == VM::DI_Virtio_SCSI )
			{
				has_virt_scsi = true;
			}
	...
	if ( HDA.Get_Enabled() )
	{
		if ( HDA.Get_Native_Mode() )
		{
			// Testing for the interface type 'virtio-scsi'
			VM::Device_Interface iftype =
				HDA.Get_Native_Device().Get_Interface();
			if ( iftype == VM::DI_Virtio_SCSI )
			{
				has_virt_scsi = true;
			}
	...
	if ( HDB.Get_Enabled() )
	{
		if ( HDB.Get_Native_Mode() )
		{
			// Testing for the interface type 'virtio-scsi'
			VM::Device_Interface iftype =
				HDB.Get_Native_Device().Get_Interface();
			if ( iftype == VM::DI_Virtio_SCSI )
			{
				has_virt_scsi = true;
			}
	...
	if ( HDC.Get_Enabled() )
	{
		if ( HDC.Get_Native_Mode() )
		{
			// Testing for the interface type 'virtio-scsi'
			VM::Device_Interface iftype =
				HDC.Get_Native_Device().Get_Interface();
			if ( iftype == VM::DI_Virtio_SCSI )
			{
				has_virt_scsi = true;
			}
	...
	if ( HDD.Get_Enabled() )
	{
		if ( HDD.Get_Native_Mode() )
		{
			// Testing for the interface type 'virtio-scsi'
			VM::Device_Interface iftype =
				HDD.Get_Native_Device().Get_Interface();
			if ( iftype == VM::DI_Virtio_SCSI )
			{
				has_virt_scsi = true;
			}
	...
	if ( Shared_Folders.count() > 0 )
	{
		...
		{
			Args << Build_Shared_Folder_Args( Shared_Folders[ix],
							  ix,
							  Build_QEMU_Args_for_Tab_Info );
		}
	}
	...
	//
	if ( has_virt_scsi ) { Args << "-device" << "virtio-scsi-pci,id=aq-vscsi"; }
	...
	if ( Current_Emulator_Devices.PSO_Boot_Order )
	{
		...
		if ( bootDevCount > 1 )
		{
			...
			for ( int ix = 0; ix < Boot_Order_List.count(); ix++ )
			{
				if ( Boot_Order_List[ix].Enabled )
				{
					if ( Boot_Order_List[ix].Type == VM::Boot_From_FDA )
						...
						bootStr += "d";
					else if ( Boot_Order_List[ix].Type
			...
			bootStr += QString( bootStr.isEmpty() ? "" : "," ) + "menu="
				   + QString( Show_Boot_Menu ? "on" : "off" );
	...
	else
	{
		if ( Use_Native_Network() )
		{
			...
			{
				QString nic_str = "";
				bool u_vlan, u_macaddr, u_model, u_name, u_hostname,
					u_port_dev, u_fd, u_ifname, u_script,
					u_downscript, u_bridge, u_helper, u_listen,
					u_connect, u_mcast, u_sock, u_port, u_group,
					u_mode, u_file, u_len, u_addr, u_vectors,
					u_net, u_host, u_restrict, u_dhcpstart,
					u_dns, u_tftp, u_bootfile, u_hostfwd,
					u_guestfwd, u_smb, u_smbserver, u_sndbuf,
					u_vnet_hdr, u_vhost, u_vhostfd;

				u_vlan = u_macaddr = u_model = u_name = u_hostname = u_port_dev = u_fd = u_ifname =
					u_script = u_downscript = u_bridge = u_helper = u_listen = u_connect =
						u_mcast = u_sock = u_port = u_group = u_mode = u_file =
							u_len = u_addr = u_vectors = u_net = u_host = u_restrict =
								u_dhcpstart = u_dns = u_tftp = u_bootfile = u_hostfwd =
									u_guestfwd = u_smb = u_smbserver =
										u_sndbuf = u_vnet_hdr =
											u_vhost = u_vhostfd =
												false;
				...
				switch ( Network_Cards_Nativ[nc].Get_Network_Type() )
				{
					...
					case VM::Net_Mode_Native_User:
						nic_str += "user";
						u_vlan = u_name = u_net = u_host = u_restrict =
							u_hostname = u_dhcpstart = u_dns =
								u_tftp = u_bootfile = u_hostfwd =
									u_guestfwd = u_smb = u_smbserver =
										true;
						break;
					...
					case VM::Net_Mode_Native_TAP:
						nic_str += "tap";
						u_vlan = u_name = u_fd = u_ifname = u_script =
							u_downscript = u_helper = u_sndbuf =
								u_vnet_hdr = u_vhost =
									u_vhostfd = true;
						break;
				...
				if ( Network_Cards_Nativ[nc].Get_Card_Model().isEmpty() == false
				     && u_model )
					nic_str += ",model="
						   + Network_Cards_Nativ[nc].Get_Card_Model();

				if ( Network_Cards_Nativ[nc].Use_Name() && u_name
				     && Current_Emulator_Devices.PSO_Net_name )
					nic_str += ",name=\""
						   + Network_Cards_Nativ[nc].Get_Name()
						   + "\"";

				if ( Network_Cards_Nativ[nc].Use_Hostname() && u_hostname )
					nic_str += ",hostname="
						   + Network_Cards_Nativ[nc].Get_Hostname();

				if ( u_port_dev )
					nic_str += ",port:"
						   + Network_Cards_Nativ[nc].Get_PortDev();

				if ( Network_Cards_Nativ[nc].Use_File_Descriptor() && u_fd )
					nic_str += ",fd="
						   + QString::number(
							   Network_Cards_Nativ[nc].Get_File_Descriptor() );

				if ( Network_Cards_Nativ[nc].Use_Interface_Name() && u_ifname
				     && Current_Emulator_Devices.PSO_Net_ifname )
					nic_str += ",ifname="
						   + Network_Cards_Nativ[nc].Get_Interface_Name();

				if ( Network_Cards_Nativ[nc].Use_Bridge_Name() && u_bridge
				     && Current_Emulator_Devices.PSO_Net_bridge )
					nic_str += ",br="
						   + Network_Cards_Nativ[nc].Get_Bridge_Name();
				...
				if ( u_downscript && Current_Emulator_Devices.PSO_Net_downscript )
				{
					...
					if ( Network_Cards_Nativ[nc].Use_TUN_TAP_Down_Script() )
						s_downscript =
							Network_Cards_Nativ[nc].Get_TUN_TAP_Down_Script();
					else
						s_downscript = "no";
				     ...
				     && Current_Emulator_Devices.PSO_Net_file )
				{
					if ( Build_QEMU_Args_for_Script_Mode )
						nic_str += ",file=\""
							   + Network_Cards_Nativ[nc].Get_File()
							   + "\"";
					else
						nic_str += ",file="
							   + Network_Cards_Nativ[nc].Get_File();
				...
				if ( Network_Cards_Nativ[nc].Use_Len() && u_len
					...
					nic_str += ",len="
						   + QString::number(
							   Network_Cards_Nativ[nc].Get_Len() );

				if ( Network_Cards_Nativ[nc].Use_Addr() && u_addr
				     && Current_Emulator_Devices.PSO_Net_addr )
					nic_str += ",addr="
						   + Network_Cards_Nativ[nc].Get_Addr();

				if ( Network_Cards_Nativ[nc].Use_Vectors() && u_vectors
				     && Current_Emulator_Devices.PSO_Net_vectors )
					nic_str += ",vectors="
						   + QString::number(
							   Network_Cards_Nativ[nc].Get_Vectors() );

				if ( Network_Cards_Nativ[nc].Use_Net() && u_net
				     && Current_Emulator_Devices.PSO_Net_net )
					nic_str += ",net="
						   + Network_Cards_Nativ[nc].Get_Net();

				if ( Network_Cards_Nativ[nc].Use_Host() && u_host
				     && Current_Emulator_Devices.PSO_Net_host )
					nic_str += ",host="
						   + Network_Cards_Nativ[nc].Get_Host();

				if ( Network_Cards_Nativ[nc].Use_Restrict() && u_restrict
				     && Current_Emulator_Devices.PSO_Net_restrict )
					nic_str +=
						",restrict="
								+ Network_Cards_Nativ[nc]
									  .Get_Restrict()
							? "y"
							: "n";

				if ( Network_Cards_Nativ[nc].Use_DHCPstart() && u_dhcpstart
				     && Current_Emulator_Devices.PSO_Net_dhcpstart )
					nic_str += ",dhcpstart="
						   + Network_Cards_Nativ[nc].Get_DHCPstart();

				if ( Network_Cards_Nativ[nc].Use_DNS() && u_dns
				     && Current_Emulator_Devices.PSO_Net_dns )
					nic_str += ",dns="
						   + Network_Cards_Nativ[nc].Get_DNS();

				if ( Network_Cards_Nativ[nc].Use_Tftp() && u_tftp
				     && Current_Emulator_Devices.PSO_Net_tftp )
				{
					if ( Build_QEMU_Args_for_Script_Mode )
						nic_str += ",tftp=\""
							   + Network_Cards_Nativ[nc].Get_Tftp()
							   + "\"";
					else
						nic_str += ",tftp="
							   + Network_Cards_Nativ[nc].Get_Tftp();
				...
				if ( Network_Cards_Nativ[nc].Use_Bootfile() && u_bootfile
				     && Current_Emulator_Devices.PSO_Net_bootfile )
				{
					if ( Build_QEMU_Args_for_Script_Mode )
						nic_str += ",bootfile=\""
							   + Network_Cards_Nativ[nc].Get_Bootfile()
							   + "\"";
					else
						nic_str += ",bootfile="
							   + Network_Cards_Nativ[nc].Get_Bootfile();
				...
				if ( Network_Cards_Nativ[nc].Use_HostFwd() && u_hostfwd
				     && Current_Emulator_Devices.PSO_Net_hostfwd )
					nic_str += ",hostfwd="
						   + Network_Cards_Nativ[nc].Get_HostFwd();

				if ( Network_Cards_Nativ[nc].Use_GuestFwd() && u_guestfwd
				     && Current_Emulator_Devices.PSO_Net_guestfwd )
					nic_str += ",guestfwd="
						   + Network_Cards_Nativ[nc].Get_GuestFwd();

				if ( Network_Cards_Nativ[nc].Use_SMB() && u_smb
				     && Current_Emulator_Devices.PSO_Net_smb )
				{
					if ( Build_QEMU_Args_for_Script_Mode )
						nic_str += ",smb=\""
							   + Network_Cards_Nativ[nc].Get_SMB()
							   + "\"";
					else
						nic_str += ",smb="
							   + Network_Cards_Nativ[nc].Get_SMB();

					if ( Network_Cards_Nativ[nc].Use_SMBserver()
					     && u_smbserver )
						nic_str += ",smbserver="
							   + Network_Cards_Nativ[nc].Get_SMBserver();
				}

				if ( Network_Cards_Nativ[nc].Use_Sndbuf() && u_sndbuf
				     && Current_Emulator_Devices.PSO_Net_sndbuf )
					nic_str += ",sndbuf="
						   + QString::number(
							   Network_Cards_Nativ[nc].Get_Sndbuf() );

				if ( Network_Cards_Nativ[nc].Use_VNet_hdr() && u_vnet_hdr
				...
				if ( Network_Cards_Nativ[nc].Get_VHost() && u_vhost
				...
				if ( Network_Cards_Nativ[nc].Use_VHostFd() && u_vhostfd
				     && Current_Emulator_Devices.PSO_Net_vhostfd )
					nic_str += ",vhostfd="
						   + QString::number(
							   Network_Cards_Nativ[nc].Get_VHostFd() );
		...
		else
		{
			for ( int nc = 0; nc < Network_Cards.count(); nc++ )
			{
				...
				if ( !Network_Cards[nc].Get_Card_Model().isEmpty() )
					nic_str += ",model="
						   + Network_Cards[nc].Get_Card_Model();
		...
		if ( Use_Redirections )
		{
			...
			for ( int rx = 0; rx < Get_Network_Redirections_Count(); rx++ )
			{
				...
				redir_str +=
					":"
					+ QString::number(
						Get_Network_Redirection( rx ).Get_Host_Port() )
					+ "-";
				redir_str +=
					Get_Network_Redirection( rx ).Get_Guest_IP() + ":";
				redir_str += QString::number(
					Get_Network_Redirection( rx ).Get_Guest_Port() );
	...
	for ( int ix = 0; ix < Serial_Ports.count(); ix++ )
	{
		...
		switch ( Serial_Ports[ix].Get_Port_Redirection() )
		{
			...
			case VM::PR_pty: Args << "pty"; break;

			case VM::PR_none: Args << "none"; break;

			case VM::PR_null: Args << "null"; break;
			...
			case VM::PR_stdio: Args << "stdio"; break;
			...
			case VM::PR_msmouse: Args << "msmouse"; break;

			case VM::PR_mon:
				Args << "mon:" << Serial_Ports[ix].Get_Parametrs_Line();
				break;

			case VM::PR_braille: Args << "braille"; break;
			...
			for ( int ux = 0; ux < USB_Ports.count(); ux++ )
			{
				if ( USB_Ports[ux].Get_Use_Host_Device() )
				{
					...
					if ( Current_Emulator_Devices.PSO_Device == false
					     || Settings.value( "USB_Style", "device" )
								.toString()
							== "usbdevice" )
					{
						if ( Settings.value( "USB_ID_Style", "" )
							     .toString()
						     == "BusAddr" )
							Args << "-usbdevice"
							     << QString( "host:%1.%"
									 "2" )
									.arg( current_USB_Device
										      .Get_Bus() )
									.arg( current_USB_Device
										      .Get_Addr() );
						else if ( Settings.value( "USB_ID_"
									  "Style",
									  "" )
								  .toString()
							  == "VendorProduct" )
				...
				else
				{
					...
					bool usb_k, usb_m, usb_t, usb_wt, usb_b;
					usb_k = usb_m = usb_t = usb_wt = usb_b = false;
					USB_Ports[ux].Get_USB_QEMU_Devices( usb_k,
									    usb_m,
									    usb_t,
									    usb_wt,
									    usb_b );
					...
					if ( usb_k )
						...
						Args << "-usbdevice" << "wacom-tablet";
					else if ( usb_b )
						Args << "-usbdevice" << "braille";
					else
					{
						AQError( "QStringList "
							 "Virtual_Machine::Build_"
							 "QEMU_Args()",
	...
	if ( SPICE.Use_SPICE() )
	{
		...
		if ( SPICE.Use_Renderer() )
		{
			...
			else { spiceArgs << "renderer=" + spiceRendersStr; }
		}
		...
		else { spiceArgs << "disable-ticketing"; }
	...
	if ( VNC )
	{
		...
		if ( VNC_Password ) vnc_args += ",password";
	...
	else
	{
		...
		if ( Settings.value( "Use_VNC_Display", "no" ).toString() == "yes" )
		{
			...
			{
				int port = ( Settings.value( "First_VNC_Port", "5910" )
						     .toString()
						     .toInt()
					     - 5900 )
					   + Embedded_Display_Port;
				...
				if ( port >= 1 && port < 59636 )
					...
					AQError( "QStringList "
						 "Virtual_Machine::Build_QEMU_Args("
						 ")",
						 "Port for Embedded Display > 1 or "
						 "< 59636" );
			}
			else
			{
						       ...
						       bool Build_QEMU_Args_for_Script_Mode )
{
	...
	if ( device.Use_Interface() )
	{
		switch ( device.Get_Interface() )
		{
			case VM::DI_IDE: opt << "if=ide"; break;

			case VM::DI_SCSI: opt << "if=scsi"; break;

			case VM::DI_SD: opt << "if=sd"; break;

			case VM::DI_MTD: opt << "if=mtd"; break;

			case VM::DI_Floppy: opt << "if=floppy"; break;

			case VM::DI_PFlash: opt << "if=pflash"; break;

			case VM::DI_Virtio: opt << "if=virtio"; break;
			...
			default:
				AQError( "QStringList "
					 "Virtual_Machine::Build_Native_Device_"
					 "Args( VM_Native_Storage_Device device, "
					 "bool Build_QEMU_Args_for_Script_Mode )",
					 "Storage Device Interface Default "
					 "Section!" );
				break;
		}
	}
	...
	if ( device.Use_Media() )
	{
		switch ( device.Get_Media() )
		{
			case VM::DM_Disk: opt << "media=disk"; break;

			case VM::DM_CD_ROM: opt << "media=cdrom"; break;
	...
	// Cache
	if ( device.Use_Cache() ) opt << "cache=" + device.Get_Cache();

	// AIO
	if ( device.Use_AIO() ) opt << "aio=" + device.Get_AIO();
...
QStringList Virtual_Machine::Build_Shared_Folder_Args( VM_Shared_Folder folder,
						       int		id,
						       bool Build_QEMU_Args_for_Script_Mode )
{
	QStringList opt;
...
bool Virtual_Machine::Start_impl()
{
	     ...
	     && Settings.value( "Disable_KVM_Module_Check", "no" ).toString() != "yes" )
	{
		...
		else
		{
			...
			else
			{
				...
				if ( !kvm_ok )
				{
					...
					else
					{
						...
						else
						{
							...
							kvm_intel_conf.setPatternSyntax(
								QRegExp::Wildcard );
							if ( kvm_intel_conf.exactMatch(
								     all_config ) )
								kvm_ok = true;
							else
							{
								QRegExp kvm_amd_conf = QRegExp( "*CONFIG_KVM_AMD=y*" );
								kvm_amd_conf.setPatternSyntax(
									QRegExp::Wildcard );
								if ( kvm_amd_conf.exactMatch(
									     all_config ) )
									kvm_ok = true;
							}
						}
				...
				if ( !kvm_ok )
				{
					int retVal = QMessageBox::question( NULL,
									    tr(
										    "Error!" ),
									    tr(
	...
	if ( Settings.value( "QEMU_AUDIO/Use_Default_Driver", "yes" ).toString() == "no" )
	{
		...
		tmp_env << "QEMU_AUDIO_DRV="
				   + Settings.value( "QEMU_AUDIO/QEMU_AUDIO_DRV", "alsa" )
					     .toString();
	...
	else
	{
		...
		if ( bin_path.isEmpty() )
		{
			AQGraphic_Error( "bool Virtual_Machine::Start()", tr( "Error!" ), tr( "Cannot start emulator! Binary path is empty!" ), false );
			Start_Snapshot_Tag = "";
			return false;
		}
	...
	{
		connect( this,
			 SIGNAL( Ready_StdOut( const QString & ) ),
			 this,
...
void Virtual_Machine::Reset()
{
	...
	else { Send_Emulator_Command( "system_reset\n" ); }
}
...
{
	if ( !QEMU_Error_Win ) QEMU_Error_Win = new Error_Log_Window();
...
{
	if ( err_str.simplified().isEmpty() ) return;
	...
	{
		AQDebug( "void Virtual_Machine::Show_QEMU_Error( const QString "
			 "&err_str )",
			 "Error Log Window Not Shown" );
		return;
	}
	else { Show_Error_Log_Window(); }
}

void Virtual_Machine::Hide_QEMU_Error_Log()
{
	if ( QEMU_Error_Win ) { QEMU_Error_Win->close(); }
}
...
const Emulator &Virtual_Machine::Get_Emulator() const { return Current_Emulator; }
...
void Virtual_Machine::Update_Current_Emulator_Devices()
{
	...
	if ( Current_Emulator_Devices.System.QEMU_Name.isEmpty() )
	{
		AQError( "void Update_Current_Emulator_Devices()", "Cannot Load Info About This Emulator! AQEMU Doesn't Work!" );
	}
}
...
QString Virtual_Machine::Get_Current_Emulator_Binary_Path( const QString &names ) const
{
	...
	{
		AQError( "QString "
			 "Virtual_Machine::Get_Current_Emulator_Binary_Path( const "
			 "QString &names ) const",
...
QString Virtual_Machine::Get_State_Text() const
{
	switch ( State )
	{
		case VM::VMS_Running: return tr( "Running" );

		case VM::VMS_Power_Off: return tr( "Power Off" );

		case VM::VMS_Pause: return tr( "Pause" );

		case VM::VMS_Saved: return tr( "Saved" );
...
const QString &Virtual_Machine::Get_Machine_Name() const { return Machine_Name; }

void Virtual_Machine::Set_Machine_Name( const QString &name )
{
	if ( name.isEmpty() )
		AQError( "void Virtual_Machine::Set_Machine_Name( const QString "
			 "&name )",
			 "File name is empty!" );
	else
		Machine_Name = name;
...
{
	if ( file_name.isEmpty() )
		AQError( "void Virtual_Machine::Set_Icon_Path( const QString "
			 "&file_name )",
			 "File name is empty!" );
	else
		Icon_Path = file_name;
...
const QString &Virtual_Machine::Get_Computer_Type() const { return Computer_Type; }
...
const QString &Virtual_Machine::Get_Machine_Type() const { return Machine_Type; }
...
void Virtual_Machine::Set_Audio_Cards( VM::Sound_Cards card ) { Audio_Card = card; }
...
const QString &Virtual_Machine::Get_SMB_Directory() const { return SMB_Directory; }
...
const VM_Net_Card &Virtual_Machine::Get_Network_Card( int index ) const
{
	...
	{
		AQError( "VM_Net_Card *Virtual_Machine::Get_Network_Card( int "
			 "index ) const",
			 "Invalid Index!" );
...
const VM_Redirection &Virtual_Machine::Get_Network_Redirection( int index ) const
{
	...
	{
		AQError(
			"VM_Redirection *Virtual_Machine::Get_Network_Redirection( "
			"int index ) const",
			"Invalid Index!" );
...
void Virtual_Machine::Set_Network_Redirection( int index, const VM_Redirection &r )
{
	...
	{
		AQError( "void Virtual_Machine::Set_Network_Redirection( int "
			 "index, VM_Redirection *r )",
			 "Invalid Index!" );
	}
}
...
const QList<VM_USB> &Virtual_Machine::Get_USB_Ports() const { return USB_Ports; }
...
const QString &Virtual_Machine::Get_bzImage_Path() const { return bzImage_Path; }
...
void Virtual_Machine::Set_Initrd_Path( const QString &path ) { Initrd_Path = path; }
...
const QString &Virtual_Machine::Get_MTDBlock_File() const { return MTDBlock_File; }
...
void Virtual_Machine::Set_PFlash_File( const QString &file ) { PFlash_File = file; }
...
bool Virtual_Machine::Use_KVM_Shadow_Memory() const { return KVM_Shadow_Memory; }

void Virtual_Machine::Use_KVM_Shadow_Memory( bool use ) { KVM_Shadow_Memory = use; }
...
const QDateTime &Virtual_Machine::Get_Start_Date() const { return Start_DateTime; }

void Virtual_Machine::Set_Start_Date( const QDateTime &dt ) { Start_DateTime = dt; }
...
int Virtual_Machine::Get_VNC_Display_Number() const { return VNC_Display_Number; }
...
void Virtual_Machine::Parse_StdOut()
{
	...
	if ( re.match( convOutput ).hasMatch() ) { Show_QEMU_Error( convOutput ); }
	...
	if ( Update_Removable_Devices_Mode )
	{
		...
		else { Removable_Devices_List += cleanOutput; }
	}
}
...
void Virtual_Machine::Send_Emulator_Command( const QString &text )
{
...
#endif
	{
		...
		{
			AQError( "void Virtual_Machine::Send_Emulator_Command( "
				 "const QString &text )",
				 "Monitor socket not connected!" );
		}
		else { Monitor_Socket->write( qPrintable( text ) ); }
	}
}
...
void Virtual_Machine::QEMU_Started()
{
	...
	if ( Start_CPU ) { Set_State( VM::VMS_Running ); }
...
#endif
	{
		Monitor_Socket->connectToHost( Monitor_Hostname,
					       Monitor_Port,
					       QIODevice::ReadWrite );
	}
...
{
	AQDebug( "void Virtual_Machine::Resume_Finished( const QString "
		 "&returned_text )",
...
void Virtual_Machine::Suspend_Finished( const QString &returned_text )
{
	...
	if ( returned_text.simplified() == "(qemu)" )
	{
		...
		if ( !Save_VM() )
			AQError( "void Virtual_Machine::Save_VM_State( const "
				 "QString &tag )",
				 "Saving Not Complete!" );
	}
	else if ( returned_text.contains( "No block device can accept snapshots" ) )
	{
		...
		Hide_VM_Save_Window();
		AQGraphic_Warning( tr( "Error!" ), tr( "You must add HDD image in QCOW2 format to save virtual machine!" ) );
	}
}
...
void Virtual_Machine::Started_Booting( const QString &text )
{
	if ( text.contains( "VNC" ) || text.contains( "Server" ) )
	{
		...
		AQDebug( "void Virtual_Machine::Started_Booting( const QString "
			 "&text )",
			 "emit Loading_Complete()" );
		emit Loading_Complete();
		Hide_VM_Load_Window();
...
void Virtual_Machine::Execute_Emu_Ctl_Command( const QString &com )
{
	...
	AQDebug( "void Virtual_Machine::Execute_Emu_Ctl_Command( const QString "
		 "&com )",
		 "Run: " + com );
}
...
#define INFO_TEXT_INDENT 25
	...
	if ( info_mode != 0 && info_mode < 3 )
	{
		...
		if ( info_mode == 1 )
			cursor.insertText(
				tr( "You must create a new virtual machine" ),
				bold_format );
		else if ( info_mode == 2 )
			cursor.insertText(
				tr( "This VM uses the emulator \"%1\" that is not "
				    "installed.\n"
				    "The VM cannot work!" )
					.arg( ( Get_Machine_Accelerator() == VM::TCG )
						      ? "TCG"
						      : "KVM" ),
				bold_format );
	...
	if ( Settings.value( "Info/Show_Screenshot_in_Save_Mode", "no" ).toString() == "yes" )
	{
		...
		{
			AQDebug( "void Main_Window::Update_Info_Text( int "
				 "info_mode )",
	     ...
	     || soph.preview )
	{
			     ...
			     == "yes" ) )
		{
			...
			cell_cursor.insertText(
				VM::Accel_To_String( Get_Machine_Accelerator() ).toUpper(),
				format );
			table->insertRows( table->rows(), 1 );
		}
				  ...
				  == "yes" ) )
		{
			...
			if ( Get_Computer_Type().isEmpty() ) name = tr( "Default" );
				  ...
				  == "yes" ) )
		{
			...
			QString name;
			for ( int i = 0;
			      i < Get_Current_Emulator_Devices()->Machine_List.count();
			      i++ )
			{
				if ( Get_Current_Emulator_Devices()
						     ->Machine_List.at( i )
						     .QEMU_Name
					     == Get_Machine_Type()
				     && !Get_Current_Emulator_Devices()
						 ...
						 .Caption.isEmpty() )
				{
					name = Get_Current_Emulator_Devices()
						       ->Machine_List.at( i )
						       .Caption;
				}
			}
			if ( Get_Machine_Type().isEmpty() ) name = tr( "Default" );
				  ...
				  == "yes" ) )
		{
			...
			cell_cursor = cell.firstCursorPosition();
			cell_cursor.insertText(
				VM::Boot_Order_To_String_List( Get_Boot_Order_List() )
					.join( "/" ),
				format );
			table->insertRows( table->rows(), 1 );
		}
		...
		if ( soph.either( Settings.value( "Info/CPU_Type", "no" ).toString() == "yes" ) )
		{
			...
			QString name;
			for ( int i = 0;
			      i < Get_Current_Emulator_Devices()->CPU_List.count();
			      i++ )
			{
				if ( Get_Current_Emulator_Devices()
						     ->CPU_List.at( i )
			...
			if ( Get_CPU_Type().isEmpty() ) name = tr( "Default" );
				  ...
				  == "yes" ) )
		{
			...
			cell	    = table->cellAt( table->rows() - 1, 2 );
			cell_cursor = cell.firstCursorPosition();
			cell_cursor.insertText( QString::number( Get_SMP_CPU_Count() ),
						format );
			table->insertRows( table->rows(), 1 );
		}
				  ...
				  == "yes" ) )
		{
			...
			QString name;
			for ( int i = 0;
			      i < Get_Current_Emulator_Devices()->Video_Card_List.count();
			      i++ )
			{
				if ( Get_Current_Emulator_Devices()
			...
			if ( Get_Video_Card().isEmpty() ) name = tr( "Default" );
				  ...
				  == "yes" ) )
		{
			...
			cell	    = table->cellAt( table->rows() - 1, 2 );
			cell_cursor = cell.firstCursorPosition();
			cell_cursor.insertText( QString::number( Get_Memory_Size() ),
						format );
			table->insertRows( table->rows(), 1 );
		}
				  ...
				  == "yes" ) )
		{
			...
			else { cell_cursor.insertText( tr( "No" ), format ); }
				   ...
				   == "yes" ) )
		{
			...
			cell_cursor = cell.firstCursorPosition();
			cell_cursor.insertText(
				Use_Fullscreen_Mode() ? tr( "Yes" ) : tr( "No" ),
				format );
			table->insertRows( table->rows(), 1 );
		}
				   ...
				   == "yes" ) )
		{
			...
			cell_cursor = cell.firstCursorPosition();
			cell_cursor.insertText(
				Use_Snapshot_Mode() ? tr( "Yes" ) : tr( "No" ),
				format );
			table->insertRows( table->rows(), 1 );
		}
				   ...
				   == "yes" ) )
		{
			...
			cell	    = table->cellAt( table->rows() - 1, 2 );
			cell_cursor = cell.firstCursorPosition();
			cell_cursor.insertText( Use_Local_Time() ? tr( "Yes" ) : tr( "No" ),
						format );
			table->insertRows( table->rows(), 1 );
		}
	}
	...
	if ( soph.either( Settings.value( "Info/Network_Cards", "yes" ).toString() == "yes" )
	     && ( ( Use_Native_Network() == false && Get_Network_Cards_Count() > 0 )
		  || ( Use_Native_Network() == true
		       && Get_Network_Cards_Nativ().count() > 0 ) ) )
	{
		cursor.setPosition( topFrame->lastPosition() );
	...
	if ( soph.setting( Settings.value( "Info/Redirections", "no" ).toString() == "yes" ) )
	{
		if ( Get_Use_Redirections() || Get_Network_Redirections_Count() < 1 )
		{
			for ( int rx = 0; rx < Get_Network_Redirections_Count(); ++rx )
			{
				...
				cell_cursor = cell.firstCursorPosition();
				cell_cursor.insertText(
					tr( "Redirection " ) + QString::number( rx + 1 ) + ":",
					format );
		  ...
		  && Get_USB_Ports().count() > 0 ) )
	{
		...
		if ( Settings.value( "Info/Serial_Port", "yes" ).toString() == "yes" )
		{
			for ( int ix = 0; ix < Get_Serial_Ports().count(); ix++ )
			{
				cell	    = table->cellAt( table->rows() - 1, 1 );
				cell_cursor = cell.firstCursorPosition();
				cell_cursor.insertText( tr( "Serial Port %1:" ).arg( ix ),
							format );
		...
		if ( Settings.value( "Info/Parallel_Port", "yes" ).toString() == "yes" )
		{
			for ( int ix = 0; ix < Get_Parallel_Ports().count(); ix++ )
			{
				cell	    = table->cellAt( table->rows() - 1, 1 );
				cell_cursor = cell.firstCursorPosition();
				cell_cursor.insertText( tr( "Parallel Port %1:" ).arg( ix ),
							format );
		...
		if ( Settings.value( "Info/USB_Port", "yes" ).toString() == "yes" )
		{
			for ( int ix = 0; ix < Get_USB_Ports().count(); ix++ )
			{
				cell	    = table->cellAt( table->rows() - 1, 1 );
				cell_cursor = cell.firstCursorPosition();
				cell_cursor.insertText( tr( "USB Port %1:" ).arg( ix ),
							format );

				cell	    = table->cellAt( table->rows() - 1, 2 );
				cell_cursor = cell.firstCursorPosition();
				cell_cursor.insertText(
					Get_USB_Ports()[ix].Get_Manufacturer_Name() + " "
	     ...
	     && !soph.preview )
	{
		...
		if ( Settings.value( "Info/VNC", "no" ).toString() == "yes" )
		{
			...
			if ( Use_VNC() )
				cell_cursor.insertText(
					QString::number( Get_VNC_Display_Number() + 5900 ),
					format );
			else
				cell_cursor.insertText(
					QString::number(
						Settings.value( "First_VNC_Port", "5910" )
							.toInt()
		...
		if ( Settings.value( "Info/SPICE", "no" ).toString() == "yes" )
		{
			...
			cell	    = table->cellAt( table->rows() - 1, 2 );
			cell_cursor = cell.firstCursorPosition();
			cell_cursor.insertText( QString::number( Get_SPICE().Get_Port() ),
						format );
			table->insertRows( table->rows(), 1 );
		}
		...
		if ( Settings.value( "Info/Linux_Boot", "no" ).toString() == "yes" )
		{
			...
			cell_cursor = cell.firstCursorPosition();
			cell_cursor.insertText(
				Get_Use_Linux_Boot() ? tr( "Yes" ) : tr( "No" ),
				format );
			table->insertRows( table->rows(), 1 );
	     ...
	     && !soph.preview )
	{
		...
		if ( Settings.value( "Info/RTC_TD_Hack", "no" ).toString() == "yes" )
		{
			...
			cell	    = table->cellAt( table->rows() - 1, 2 );
			cell_cursor = cell.firstCursorPosition();
			cell_cursor.insertText( Use_RTC_TD_Hack() ? tr( "Yes" ) : tr( "No" ),
						format );
			table->insertRows( table->rows(), 1 );
		}
		...
		if ( Settings.value( "Info/Win2K_Hack", "no" ).toString() == "yes" )
		{
			...
			cell	    = table->cellAt( table->rows() - 1, 2 );
			cell_cursor = cell.firstCursorPosition();
			cell_cursor.insertText( Use_Win2K_Hack() ? tr( "Yes" ) : tr( "No" ),
						format );
			table->insertRows( table->rows(), 1 );
		}
		...
		if ( Settings.value( "Info/No_Shutdown", "no" ).toString() == "yes" )
		{
			...
			cell	    = table->cellAt( table->rows() - 1, 2 );
			cell_cursor = cell.firstCursorPosition();
			cell_cursor.insertText( Use_No_Shutdown() ? tr( "Yes" ) : tr( "No" ),
						format );
			table->insertRows( table->rows(), 1 );
		}
		...
		if ( Settings.value( "Info/No_Reboot", "no" ).toString() == "yes" )
		{
			...
			cell	    = table->cellAt( table->rows() - 1, 2 );
			cell_cursor = cell.firstCursorPosition();
			cell_cursor.insertText( Use_No_Reboot() ? tr( "Yes" ) : tr( "No" ),
						format );
			table->insertRows( table->rows(), 1 );
		}
		...
		if ( Settings.value( "Info/Start_CPU", "no" ).toString() == "yes" )
		{
			...
			cell	    = table->cellAt( table->rows() - 1, 2 );
			cell_cursor = cell.firstCursorPosition();
			cell_cursor.insertText( Use_Start_CPU() ? tr( "Yes" ) : tr( "No" ),
						format );
			table->insertRows( table->rows(), 1 );
		}
		...
		if ( Settings.value( "Info/Check_Boot_on_FDD", "no" ).toString() == "yes" )
		{
			...
			cell_cursor = cell.firstCursorPosition();
			cell_cursor.insertText(
				Use_Check_FDD_Boot_Sector() ? tr( "Yes" ) : tr( "No" ),
				format );
			table->insertRows( table->rows(), 1 );
		}
		...
		if ( Settings.value( "Info/ACPI", "no" ).toString() == "yes" )
		{
			...
			cell	    = table->cellAt( table->rows() - 1, 2 );
			cell_cursor = cell.firstCursorPosition();
			cell_cursor.insertText( Use_ACPI() ? tr( "Yes" ) : tr( "No" ),
						format );
			table->insertRows( table->rows(), 1 );
		}
		...
		if ( Settings.value( "Info/No_Frame", "no" ).toString() == "yes" )
		{
			...
			cell	    = table->cellAt( table->rows() - 1, 2 );
			cell_cursor = cell.firstCursorPosition();
			cell_cursor.insertText( Use_No_Frame() ? tr( "Yes" ) : tr( "No" ),
						format );
			table->insertRows( table->rows(), 1 );
		}
		...
		if ( Settings.value( "Info/Alt_Grab", "no" ).toString() == "yes" )
		{
			...
			cell	    = table->cellAt( table->rows() - 1, 2 );
			cell_cursor = cell.firstCursorPosition();
			cell_cursor.insertText( Use_Alt_Grab() ? tr( "Yes" ) : tr( "No" ),
						format );
			table->insertRows( table->rows(), 1 );
		}
		...
		if ( Settings.value( "Info/No_Quit", "no" ).toString() == "yes" )
		{
			...
			cell	    = table->cellAt( table->rows() - 1, 2 );
			cell_cursor = cell.firstCursorPosition();
			cell_cursor.insertText( Use_No_Quit() ? tr( "Yes" ) : tr( "No" ),
						format );
			table->insertRows( table->rows(), 1 );
		}
		...
		if ( Settings.value( "Info/Portrait", "no" ).toString() == "yes" )
		{
			...
			cell	    = table->cellAt( table->rows() - 1, 2 );
			cell_cursor = cell.firstCursorPosition();
			cell_cursor.insertText( Use_Portrait() ? tr( "Yes" ) : tr( "No" ),
						format );
			table->insertRows( table->rows(), 1 );
		}
		...
		if ( Settings.value( "Info/Curses", "no" ).toString() == "yes" )
		{
			...
			cell	    = table->cellAt( table->rows() - 1, 2 );
			cell_cursor = cell.firstCursorPosition();
			cell_cursor.insertText( Use_Curses() ? tr( "Yes" ) : tr( "No" ),
						format );
			table->insertRows( table->rows(), 1 );
		}
		...
		if ( Settings.value( "Info/Show_Cursor", "no" ).toString() == "yes" )
		{
			...
			cell	    = table->cellAt( table->rows() - 1, 2 );
			cell_cursor = cell.firstCursorPosition();
			cell_cursor.insertText( Use_Show_Cursor() ? tr( "Yes" ) : tr( "No" ),
						format );
			table->insertRows( table->rows(), 1 );
		}
