...
Emulator Emulator_Options_Window::Get_Emulator() const { return Current_Emulator; }
...
void Emulator_Options_Window::Set_Emulator( const Emulator &emul )
{
	...
	if ( Current_Emulator.Get_Check_Version() )
		...
		ui.RB_Save_Options->setChecked( true );	   // Use saved emulator available options

	// Force Version
	switch ( Current_Emulator.Get_Version() )
	{
		case VM::QEMU_2_0: ui.CB_Version->setCurrentIndex( 1 ); break;

		default:
			AQWarning( "void Emulator_Options_Window::Set_Emulator( "
				   "Emulator emul )",
	...
	if ( Update_Info )
	{
		     ...
		     ui.RB_Check_Options->isChecked() )
		{
			...
			for ( int ix = 0; ix < ui.Table_Systems->rowCount(); ix++ )
			{
				...
				if ( ok )
					devList[ui.Table_Systems->item( ix, 0 )->text()] =
						tmpDev;
				else
					AQGraphic_Warning(
						tr( "Error!" ),
						tr( "Cannot get emulator info! For "
						    "file: %1" )
							.arg( ui.Table_Systems
								      ->item( ix, 1 )
								      ->text() ) );
			}
		...
		else if ( ui.RB_Check_Version->isChecked() )	// Update version
		{
			...
			if ( emul_version == VM::Obsolete )
				AQGraphic_Warning(
					tr( "Error!" ),
					tr( "Cannot get version for emulator!" ) );
			else
