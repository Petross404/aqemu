...
void VM_Wizard_Window::on_Button_Next_clicked()
{
	...
	else if ( ui.General_Settings_Page == ui.Wizard_Pages->currentWidget() )
	{
		...
		{
			if ( VM_List->at( vx )->Get_Machine_Name()
			     == ui.Edit_VM_Name->text() )
			{
				AQGraphic_Warning(
					tr( "Warning" ),
...
void VM_Wizard_Window::applyTemplate()
{
	...
	if ( ui.RB_VM_Template->isChecked() )
	{
		...
		{
			AQGraphic_Error(
				"void VM_Wizard_Window::Create_New_VM()",
				tr( "Error!" ),
	...
	if ( !devices_found )
	{
		AQGraphic_Error( "void VM_Wizard_Window::applyTemplate()",
				 tr( "Error!" ),
...
bool VM_Wizard_Window::Create_New_VM( bool simulate )
{
	...
	else { New_VM->Set_Icon_Path( icon_path ); }
	...
	else
	{
		...
		else
		{
			...
			{
				Current_Devices =
					&All_Systems["qemu-system-x86_64"];
				if ( !Current_Devices->System.QEMU_Name.isEmpty() )
					devices_found = true;
			}
			else	// QEMU
			{
				for ( QMap<QString, Available_Devices>::const_iterator it =
				      ...
				      it++ )
				{
					if ( it.value().System.Caption
					     == ui.CB_Computer_Type->currentText() )
					{
						Current_Devices = &it.value();
						devices_found	= true;
						break;
		...
		if ( !devices_found )
		{
			AQGraphic_Error( "bool VM_Wizard_Window::Create_New_VM()",
					 tr( "Error!" ),
...
QString VM_Wizard_Window::Find_OS_Icon( const QString os_name )
{
	if ( os_name.isEmpty() )
	{
		AQError( "QString VM_Wizard_Window::Find_OS_Icon( const QString "
			 "os_name )",
			 "os_name is Empty!" );
		return "";
	}
	...
	if ( rex.exactMatch( os_name ) ) return ":/default_linux.png";

	// Windows
	rex.setPattern( "*windows*" );
	if ( rex.exactMatch( os_name ) ) return ":/default_windows.png";
...
{
	if ( index == 0 ) { ui.Button_Next->setEnabled( false ); }
	else
	{
	     ...
	     && ( ui.CH_Remove_RAM_Size_Limitation->isChecked() == false ) )
	{
		AQGraphic_Warning( tr( "Error" ),
				   tr( "Your memory size %1 MB > %2 MB - all free "
				       "RAM on this system!\n"
				       "To setup this value, check \"Remove "
...
void VM_Wizard_Window::Update_RAM_Size_ComboBox( int freeRAM )
{
	...
	{
		AQError( "void VM_Wizard_Window::Update_RAM_Size_ComboBox( int "
			 "freeRAM )",
