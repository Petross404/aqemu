...
void Create_HDD_Image_Window::Set_Image_Info( VM::Disk_Info info )
{
	...
	if ( format_ix != -1 ) { ui.CB_Format->setCurrentIndex( format_ix ); }
	else
	{
		AQError( "void Create_HDD_Image_Window::Set_Image_Info( "
			 "VM::Disk_Info info )",
			 "Cannot Find Format" );
	}
...
void Create_HDD_Image_Window::on_Button_Create_clicked()
{
	...
	if ( ui.CH_Base_Image->isChecked() )
	{
		if ( !QFile::exists( ui.Edit_Base_Image_File_Name->text() ) )
		{
			AQGraphic_Warning(
				tr( "Error!" ),
				tr( "Base Image File doesn't Exists!" ) );
			return;
		}
		else
		{
			Create_OK = Create_New_HDD_Image(
				false,
				ui.Edit_Base_Image_File_Name->text(),
				ui.Edit_File_Name->text(),
				ui.CB_Format->currentText(),
				hd_size,
				true );
		}
	}
	...
	{
		Create_OK = Create_New_HDD_Image( false,
						  "",
						  ui.Edit_File_Name->text(),
						  ui.CB_Format->currentText(),
						  hd_size,
...
void Create_HDD_Image_Window::on_Button_Format_Help_clicked()
{
	QMessageBox::information(
		this,
		tr( "QEMU-IMG Supported formats" ),
		tr( "raw\nRaw disk image format. This format has the advantage of "
		    "being simple and easily "
		    "exportable to all other emulators. If your file system "
		    "supports holes (for example in "
		    "ext2 or ext3 on Linux or NTFS on Windows), then only the "
		    "written sectors will reserve "
		    "space. Use qemu-img info to know the real size used by the "
		    "image or ls -ls on Unix/Linux."
		    "\n\nqcow2\nQEMU image format, the most versatile format. Use "
		    "it to have smaller images "
		    "(useful if your filesystem does not supports holes, for "
		    "example on Windows), optional "
		    "AES encryption, zlib based compression and support of "
		    "multiple VM snapshots."
		    "\n\nqcow\nOld QEMU image format. Left for compatibility."
		    "\n\ncow\nUser Mode Linux Copy On Write image format. Used to "
		    "be the only growable image "
		    "format in QEMU. It is supported only for compatibility with "
		    "previous versions. "
