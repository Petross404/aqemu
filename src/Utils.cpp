...
{
	if ( Show_User_Graphic_Warning == false ) return;
...
void AQGraphic_Warning( const QString &sender, const QString &caption, const QString &mes, bool fatal )
{
	...
	if ( fatal )
	{
		QMessageBox::warning(
			...
			QString( "Sender: %1\nMessage: %2\n" ).arg( sender ).arg( mes )
				+ QObject::tr(
					"This Fatal Error. Recomend Close AQEMU." ),
			QMessageBox::Ok );
	}
	else
	{
	...
	if ( Save_Messages_To_Log ) AQSave_To_Log( "Warning", sender, mes );
}
...
void AQGraphic_Error( const QString &sender, const QString &caption, const QString &mes, bool fatal )
{
	if ( fatal )
	{
		QMessageBox::critical(
			NULL,
			caption,
			QString( "Sender: %1\nMessage: %2\n" ).arg( sender ).arg( mes ) + QObject::tr( "Fatal error. It's recommended to close AQEMU" ),
			QMessageBox::Ok );
	}
	else
	{
	...
	if ( Save_Messages_To_Log ) AQSave_To_Log( "Error", sender, mes );
}
...
void AQSave_To_Log( const QString &mes_type, const QString &sender, const QString &mes )
{
	...
	if ( !log_file.open( QIODevice::Append | QIODevice::Text ) )
	{
		...
		AQError( "void AQSave_To_Log( const QString& mes_type, const "
			 "QString& sender, const QString& mes )",
...
bool Create_New_HDD_Image( bool		   encrypted,
			   const QString  &base_image,
			   ...
			   bool		   verbose )
{
	...
	if ( !base_image.isEmpty() ) args << "-b" << base_image;
	...
	if ( !qemu_img.waitForFinished( 10000 ) )
	{
		AQGraphic_Error(
			...
			QObject::tr( "Error!" ),
			QObject::tr(
				"qemu-img Cannot Finish! Image isn't Created!" ) );
		return false;
	}
	else
	{
		...
		{
			AQGraphic_Error(
				"bool Create_New_HDD_Image( bool encrypted, const "
				"QString &base_image,"
				"const QString &file_name, const QString &format, "
				"VM::Device_Size size, bool verbose )",
				QObject::tr( "Error!" ),
		...
		if ( verbose )
		{
			...
			else
			{
				QMessageBox::information( NULL,
							  ...
							  QObject::
									  tr(
										  "QEMU-IMG is Returned non Standard Message!.\nAdditional Information:\n" )
								  + out );
			}
		}
...
QList<QString> Get_Templates_List()
{
	...
	QFileInfoList sys_templates_list =
		sys_templates_dir.entryInfoList( QStringList( "*.aqvmt" ),
						 QDir::Files,
						 QDir::Unsorted );
	...
	QFileInfoList user_templates_list =
		user_templates_dir.entryInfoList( QStringList( "*.aqvmt" ),
						  QDir::Files,
						  QDir::Unsorted );
...
QString Get_Complete_VM_File_Path( const QString &vm_name )
{
	...
	if ( !ret_str.endsWith( ".aqemu" ) ) ret_str += ".aqemu";
...
QString Get_TR_Size_Suffix( VM::Device_Size suf )
{
	switch ( suf.Suffix )
	{
		case VM::Size_Suf_Kb: return QObject::tr( "Kb" );

		case VM::Size_Suf_Mb: return QObject::tr( "Mb" );

		case VM::Size_Suf_Gb: return QObject::tr( "Gb" );
	...
	if ( test_perm.exists() )
	{
		if ( !test_perm.isWritable() )
		{
			AQGraphic_Error(
				"void Check_AQEMU_Permissions()",
				QObject::tr( "Error!" ),
				QObject::tr( "AQEMU Config File is Read "
	...
	if ( test_perm.exists() )
	{
		if ( !test_perm.isWritable() )
		{
			AQGraphic_Error(
				"void Check_AQEMU_Permissions()",
				QObject::tr( "Error!" ),
				QObject::tr( "AQEMU VM Directory is Read "
		...
		if ( test_perm.exists() )
		{
			if ( !test_perm.isWritable() )
			{
				AQGraphic_Error(
					"void Check_AQEMU_Permissions()",
					QObject::tr( "Error!" ),
					QObject::tr( "AQEMU Log File is Read "
						     "Only!\nCheck Permissions For "
						     "File: " )
...
VM::Emulator_Version String_To_Emulator_Version( const QString &str )
{
	...
	{
		AQError( "VM::Emulator_Version String_To_Emulator_Version( const "
			 "QString &str )",
...
QString Emulator_Version_To_String( VM::Emulator_Version ver )
{
	switch ( ver )
	{
		case VM::QEMU_2_0: return "QEMU 2.0";

		case VM::Obsolete: return "Obsolete";

		default:
			AQError( "QString Emulator_Version_To_String( "
				 "VM::Emulator_Version ver )",
...
bool Update_Emulators_List()
{
	...
	if ( !( aqemuSettingsFolder.endsWith( "/" ) || aqemuSettingsFolder.endsWith( "\\" ) ) )
		aqemuSettingsFolder =
			QDir::toNativeSeparators( aqemuSettingsFolder + "/" );
	...
	for ( int ex = 0; ex < emulFiles.count(); ++ex )
	{
		...
		{
			AQError( "bool Update_Emulators_List()",
				 QString( "Cannot load emulator from file: \"%1\"" )
					 .arg( emulFiles[ex] ) );
			continue;
		}
		...
		if ( tmp.Get_Check_Available_Options() )
		{
			...
			for ( QMap<QString, QString>::const_iterator it =
				      tmpBinFiles.constBegin();
			      it != tmpBinFiles.constEnd();
			      ++it )
			{
				if ( !it.value().isEmpty() )
				{
		...
		if ( tmp.Get_Check_Version() )
		{
			...
			if ( binFilePath.isEmpty() )
			{
				AQError(
					"bool Update_Emulators_List()",
					QString(
						"Cannot find exists emulator "
						"binary file for emulator \"%1\"!" )
						.arg( tmp.Get_Name() ) );
			}
...
bool Remove_All_Emulators_Files()
{
	...
	else
	{
		...
		QDir	    emulDir( aqemuSettingsFolder );
		QStringList emulFiles =
			emulDir.entryList( QStringList( "*.emulator" ),
					   QDir::Files,
					   QDir::Name );
...
{
	if ( min < 0 || max > RAND_MAX || min > max ) { return -1; }
...
void Load_Recent_Images_List()
{
	...
	{
		QString tmp = settings.value( "CD_ROM_Existing_Images/item"
						      + QString::number( ix ),
					      "" )
				      .toString();
	...
	{
		QString tmp = settings.value( "Floppy_Existing_Images/item"
						      + QString::number( ix ),
					      "" )
				      .toString();
...
void Add_To_Recent_CD_Files( const QString &path )
{
	...
	else
	{
		...
		{
			settings.setValue(
				"CD_ROM_Existing_Images/item" + QString::number( ix ),
				Recent_CD_Images[ix] );
		}
	}
}
...
void Add_To_Recent_FDD_Files( const QString &path )
{
	...
	for ( int fx = 0; fx < Recent_FDD_Images.count() && fx < max; ++fx )
	{
		...
		{
			AQDebug( "void Add_To_Recent_FDD_Files( const QString "
				 "&path )",
	...
	else
	{
		...
		{
			settings.setValue(
				"Floppy_Existing_Images/item" + QString::number( ix ),
				     ...
				     bool	       enabled )
{
	...
	if ( !checkbox->isChecked() ) enabled = false;
