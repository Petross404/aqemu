...
void Edit_Emulator_Version_Window::Load_Emulators()
{
	...
	{
		new QListWidgetItem( tr( "%1 (Located in %2)" )
					     .arg( Emulator_Version_To_String(
						     Emulators[ix].Get_Version() ) )
...
void Edit_Emulator_Version_Window::on_List_Emulators_currentRowChanged( int currentRow )
{
	...
	if ( currentRow >= 0 && currentRow < ui.List_Emulators->count() )
	{
		...
		item_index = ui.CB_Versions->findText( Emulator_Version_To_String(
			Emulators[currentRow].Get_Version() ) );

		// Select Version
		if ( item_index < 0 )
			AQError( "void "
				 "Edit_Emulator_Version_Window::on_List_Emulators_"
				 "currentRowChanged( int currentRow )",
				 "Cannot Find Version: "
					 + Emulator_Version_To_String(
						 Emulators[currentRow].Get_Version() ) );
		else
			ui.CB_Versions->setCurrentIndex( item_index );
	}

	connect( ui.CB_Versions,
		 SIGNAL( currentIndexChanged( const QString & ) ),
		 this,
		 SLOT( CB_Versions_currentIndexChanged( const QString & ) ) );
}
