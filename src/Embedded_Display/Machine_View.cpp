...
void MachineView::connectView()
{
	...
	connect( View,
		 SIGNAL( framebufferSizeChanged( int, int ) ),
		 this,
		 SLOT( newViewSize( int, int ) ) );

	connect( View,
		 SIGNAL( MouseEnteredFromTheLeft() ),
		 this,
		 SLOT( on_MouseEnteredFromTheLeft() ) );
	connect( View,
		 SIGNAL( MouseEnteredFromTheRight() ),
		 this,
		 SLOT( on_MouseEnteredFromTheRight() ) );
	connect( View,
		 SIGNAL( MouseEnteredFromTheTop() ),
		 this,
		 SLOT( on_MouseEnteredFromTheTop() ) );
	connect( View,
		 SIGNAL( MouseEnteredFromTheBottom() ),
		 this,
...
void MachineView::Check_Connection()
{
	if ( !VNC_Connected )
	{
		connect( Reinit_Timer, SIGNAL( timeout() ), this, SLOT( initView() ) );	   // FIXME reinitVNC()
...
void MachineView::reinitVNC()
{
	...
	else
	{
		...
		AQGraphic_Error( "void MachineView::reinitVNC()",
				 tr( "Error!" ),
				 tr( "Cannot Reinit VNC Connection!" ),
				 false );
	}
}
