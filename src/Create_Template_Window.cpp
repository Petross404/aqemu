		...
		for ( int ix = 0; ix < List_Templates.count(); ++ix )
		{
			...
			if ( tmp_info.completeBaseName() == new_template_name )
			{
				int mes_ret = QMessageBox::question(
					this,
					tr( "Replace" ),
					tr( "Your template name is already used! "
					    "Do you want replace previous "
					    "template?" ),
					QMessageBox::Yes | QMessageBox::No,
					QMessageBox::No );

				if ( mes_ret == QMessageBox::Yes ) { return true; }
				else { return false; }
			}
		}
	...
	else
	{
		...
		if ( QFile::exists( ui.Edit_Template_Folder->text() ) )
		{
			int mes_ret = QMessageBox::question(
				this,
				tr( "Replace" ),
				tr( "You Name For Template in Not Unique! Replace "
				    "Previous Template?" ),
				QMessageBox::Yes | QMessageBox::No,
				QMessageBox::No );
