...
SPICE_Settings_Widget::SPICE_Settings_Widget( QWidget *parent )
	: QWidget{ parent }
	, ui{ std::make_unique<Ui::SPICE_Settings_Widget>() }
{
	ui->setupUi( this );
	...
	connect( ui->Edit_SPICE_Port,
		 SIGNAL( valueChanged( int ) ),
		 this,
		 SIGNAL( State_Changed() ) );
	connect( ui->CH_SPICE_SPort, SIGNAL( clicked() ), this, SIGNAL( State_Changed() ) );
	connect( ui->Edit_SPICE_SPort,
		 SIGNAL( valueChanged( int ) ),
		 this,
		 SIGNAL( State_Changed() ) );
	connect( ui->CH_SPICE_Host, SIGNAL( clicked() ), this, SIGNAL( State_Changed() ) );
	connect( ui->Edit_SPICE_Host,
		 SIGNAL( textChanged( const QString & ) ),
		 this,
		 SIGNAL( State_Changed() ) );

	connect( ui->CH_Use_Image_Compression,
		 SIGNAL( clicked() ),
		 this,
		 SIGNAL( State_Changed() ) );
	connect( ui->CB_Image_Compression,
	...
	connect( ui->CH_Use_Video_Compression,
		 SIGNAL( clicked() ),
		 this,
		 SIGNAL( State_Changed() ) );
	connect( ui->CH_Use_Playback_Compression,
		 SIGNAL( clicked() ),
		 this,
		 SIGNAL( State_Changed() ) );
...
const VM_SPICE &SPICE_Settings_Widget::Get_Settings( bool &settingsValidated )
{
	...
	/*if( val > 0 ) spiceSettings.Set_Port( val ); //TODO: seperate validation
	...
	{
		tmpRendersList
			<< (VM::SPICE_Renderer)ui->Renderer_Order_List->item( ix )
				   ->data( Qt::UserRole )
				   .toInt();
	}
...
void SPICE_Settings_Widget::Set_Settings( const VM_SPICE &settings )
{
	...
	switch ( settings.Get_Image_Compression() )
	{
		...
		default: ui->CB_Image_Compression->setCurrentIndex( 0 ); break;
	}
...
void SPICE_Settings_Widget::on_TB_Up_clicked()
{
	...
	QString tmpStr = ui->Renderer_Order_List
				 ...
				 ->text();
	VM::SPICE_Renderer tmpRender =
		(VM::SPICE_Renderer)ui->Renderer_Order_List
			->item( ui->Renderer_Order_List->row(
				ui->Renderer_Order_List->currentItem() ) )
			->data( Qt::UserRole )
			.toInt();
	...
	for ( int ix = 0; ix < list.count(); ++ix )
	{
		...
		switch ( list[ix] )
		{
			...
			default:
				...
				AQError(
					"void "
					"SPICE_Settings_Widget::Show_Renderer_List("
					" const QList<VM::SPICE_Renderer> &list )",
					"Invalid SPICE render type!" );
				break;
		}

		QListWidgetItem *tmpItem = new QListWidgetItem( renderStr,
								ui->Renderer_Order_List,
